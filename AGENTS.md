...
Before editing or creating a file, read every `AGENTS.md` along its directory path, starting at the
repository root. Follow each file's instructions within its directory and descendants. More specific
instructions take precedence within their scope. Do this even when the session starts at the
repository root.
...
1. All PRs need a `prdoc` file unless labeled `R0-silent`
...
When a prdoc is required, a branch has exactly one. Every change made on the branch, including
follow-up work added later, must be folded into that branch's own `prdoc/pr_<NUMBER>.prdoc`. Never
...
The `R0-no-crate-publish-required` label means no crates need publishing; it does not waive the prdoc
requirement. Apply both labels when both conditions hold.
