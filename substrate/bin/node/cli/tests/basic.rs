...
fn xt() -> UncheckedExtrinsic {
	sign(CheckedExtrinsic {
		format: sp_runtime::generic::ExtrinsicFormat::Signed(alice(), tx_ext(0, 0), Default::default()),
...
fn blocks() -> ((Vec<u8>, Hash), (Vec<u8>, Hash)) {
	...
	let block2 = construct_block(
		...
		vec![
			...
			CheckedExtrinsic {
				format: sp_runtime::generic::ExtrinsicFormat::Signed(bob(), tx_ext(0, 0), Default::default()),
				function: RuntimeCall::Balances(pallet_balances::Call::transfer_allow_death {
					dest: alice().into(),
					value: 5 * DOLLARS,
				}),
			...
			CheckedExtrinsic {
				format: sp_runtime::generic::ExtrinsicFormat::Signed(alice(), tx_ext(1, 0), Default::default()),
				function: RuntimeCall::Balances(pallet_balances::Call::transfer_allow_death {
					dest: bob().into(),
					value: 15 * DOLLARS,
				}),
...
fn block_with_size(time: u64, nonce: u32, size: usize) -> (Vec<u8>, Hash) {
	construct_block(
		...
		vec![
			...
			CheckedExtrinsic {
				format: sp_runtime::generic::ExtrinsicFormat::Signed(alice(), tx_ext(nonce, 0), Default::default()),
