...
mod std_reexport {
	pub use crate::{
		...
		trie_backend::{create_proof_check_backend, AsDB},
	};
	pub use sp_trie::{
...
mod execution {
	...
	pub type InMemoryBackend<H> = TrieBackend<H>;
	...
	where
		...
		Exec: CodeExecutor + Clone + 'static,
	{
		let trie_backend = backend.as_trie_backend_mut();
		prove_execution_on_trie_backend::<_, _>(
			trie_backend,
			overlay,
			exec,
			method,
	...
	/// Note: changes to code will be in place if this call is made again. For running partial
	/// blocks (e.g. a transaction at a time), ensure a different method is used.
	pub fn prove_execution_on_trie_backend<H, Exec>(
	...
	pub fn prove_range_read_with_child_with_size_on_trie_backend<H>(
		trie_backend: &TrieBackend<H>,
		size_limit: usize,
		start_at: &[Vec<u8>],
	) -> Result<(StorageProof, u32), Box<dyn Error>>
	...
	pub fn prove_range_read_with_size_on_trie_backend<H>(
		trie_backend: &mut TrieBackend<H>,
		child_info: Option<&ChildInfo>,
		prefix: Option<&[u8]>,
		size_limit: usize,
		start_at: Option<&[u8]>,
	...
	pub fn prove_read_on_trie_backend<H, I>(
		trie_backend: &mut TrieBackend<H>,
		keys: I,
	) -> Result<StorageProof, Box<dyn Error>>
	where
	...
	/// Generate storage read proof on pre-created trie backend.
	pub fn prove_child_read_on_trie_backend<H, I>(
		trie_backend: &mut TrieBackend<H>,
		child_info: &ChildInfo,
		keys: I,
	) -> Result<StorageProof, Box<dyn Error>>
...
mod tests {
	...
	fn inner_state_versioning_switch_proofs() {
		...
		{
			let mut trie = TrieDBMutBuilderV0::from_existing(&mut mdb, root).build();
			...
			trie.insert(b"foo222", vec![5u8; 100].as_slice()) // inner hash
				.expect("insert failed");
			root = trie.commit().apply_to(&mut mdb);
		}
		...
		{
			let mut trie = TrieDBMutBuilderV1::from_existing(&mdb, root).build();
			trie.insert(b"foo222", vec![5u8; 100].as_slice()) // inner hash
				.expect("insert failed");
	...
	fn compact_multiple_child_trie_inner(state_version: StateVersion) -> usize {
		...
		let transaction = remote_backend.full_storage_root(
			std::iter::empty(),
			vec![
				(
					&child_info1,
