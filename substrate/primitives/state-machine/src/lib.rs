...
mod std_reexport {
	pub use crate::{
		state_backend::ProofRecorder, testing::TestExternalities,
...
mod execution {
	use crate::{
		...
		state_backend::StateBackendBuilder,
	};
		...
		Exec: CodeExecutor + 'static + Clone,
	{
		...
		todo!("handle sp_state_machine::state_backend::StorageProof");
		// Ok((result, proof))
	}
	...
	where
		...
		I::Item: AsRef<[u8]>,
	{
		prove_read_on_backend(backend, keys)
		// let trie_backend = backend.as_trie_backend();
	...
	/// Generate storage read proof on pre-created trie backend.
	pub fn prove_read_on_backend<B, H, I>(
		backend: B,
		keys: I,
	) -> Result<StorageProof, Box<dyn Error>>
	where
