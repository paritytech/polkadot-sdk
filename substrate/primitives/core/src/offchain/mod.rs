...
pub trait OffchainStorage: Clone + Send + Sync {
	/// Persist a value in storage under the given key and prefix.
	fn set(&mut self, prefix: &[u8], key: &[u8], value: &[u8]);

	/// Clear a storage entry under the given key and prefix.
	fn remove(&mut self, prefix: &[u8], key: &[u8]);

	/// Retrieve a value from storage under the given key and prefix.
...
#[repr(C)]
pub enum HttpError {
	/// The requested action couldn't be completed within a deadline.
...
impl Duration {
	/// Create new duration representing the given number of milliseconds.
...
pub trait Externalities: Send {
	...
	/// - The name or the value contain invalid characters.
	///
	...
	/// is reached, or the server closed the connection.
	...
	/// each one is identified with an `OpaquePeerId`, here it just uses plain bytes
...
/// An externalities extension for accessing the Offchain DB.
pub trait DbExternalities: Send {
	...
	/// Sets a value in the local storage if it matches the current value.
	///
	/// Since multiple off-chain workers may be running concurrently, use CAS to
	/// coordinate between them, and prevent data races.
	///
...
/// Change to be applied to the offchain worker db in regard to a key.
