...
pub mod ecdsa_bls377 {
	...
	#[cfg(feature = "full_crypto")]
	impl Pair {
		/// Hashes the `message` with the specified [`Hasher`] and then signs it using ECDSA
		/// algorithm.
		///
		/// The hasher does not affect the behavior of BLS12-377 component. This generates
		...
		/// Hashes the `message` with the specified [`Hasher`] and then verifies it using ECDSA
		/// algorithm.
		///
		/// The hasher does not affect the behavior of BLS12-377 component. This verifies whether
		/// the BLS12-377 Signature was hashed and signed according to IETF standard and disregards
		/// the hasher for the BLS12-377 component.
