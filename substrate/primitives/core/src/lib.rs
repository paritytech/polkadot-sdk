...
// everybody.
//
// TRACING/DEV ONLY (this branch): raised 2^25 -> 2^28 so the ExecutionTracer can
// hold full EVM traces (~150k+ steps) in its `struct_logs` Vec — the Vec's
// doubling growth requests one contiguous allocation past 32 MiB at ~131k steps.
// Must move together with `N_ORDERS` in `sc-allocator`. Do NOT ship: allocation
// failure behavior is consensus-observable.
