...
// The Polkadot host function set: the host functions as they exist on Polkadot today, which is
// the content of this crate's `lib.rs` before the JAM host function set was added. This file is
// compiled in builds without `--cfg jam`; the JAM set lives in `jam_io.rs`.
use alloc::vec::Vec;
