...
/// `HostAllocator`, but also pointers allocated by the host. The header is stored as a
...
///
/// The `HostAllocator` aligns the pointer to the required alignment before returning it to the
...
impl HostAllocator {
	...
	pub(crate) unsafe fn dealloc(&self, ptr: *mut u8) {
