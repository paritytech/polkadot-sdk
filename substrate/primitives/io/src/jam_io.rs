...
// Host function set for the JAM mode: the host functions specified by [RFC-145](
// https://polkadot-fellows.github.io/RFCs/approved/0145-remove-unnecessary-allocator-usage.html),
// which never make the host allocate runtime memory. This file is compiled instead of
// `polkadot_io.rs` in builds with `--cfg jam`.
//
// The Rust API of the modules is the one of `polkadot_io.rs`, so runtime code compiles unchanged
// in both builds: where the RFC changed a host function to write into a runtime-provided buffer,
// a `#[wrapper]` function with the pre-RFC signature is provided on top of the raw host function.
...
#[cfg(any(feature = "bls-experimental", feature = "bandersnatch-experimental"))]
use sp_runtime_interface::pass_by::{AllocateAndReturnByCodec, AllocateAndReturnPointer};
...
/// Number of error variants of an FFI error code enum. The error codes are the negative
/// integers `-1..=-COUNT`.
trait FfiErrorCode {
	const COUNT: i64;
}
...
impl FfiErrorCode for RIEcdsaVerifyError {
	const COUNT: i64 = 3;
}
...
impl FfiErrorCode for RIHttpError {
	const COUNT: i64 = 3;
}
...
impl FfiErrorCode for VoidError {
	const COUNT: i64 = 1;
}
...
impl<R: Into<i64> + IntoI64, E: Into<i64> + FfiErrorCode> TryFrom<RIIntResult<R, E>> for i64 {
	type Error = ();
	...
	fn try_from(result: RIIntResult<R, E>) -> Result<Self, ()> {
		match result {
			...
			RIIntResult::Err(e) => {
				...
				if error_code < 0 && error_code >= -E::COUNT {
					Ok(error_code)
				} else {
					Err(())
				}
...
impl<R: TryFrom<i64> + IntoI64, E: TryFrom<i64> + FfiErrorCode> TryFrom<i64> for RIIntResult<R, E> {
	type Error = ();

	fn try_from(value: i64) -> Result<Self, Self::Error> {
		if value >= 0 && value <= R::MAX.into() {
			Ok(RIIntResult::Ok(value.try_into().map_err(|_| ())?))
		} else if value < 0 && value >= -E::COUNT {
...
impl<R: TryFrom<i64> + IntoI64, E: TryFrom<i64> + FfiErrorCode> TryFrom<i32> for RIIntResult<R, E> {
	type Error = ();
...
impl<E: Into<i64> + FfiErrorCode> TryFrom<RIIntResult<VoidResult, E>> for i32 {
	type Error = ();
...
/// Length of the buffer the first read of a storage value of unknown length goes into. Values
/// that don't fit are fetched with a second host call.
const INITIAL_VALUE_BUFFER_LEN: usize = 256;

/// Length of the buffer the first read of a storage key of unknown length goes into. Keys that
/// don't fit are fetched with a second host call.
const INITIAL_KEY_BUFFER_LEN: usize = 128;

/// Length in bytes of a state root, as assumed by the JAM host function ABI.
const STATE_ROOT_LEN: usize = 32;

/// Reads a value of unknown length through a `read`-style host function.
///
/// `read` gets the output buffer, the offset into the value and the `allow_partial` flag, and
/// returns the length of the value beyond the offset, or `None` if there is no value. The first
/// call reads a partial prefix into a buffer of [`INITIAL_VALUE_BUFFER_LEN`] bytes; a value that
/// doesn't fit is completed with a second, exact call for its remainder.
fn read_value(mut read: impl FnMut(&mut [u8], u32, u32) -> Option<u32>) -> Option<Vec<u8>> {
	let mut value = vec![0u8; INITIAL_VALUE_BUFFER_LEN];
	let len = read(&mut value[..], 0, 1)? as usize;
	if len > value.len() {
		let head = value.len();
		value.resize(len, 0);
		read(&mut value[head..], head as u32, 0)?;
	}
	value.truncate(len);
	Some(value)
}

/// Reads a key of unknown length through a `next_key`-style host function.
///
/// `next_key` gets the output buffer and returns the full length of the key (`0` if there is
/// none), writing the key only if it fits the buffer. A key that doesn't fit the
/// [`INITIAL_KEY_BUFFER_LEN`] bytes buffer is fetched with a second call.
fn read_next_key(mut next_key: impl FnMut(&mut [u8]) -> u32) -> Option<Vec<u8>> {
	let mut key = vec![0u8; INITIAL_KEY_BUFFER_LEN];
	let len = next_key(&mut key[..]) as usize;
	if len == 0 {
		return None;
	}
	if len > key.len() {
...
/// Converts the result of a cursor based removal host function into the [`KillStorageResult`]
/// of the pre-RFC-145 interface: the removal is complete when no continuation cursor was
/// returned.
fn kill_storage_result(cursor_len: u32, counters: &StorageIterations) -> KillStorageResult {
	// `loops` matches the pre-RFC-145 host functions, see `From<MultiRemovalResults>`.
	if cursor_len == 0 {
		KillStorageResult::AllRemoved(counters.loops)
	} else {
		KillStorageResult::SomeRemaining(counters.loops)
	}
}
...
pub trait Storage {
	...
	fn read(key: &[u8], value_out: &mut [u8], value_offset: u32) -> Option<u32> {
		read__raw(key, value_out, value_offset, 1)
	}

	/// Returns the data for `key` in the storage or `None` if the key can not be found.
	#[wrapper]
	fn get(key: &[u8]) -> Option<bytes::Bytes> {
		read_value(|value_out, value_offset, allow_partial| {
			read__raw(key, value_out, value_offset, allow_partial)
		})
		.map(Into::into)
	}
	...
	/// backend.
	#[wrapper]
	fn clear_prefix(prefix: &[u8], limit: Option<u32>) -> KillStorageResult {
		let mut counters = StorageIterations::default();
		// The continuation cursor is not exposed by this interface, so no buffer is provided for
		// it; whether one exists is all that matters.
		let cursor_len = clear_prefix__raw(prefix, limit, None, &mut [], &mut counters);
		kill_storage_result(cursor_len, &counters)
	}
	...
	///
	/// The hashing algorithm is defined by the `Block`. The trie layout is defined by the state
	/// version declared by the runtime (PPP#6), see `Externalities::runtime_state_version`.
	///
	...
	fn root(&mut self, out: PassFatPointerAndWrite<&mut [u8]>) {
		let root = self.storage_root(self.runtime_state_version());
		assert!(
			out.len() >= root.len(),
	...
	/// environment, which the host learns from the `RuntimeVersion` of the runtime (PPP#6). The
	/// JAM host function ABI assumes [`STATE_ROOT_LEN`] bytes long state roots.
	#[wrapper]
	fn root(_version: StateVersion) -> Vec<u8> {
		let mut root = vec![0u8; STATE_ROOT_LEN];
		root__raw(&mut root[..]);
		root
	}
	...
	fn next_key(key: &[u8]) -> Option<Vec<u8>> {
		read_next_key(|key_out| next_key__raw(key, key_out))
	}
...
pub trait DefaultChildStorage {
	...
	fn read(
		...
		read__raw(storage_key, key, value_out, value_offset, 1)
	}
	...
	fn get(storage_key: &[u8], key: &[u8]) -> Option<Vec<u8>> {
		read_value(|value_out, value_offset, allow_partial| {
			read__raw(storage_key, key, value_out, value_offset, allow_partial)
		})
	}
	...
	///
	/// If it exists, the child storage for `storage_key` is removed.
	///
	/// See `Storage` module `clear_prefix` documentation for `limit` usage and the cursor based
	/// interface.
	...
	#[wrapper]
	fn storage_kill(storage_key: &[u8], limit: Option<u32>) -> KillStorageResult {
		let mut counters = StorageIterations::default();
		let cursor_len = storage_kill__raw(storage_key, limit, None, &mut [], &mut counters);
		kill_storage_result(cursor_len, &counters)
	}
	...
	/// See `Storage` module `clear_prefix` documentation for `limit` usage.
	#[wrapper]
	fn clear_prefix(storage_key: &[u8], prefix: &[u8], limit: Option<u32>) -> KillStorageResult {
		let mut counters = StorageIterations::default();
		let cursor_len =
			clear_prefix__raw(storage_key, prefix, limit, None, &mut [], &mut counters);
		kill_storage_result(cursor_len, &counters)
	}
	...
	fn root(
		...
		let child_info = ChildInfo::new_default(storage_key);
		let root = self.child_storage_root(&child_info, self.runtime_state_version());
		assert!(
			out.len() >= root.len(),
	...
	/// Returns a `Vec<u8>` that holds the SCALE encoded hash.
	///
	/// The `version` argument is ignored, see `storage::root`.
	#[wrapper]
	fn root(storage_key: &[u8], _version: StateVersion) -> Vec<u8> {
		let mut root = vec![0u8; STATE_ROOT_LEN];
		root__raw(storage_key, &mut root[..]);
		root
	}
	...
	/// Get the next key in storage after the given one in lexicographic order in child storage.
	#[wrapper]
	fn next_key(storage_key: &[u8], key: &[u8]) -> Option<Vec<u8>> {
		read_next_key(|key_out| next_key__raw(storage_key, key, key_out))
	}
}
...
pub trait Misc {
	...
	fn runtime_version(wasm: &[u8]) -> Option<Vec<u8>> {
		...
		let len = runtime_version__raw(wasm, &mut version[..])?;
		if len as usize > version.len() {
			version.resize(len as usize, 0);
			runtime_version__raw(wasm, &mut version[..])?;
		}
...
pub trait Crypto {
	...
	// NOTE: The experimental key generation and signing host functions below still make the host
	// allocate runtime memory, as RFC-145 does not specify replacements for them. They are
	// therefore not available to JAM runtimes; calling them fails as the host cannot allocate
	// runtime memory.
...
pub trait Offchain {
	...
	///
	/// Only the peer id of the local node is available through the JAM host functions, the list
	/// of external addresses is always empty.
	#[wrapper]
	fn network_state() -> Result<OpaqueNetworkState, ()> {
		let mut peer_id = NetworkPeerId::default();
		network_peer_id__raw(&mut peer_id)?;
		Ok(OpaqueNetworkState {
			peer_id: OpaquePeerId::new(peer_id.0.to_vec().encode()),
	...
	fn local_storage_get(kind: StorageKind, key: &[u8]) -> Option<Vec<u8>> {
		...
		let len = local_storage_read(kind, key, &mut value_out[..], 0)?;
		if len as usize > value_out.len() {
			value_out.resize(len as usize, 0);
			local_storage_read(kind, key, &mut value_out[..], 0)?;
		}
	...
	fn http_response_wait(
		...
		http_response_wait__raw(ids, deadline, &mut statuses[..]);
		statuses
			.into_iter()
