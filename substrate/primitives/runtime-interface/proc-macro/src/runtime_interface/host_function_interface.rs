...
pub fn generate(trait_def: &ItemTrait, is_wasm_only: bool) -> Result<TokenStream> {
	...
	let extern_host_function_impls = get_runtime_interface(trait_def)?
		...
		.try_fold(TokenStream::new(), |mut t, (version, method)| {
			...
			t.extend(generate_extern_host_function(method, version, trait_name, fn_name)?);
			Ok::<_, Error>(t)
		})?;
	// Per-version extern host functions, under `name_version_N` names. They back the
	// per-version bindings generated by the bare function interface.
	let extern_host_function_impls = get_runtime_interface(trait_def)?.all_versions().try_fold(
	...
	let exchangeable_host_functions = get_runtime_interface(trait_def)?
		...
		t.extend(generate_exchangeable_host_function(m, fn_name)?);
		Ok::<_, Error>(t)
	})?;
	// Per-version exchangeable host functions, under `host_name_version_N` names. They back
	// the per-version bindings generated by the bare function interface.
	let exchangeable_host_functions = get_runtime_interface(trait_def)?.all_versions().try_fold(
		exchangeable_host_functions,
		|mut t, (version, method)| {
			let fn_name = create_function_ident_with_version(&method.sig.ident, version);
			t.extend(generate_exchangeable_host_function(method, fn_name)?);
			Ok::<_, Error>(t)
		},
	)?;
...
///
/// The generated function is named `fn_name`: the plain method name for the latest callable
/// version, or `name_version_N` for `register_only` versions.
...
///
/// The generated static is named `host_<fn_name>`: `fn_name` is the plain method name for the
/// latest callable version, or `name_version_N` for `register_only` versions.
fn generate_exchangeable_host_function(
	...
	let arg_types = get_function_argument_types(&method.sig);
	let function = &fn_name;
	let exchangeable_function = create_exchangeable_host_function_ident(&fn_name);
	let doc_string = format!(" Exchangeable host function used by [`{}`].", fn_name);
...
fn generate_host_function_implementation(
	...
	// Conditional compilation of versioned functions is validated in
	// `utils::get_runtime_interface`: all callable versions of a function must carry identical
	// `#[cfg]` attributes and `register_only` versions must carry none.
	let cfg_attrs: Vec<_> =
