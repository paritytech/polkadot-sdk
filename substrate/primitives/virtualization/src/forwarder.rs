...
impl Module {
	...
	///
	/// Every subsequent [`Module::lookup`] misses, as at the start of a fresh block. Dry-run
	/// paths call this so a transaction is priced as the first one in its block.
	pub fn clear_cache() {
		host_fn::clear_module_cache()
	}
