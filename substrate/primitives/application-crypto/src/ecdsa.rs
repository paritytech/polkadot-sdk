...
impl RuntimePublic for Public {
	...
	fn generate_pair(key_type: KeyTypeId, seed: Option<Vec<u8>>) -> Self {
		let mut out = sp_core::ecdsa::Public::default();
		sp_io::crypto::ecdsa_generate(key_type, seed, &mut out);
		out.into()
	}

	fn sign<M: AsRef<[u8]>>(&self, key_type: KeyTypeId, msg: &M) -> Option<Self::Signature> {
		let mut out = sp_core::ecdsa::Signature::default();
		sp_io::crypto::ecdsa_sign(key_type, self, msg.as_ref(), &mut out).ok()?;
		Some(out)
	}
	...
	fn generate_proof_of_possession(&mut self, key_type: KeyTypeId) -> Option<Self::Signature> {
		...
		sp_io::crypto::ecdsa_sign(key_type, self, &proof_of_possession_statement, &mut out).ok()?;
		Some(out)
	}
