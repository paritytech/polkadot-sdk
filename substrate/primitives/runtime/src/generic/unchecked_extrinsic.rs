...
impl<T: DecodeWithMemLimit, const MAX_HEAP_SIZE: usize> LazyCall<T, MAX_HEAP_SIZE> {
	...
	pub fn try_get_or_decode(&mut self) -> Result<&T, codec::Error> {
		if let LazyCall::Encoded(bytes) = self {
			let call = T::decode_with_mem_limit(&mut &bytes[..], MAX_HEAP_SIZE)?;
...
pub struct UncheckedExtrinsic<Address, Call, Signature, Extension> {
	...
	/// The function that should be called. It is initially stored in the form that it is provided
	/// (encoded/decoded) and if needed it will be decoded (and probably also cached)
	/// the first time it is accessed.
	#[codec(skip)]
	pub call: LazyCall<Call, MAX_CALL_HEAP_SIZE>,
}
	...
	Self: 'a,
{
	...
	fn try_as_full(&'a mut self) -> Result<Self::FullExtrinsicRef, codec::Error> {
		Ok(FullUncheckedExtrinsicRef {
			...
			call: self.call.try_get_or_decode()?,
		})
	}
}
...
#[derive(PartialEq, Eq, Clone, Debug)]
pub struct FullUncheckedExtrinsicRef<'a, Address, Call, Signature, Extension> {
	/// Information regarding the type of extrinsic this is (inherent or transaction) as well as
	/// associated extension (`Extension`) data if it's a transaction and a possible signature.
...
impl<'a, Address, Call: Dispatchable, Signature, Extension: TransactionExtension<Call>>
	FullUncheckedExtrinsicRef<'a, Address, Call, Signature, Extension>
{
...
impl<'a, Address, Call, Signature, Extension> ExtrinsicLike
	for FullUncheckedExtrinsicRef<'a, Address, Call, Signature, Extension>
{
	fn is_signed(&self) -> Option<bool> {
		Some(matches!(self.preamble, Preamble::Signed(..)))
	}
...
mod tests {
	...
	fn unsigned_codec_should_work() {
		...
		assert_eq!(
			Ex::decode(&mut &encoded[..]).unwrap().try_as_full(),
			Ok(FullUncheckedExtrinsicRef { preamble: &ux.preamble, call: &call })
		);
	}
	...
	fn encoding_matches_vec() {
		...
		assert_eq!(
			decoded.try_as_full(),
			Ok(FullUncheckedExtrinsicRef { preamble: &ex.preamble, call: &call })
		);
	...
	fn legacy_short_signed_encode_decode() {
		...
		assert_eq!(decoded_old_ux.expect_as_full().call, &call);
		assert_eq!(
	...
	fn legacy_unsigned_encode_decode() {
		...
		assert_eq!(
			decoded_new_ux.try_as_full(),
			Ok(FullUncheckedExtrinsicRef { preamble: &new_ux.preamble, call: &call })
		);
	...
	fn max_call_heap_size_should_be_checked() {
		...
		let call: TestCall = vec![0u8; MAX_CALL_HEAP_SIZE - 1].into();
		let ux = Ex::new_bare(call.clone());
		let encoded = ux.encode();
		assert_eq!(
			Ex::decode(&mut &encoded[..]).unwrap().try_as_full(),
