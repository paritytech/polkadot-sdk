...
// NOTE: The addition of new digest items for use by the Polkadot & Kusama relay chains should
// ideally undergo an RFC process.
// Various projects depend strictly on a specific header structure (including digest items)
// and will need to update their systems in advance to interpret & validate any new digest items
// that appear. These projects include light client bridges running on foreign chains.
...
pub enum DigestItem {
	...
	/// Custom application-specific data generated by runtimes.
	Other(Vec<u8>),
