...
mod v2 {
	...
	impl<Call: Dispatchable> TransactionExtension<Call> for () {
		...
		fn validate(
			...
			_len: usize,
			_implicit: &[u8],
		) -> Result<(ValidTransaction, (), <Call as Dispatchable>::RuntimeOrigin), TransactionValidityError> {
			Ok((ValidTransaction::default(), (), origin))
		}
		fn prepare(
			self,
	...
	impl<SE: SignedExtension> TransactionExtension<SE::Call> for AsTransactionExtension<SE> where
		<SE::Call as Dispatchable>::RuntimeOrigin: CloneSystemOriginSigner<SE::AccountId> + Clone,
	{
		const IDENTIFIER: &'static str = SE::IDENTIFIER;
		type Val = ();
		...
		fn validate(
			...
			len: usize,
			_implicit: &[u8],
		) -> Result<(ValidTransaction, (), <SE::Call as Dispatchable>::RuntimeOrigin), TransactionValidityError> {
			let who = origin.clone_system_origin_signer().ok_or(InvalidTransaction::BadSigner)?;
			Ok((self.0.validate(&who, call, info, len)?, (), origin))
		}
		fn prepare(
