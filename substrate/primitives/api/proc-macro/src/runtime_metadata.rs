...
pub fn generate_decl_runtime_metadata(decl: &ItemTrait, method_versions: &BTreeMap<&syn::Ident, u64>) -> TokenStream2 {
	...
	for item in &decl.items {
		...
		let method_version = method_versions.get(&signature.ident).copied().unwrap_or(1);
...
pub fn generate_impl_runtime_metadata(impls: &[ItemImpl]) -> Result<TokenStream2> {
	...
	for impl_ in impls {
		...
		// We want to filter runtime API metadata to only include the methods that we are actually
		// implementing. So, get the `api_version` that we are implementing (1 by default),
		// handling the possibility that it's guarded by a #[cfg_attr] attribute, and generate a
		// relevant filter method body based on this.
		let api_version_comparison = {
			let api_version = extract_api_version(&impl_.attrs, impl_.span())?;
			let base_version = api_version.custom.unwrap_or(1);
		...
		metadata.push(quote!(
			#( #attrs )*
			#trait_::runtime_metadata::#generics().keeping_methods(|method| {
				#api_version_comparison
			})
		));
	}
