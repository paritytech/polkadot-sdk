...
use crate::utils::{
	extract_api_version, extract_impl_trait, extract_parameter_names_types_and_borrows,
	generate_crate_access, generate_runtime_mod_name_for_trait, parse_runtime_api_version,
	prefix_function_with_trait, versioned_trait_name, AllowSelfRefInParameters, ApiVersion,
...
use syn::{
	...
	Attribute, Ident, ImplItem, ItemImpl, Path, Signature, Type,
};
...
fn generate_api_impl_for_runtime_api(impls: &[ItemImpl]) -> Result<TokenStream> {
	...
	for impl_ in impls {
		...
		let mut processed_impl = ApiRuntimeImplToApiRuntimeApiImpl.process(impl_.clone());
