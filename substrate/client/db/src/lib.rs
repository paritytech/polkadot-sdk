...
impl<Block: BlockT> Backend<Block> {
	...
	fn note_finalized(
		...
		let pruned_block_headers = self.prune_blocks(
			transaction,
			f_num,
			&new_displaced,
			current_transaction_justifications,
	...
	fn prune_blocks(
		...
		match self.blocks_pruning {
			...
			BlocksPruning::Some { blocks: blocks_pruning, prune_headers } => {
				...
				let pruned_block_headers_part =
					self.prune_displaced_branches(transaction, displaced, prune_headers)?;
				pruned_block_headers.extend_from_slice(&pruned_block_headers_part);
	...
	// Note: returns a list of hashes of the pruned block headers (can be empty). It doesn't prune
	// block headers in place because there is cache refreshment upper in the stack and before the
	...
	fn prune_displaced_branches(
		...
		for (_, tree_route) in displaced.tree_routes.iter() {
			for r in tree_route.retracted() {
				...
				if prune_headers {
					pruned_block_headers.push(r.hash);
				}
			}
		}
		Ok(pruned_block_headers)
