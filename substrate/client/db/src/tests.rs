...
fn test_state_importer() {
	...
	let build_block =
		|backend: &Backend<Block>, number, parent_hash, changes: Vec<(Vec<u8>, Vec<u8>)>| {
			let mut op = backend.begin_operation().unwrap();
	...
	let build_block_1_with_state_importer = |incremental| {
		let backend = Backend::<Block>::new_test(10, 10);

		let (hash0, state_root0) =
			build_block(&backend, 0, Default::default(), storage_changes_block_0.clone());

		let (hash1, state_root1) = build_block(&backend, 1, hash0, storage_changes_block_1.clone());
		...
		let state_root1_state_importer = header.state_root;
		...
		assert_eq!(state_storage_root, state_root1_state_importer);
	};

	for incremental in [true, false] {
		build_block_1_with_state_importer(incremental);
	}
}
