...
#[ignore]
async fn tx_proto_test() -> Result<(), anyhow::Error> {
	let node_count = std::env::var("NODE_COUNT")
		.expect("Environment variable NODE_COUNT not set")
		.parse()
		.unwrap();

	let txs_count = std::env::var("PER_ACCOUNT_TXS_COUNT")
		.expect("Environment variable PER_ACCOUNT_TXS_COUNT not set")
		.parse()
		.unwrap();

	let accounts_count: u32 = std::env::var("ACCOUNTS_COUNT")
		.expect("Environment variable ACCOUNTS_COUNT not set")
		.parse()
		.unwrap();

	let send_threshold = std::env::var("SEND_THRESHOLD")
		.expect("Environment variable SEND_THRESHOLD not set")
		.parse()
		.unwrap();

	let spawner = NetworkSpawner::with_closure(|| {
		let images = zombienet_sdk::environment::get_images_from_env();
		NetworkConfigBuilder::new().with_relaychain(|r| {
			let r = r
				...
				.with_genesis_overrides(json!({
					"balances": {
						"devAccounts": [
							accounts_count, 1000000000000000000u64, "//Sender//{}"
						]
					}
				}))
				...
				.with_default_args(vec![
					...
					("-ltxpool=debug,txpoolstat=debug").into(),
				])
				.with_node(|node| node.with_name("alice").with_rpc_port(9944));

			(1..node_count)
				.fold(r, |acc, i| acc.with_node(|node| node.with_name(&format!("node{i:02}"))))
		})
	})
	.await
	.unwrap();

	spawner.wait_for_block_production("alice").await.unwrap();

	// Create txs executor.
	let ws = spawner.node_rpc_uri("alice").unwrap();
	let executor = {
	...
	.with_rpc_uri(ws)
	.with_start_id(0)
	.with_last_id(accounts_count - 1)
	.with_txs_count(txs_count)
	...
	assert_eq!(finalized_txs, (accounts_count * txs_count) as usize);
...
#[tokio::test(flavor = "multi_thread")]
#[ignore]
async fn tx_proto_test_yap() -> Result<(), anyhow::Error> {
	let node_count = std::env::var("NODE_COUNT")
		.expect("Environment variable NODE_COUNT not set")
		.parse()
		.unwrap();
	...
	let spawner = NetworkSpawner::with_closure(|| {
		...
		NetworkConfigBuilder::new()
			...
			.with_parachain(|p| {
				...
				(1..node_count).fold(p, |acc, i| {
					acc.with_collator(|node| node.with_name(&format!("node{i:02}")))
				})
			})
	})
	.await
