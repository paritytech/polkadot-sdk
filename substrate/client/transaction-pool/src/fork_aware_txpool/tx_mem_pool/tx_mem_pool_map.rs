...
impl<K, V> TrackedMap<K, V> {
	...
	/// Lock map for read.
	pub async fn read(&self) -> TrackedMapReadAccess<K, V> {
		TrackedMapReadAccess { inner_guard: self.index.read().await }
	}

	/// Lock map for write.
	pub async fn write(&self) -> TrackedMapWriteAccess<K, V> {
...
mod tests {
	...
	async fn basic() {
		...
		map.write().await.insert(5, 10);
		map.write().await.insert(6, 20);

		assert_eq!(map.bytes(), 3);
		assert_eq!(map.len(), 2);

		map.write().await.insert(6, 30);

		assert_eq!(map.bytes(), 4);
		assert_eq!(map.len(), 2);

		map.write().await.remove(&6);
