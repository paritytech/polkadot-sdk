		...
		if next_action.resubmit {
			...
			for retracted in tree_route.retracted() {
				...
				let block_transactions = api
					...
					.unwrap_or_default()
					.into_iter()
					// XXX: This isn't really what we mean - we really want a `tx.is_transaction`,
					// since bare transactions may be gossipped as in the case of Frontier txs or
					// claims. This will be sorted once we dispense with the concept of bare
					// transactions and make inherents the only possible type of extrinsics which
					// are bare. At this point we can change this to `tx.is_transaction()`.
