...
impl ShieldKeystore for MemoryShieldKeystore {
	fn roll_for_next_slot(&self) -> TraitResult<()> {
		self.0.write().map_err(|_| TraitError::Unavailable)?.roll_for_next_slot()
	}

	fn next_enc_key(&self) -> TraitResult<Vec<u8>> {
		self.0.read().map_err(|_| TraitError::Unavailable)?.next_enc_key()
	}

	fn current_dec_key(&self) -> TraitResult<Vec<u8>> {
		self.0.read().map_err(|_| TraitError::Unavailable)?.current_dec_key()
	}
}
...
impl ShieldKeystoreInner {
	fn new() -> Self {
		Self { current_pair: ShieldKeyPair::generate(), next_pair: ShieldKeyPair::generate() }
	}
...
mod tests {
	...
	fn current_dec_key_matches_previous_enc_key() {
		...
		let enc_key =
			EncapsulationKey::<MlKem768Params>::from_bytes(ek_bytes.as_slice().try_into().unwrap());
		...
		let dec_key =
			DecapsulationKey::<MlKem768Params>::from_bytes(dk_bytes.as_slice().try_into().unwrap());
