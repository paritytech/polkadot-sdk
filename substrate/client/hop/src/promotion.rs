...
//! - [`RuntimeApiPromoter`] — concrete implementation using [`sp_hop::HopPromotionApi`] and
...
/// ([`RuntimeApiPromoter`]) uses the `HopPromotionApi` runtime API.
...
/// Concrete [`HopPromoter`] that uses the [`sp_hop::HopPromotionApi`] runtime
...
where
	...
	C::Api: sp_hop::HopPromotionApi<Block>,
...
/// Try to build a [`HopPromoter`] by detecting the `HopPromotionApi` runtime
	...
	P: sc_transaction_pool_api::LocalTransactionPool<Block = Block> + 'static,
{
	...
	match client
		.runtime_api()
		.has_api_with::<dyn sp_hop::HopPromotionApi<Block>, _>(best_hash, |v| v >= 1)
	{
		Ok(true) => {
			tracing::info!(target: "hop", "HopPromotionApi detected — promotion enabled");
			Some(Arc::new(RuntimeApiPromoter::new(client.clone(), tx_pool.clone())))
		},
		Ok(false) => {
			tracing::warn!(
				target: "hop",
				"HOP enabled but runtime does not support HopPromotionApi — running cleanup only"
			);
			None
		},
		Err(e) => {
			tracing::warn!(
				target: "hop",
				error = %e,
				"Failed to check HopPromotionApi support — running cleanup only"
			);
			None
		},
	}
