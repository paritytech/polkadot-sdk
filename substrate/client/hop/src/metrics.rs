...
impl HopMetrics {
	...
	/// Set both pool gauges (used after disk recovery).
	pub(crate) fn set_pool_status(&self, bytes: u64, max_bytes: u64) {
	...
	/// Snapshot the pool bytes gauge. Set from the authoritative counter rather
	/// than inc/dec'd: a `Gauge<U64>` wraps on underflow, so any missed delta
	/// would poison the gauge forever, while a snapshot self-corrects on the
	/// next update. Callers publish under the pool's index lock, so mutations
	/// arrive here in lock order.
	pub(crate) fn set_pool_bytes(&self, bytes: u64) {
		if let Some(inner) = &self.inner {
			inner.pool_bytes.set(bytes);
		}
	}
	...
	/// Current value of the pool bytes gauge.
	#[cfg(test)]
	pub(crate) fn pool_bytes(&self) -> u64 {
		self.inner.as_ref().map(|i| i.pool_bytes.get()).unwrap_or(0)
	}
...
mod tests {
	...
	fn no_registry_or_disabled_means_no_ops() {
		...
		metrics.set_pool_status(2, 3);
		metrics.set_pool_bytes(1);
	...
	fn enabled_metrics_track_values() {
		...
		metrics.set_pool_status(200, 1000);
		metrics.set_pool_bytes(300);
		...
		assert_eq!(metrics.pool_bytes(), 300);
