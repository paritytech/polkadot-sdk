...
///
/// The scenario (from the original bug report):
/// 1. Node has a block gap of 1 (e.g., block #5800 with MissingBody).
/// 2. Gap sync requests the block; peer provides a full response including body.
/// 3. Import fails with a consensus error (parent state already discarded).
/// 4. Chain sync restarts, re-detects the gap, and repeats — infinite loop.
///
/// The fix ensures that gap-sync blocks are imported with `skip_execution: true`
/// (StateAction::Skip), so the collator's block import layer does not attempt to
/// execute blocks whose parent state has been pruned.
///
/// This test exercises the chain sync layer: it verifies that when a gap-sync
/// block import fails with `BlockImportError::UnknownParent` (same restart path
/// as `BlockImportError::Other` for consensus errors), the gap sync is
/// re-initialized (the restart path), and conversely that a successful import
/// of the gap target clears the gap entirely.
#[test]
fn gap_sync_single_block_error_restarts_and_success_clears_gap() {
	sp_tracing::try_init_simple();
	...
	let peer_id1 = PeerId::random();
	// Use a dummy block hash and number to simulate the gap block.
	let gap_block_number: u64 = 1;
	let gap_block_hash = Hash::random();

	// Simulate a block gap of exactly 1 block.
	sync.gap_sync = Some(GapSync {
		best_queued_number: gap_block_number,
		target: gap_block_number,
		blocks: BlockCollection::new(),
		stats: GapSyncStats::new(),
	});
	assert!(sync.gap_sync.is_some());

	// Phase 1: Simulate the pre-fix behavior where import fails with a
	// consensus error ("State already discarded for parent"). Both
	// `UnknownParent` and `Other` trigger the same restart() path.
	// In production, restart() → reset_sync_start_point() would re-read
	// client.info().block_gap and re-create the gap_sync, causing an
	// infinite loop. The fix breaks this loop at the block import layer
	// by skipping execution for gap-sync blocks (StateAction::Skip).
	let error_results = vec![(Err(BlockImportError::UnknownParent), gap_block_hash)];
	sync.on_blocks_processed(0, 1, error_results);

	// After error + restart, gap_sync persists because restart() →
	// reset_sync_start_point() only replaces gap_sync when
	// client.info().block_gap is Some. The test client has no real gap,
	// so the existing gap_sync remains unchanged. In production, the
	// client WOULD report the gap (since the block was never imported),
	// and reset_sync_start_point() would re-create gap_sync — the
	// infinite loop.
	assert!(
		sync.gap_sync.is_some(),
		"Gap sync should persist after failed import + restart (gap is not resolved)"
	);

	// Phase 2: Re-create the gap and simulate the post-fix behavior where
	// the import succeeds (because StateAction::Skip prevents execution).
	sync.gap_sync = Some(GapSync {
		best_queued_number: gap_block_number,
		target: gap_block_number,
		blocks: BlockCollection::new(),
	...
	sync.on_blocks_processed(1, 1, success_results);

	// With a successful import of the gap target, the gap is cleared.
	assert!(
		sync.gap_sync.is_none(),
		"Gap sync should be cleared after successfully importing the gap target block"
	);
}
