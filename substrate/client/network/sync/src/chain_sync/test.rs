...
fn processes_empty_response_on_justification_request_for_unknown_block() {
	...
	let mut sync = ChainSync::new(ChainSyncMode::Full, client.clone(), 1, 64).unwrap();
...
fn do_ancestor_search_when_common_block_to_best_qeued_gap_is_to_big() {
	...
	let mut sync = ChainSync::new(ChainSyncMode::Full, client.clone(), 5, 64).unwrap();
...
fn can_import_response_with_missing_blocks() {
	...
	let mut sync = ChainSync::new(ChainSyncMode::Full, empty_client.clone(), 1, 64).unwrap();
