...
pub struct Litep2pNetworkBackend {
	...
	/// External addresses.
	_external_addresses: Arc<RwLock<HashSet<Multiaddr>>>,
}
...
impl<B: BlockT + 'static, H: ExHashT> NetworkBackend<B, H> for Litep2pNetworkBackend {
		...
		Self: Sized,
	{
		...
		// handshake for all but the syncing protocol is set as the node role
		...
		let listen_addresses: HashSet<_> = litep2p.listen_addresses().cloned().collect();
		let external_addresses = Arc::new(RwLock::new(listen_addresses.clone()));
		...
		Ok(Self {
			...
			litep2p,
			_external_addresses: external_addresses,
		})
	}
		...
		loop {
			...
			tokio::select! {
				...
				event = self.discovery.next() => match event {
					...
					Some(DiscoveryEvent::Identified { peer, listen_addresses, .. }) => {
						self.discovery.add_known_peer(peer, listen_addresses).await;
