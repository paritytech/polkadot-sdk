...
impl<B: BlockT + 'static, H: ExHashT> NetworkBackend<B, H> for Litep2pNetworkBackend {
		...
		Self: Sized,
	{
		...
		// The WebRTC transport terminates DTLS with a single certificate and is litep2p's only
		// `/certhash` emitter, so every `webrtc-direct` listen address carries the same hash.
		let webrtc_certhash = litep2p.listen_addresses().find_map(|address| {
...
mod tests {
	...
	fn certhash_appended_to_bare_webrtc_address() {
		assert_eq!(
			add_webrtc_certhash(webrtc_address(), Some(a_certhash())),
	...
	fn certhash_inserted_before_p2p() {
		...
		assert_eq!(
			add_webrtc_certhash(
				webrtc_address().with(Protocol::P2p(peer.into())),
				Some(a_certhash()),
			),
			webrtc_address()
	...
	fn operator_supplied_certhash_kept() {
		...
		assert_eq!(add_webrtc_certhash(address.clone(), Some(a_certhash())), address);
	}
