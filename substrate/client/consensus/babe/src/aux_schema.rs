	...
	F: FnOnce(&[(&'static [u8], &[u8])]) -> R,
{
	...
	warn!(target: LOG_TARGET, "[DOPPELGANGER] auths {:?}", auths);

	let epoch_changes_augmented = epoch_changes.clone().map(|hash, n, mut epoch| {
		warn!(target: LOG_TARGET, "[DOPPELGANGER] hash {:?}", hash);
		warn!(target: LOG_TARGET, "[DOPPELGANGER] n {:?}", n);
		warn!(target: LOG_TARGET, "[DOPPELGANGER] epoch {:?}", epoch);
		if epoch.0.authorities.is_empty() {
