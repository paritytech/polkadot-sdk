...
async fn voter_init_setup(
	...
	let (gossip_validator, _) =
		GossipValidator::<Block, ecdsa_crypto::AuthorityId>::new(known_peers);
...
async fn should_initialize_voter_at_custom_genesis() {
	...
	// first `GossipEngine`
	let known_peers = Arc::new(Mutex::new(KnownPeers::new()));
	let (gossip_validator, _) = GossipValidator::<_, ecdsa_crypto::AuthorityId>::new(known_peers);
	let gossip_validator = Arc::new(gossip_validator);
	let mut gossip_engine = sc_network_gossip::GossipEngine::new(
		net.peer(0).network_service().clone(),
		net.peer(0).sync_service().clone(),
