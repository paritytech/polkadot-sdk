...
use sp_consensus_beefy::{
	ecdsa_bls_crypto,
	ecdsa_bls_crypto::{AuthorityId, Signature},
	known_payloads,
...
pub(crate) fn create_beefy_keystore(authority: &BeefyKeyring<AuthorityId>) -> KeystorePtr {
	...
	keystore
		.ecdsa_bls381_generate_new(BEEFY_KEY_TYPE, Some(&authority.to_seed()))
...
async fn voter_init_setup(
	...
	api: &TestApi,
) -> Result<PersistedState<Block, ecdsa_bls_crypto::AuthorityId>, Error> {
...
async fn gossipped_finality_proofs() {
	...
	charlie_gossip_validator.update_filter(GossipFilterCfg::<Block, ecdsa_bls_crypto::AuthorityId> {
		start: 1,
		end: 10,
	...
	let gossip_proof = GossipMessage::<Block, ecdsa_bls_crypto::AuthorityId>::FinalityProof(proof);
	...
	let encoded_vote =
		GossipMessage::<Block, ecdsa_bls_crypto::AuthorityId>::Vote(vote_message).encode();
	...
	let mut charlie_gossip_proofs = Box::pin(
		charlie_gossip_engine
			...
			.filter_map(|notification| async move {
				GossipMessage::<Block, ecdsa_bls_crypto::AuthorityId>::decode(
					&mut &notification.message[..],
				)
				.ok()
				.and_then(|message| match message {
					GossipMessage::<Block, ecdsa_bls_crypto::AuthorityId>::Vote(_) => unreachable!(),
					GossipMessage::<Block, ecdsa_bls_crypto::AuthorityId>::FinalityProof(proof) =>
						Some(proof),
				})
			})
			.fuse(),
	...
	loop {
		...
		futures::select! {
			...
			proof = charlie_gossip_proofs.next() => {
				...
				let (round, _) = proof_block_num_and_set_id::<Block, ecdsa_bls_crypto::AuthorityId>(&proof);
				match round {
