...
mod tests {
	...
	#[test]
	fn concurrent_distinct_statement_submissions_all_succeed() {
		let (store, _temp) = test_store();
		let store = std::sync::Arc::new(store);
		let num_threads = 20usize;
		let data_len = 50usize;

		// Each thread gets a unique account ID (5..25) which maps to the default allowance
		let handles: Vec<_> = (0..num_threads)
			.map(|i| {
				let store = std::sync::Arc::clone(&store);
				let s = statement(5 + i as u64, 1, None, data_len);
				std::thread::spawn(move || store.submit(s, StatementSource::Network))
			})
			.collect();
		...
		for (i, result) in results.iter().enumerate() {
			assert_eq!(*result, SubmitResult::New, "Thread {i} should succeed");
		}

		assert_eq!(store.statements().unwrap().len(), num_threads);

		let index = store.index.read();
		assert_eq!(index.entries.len(), num_threads);
		assert_eq!(index.total_size, num_threads * data_len);
	}

	#[test]
	fn concurrent_submit_and_take_recent_no_statement_lost() {
		let (store, _temp) = test_store();
		let store = std::sync::Arc::new(store);
		let num_statements = 50usize;

		let store_submitter = std::sync::Arc::clone(&store);
		let submitter = std::thread::spawn(move || {
			for i in 0..num_statements {
				let s = statement(5 + i as u64, 1, None, 50);
				let result = store_submitter.submit(s, StatementSource::Network);
				assert_eq!(result, SubmitResult::New, "Submit {i} should succeed");
				std::thread::sleep(std::time::Duration::from_micros(100));
			}
		});

		let store_taker = std::sync::Arc::clone(&store);
		let taker = std::thread::spawn(move || {
			let mut collected = Vec::new();
			for _ in 0..200 {
				if let Ok(recent) = store_taker.take_recent_statements() {
		...
		// Final drain of any remaining recent statements
		taken.extend(store.take_recent_statements().unwrap());

		// Verify no statements lost or duplicated
		let hashes: std::collections::HashSet<_> = taken.iter().map(|(h, _)| *h).collect();
		assert_eq!(
			hashes.len(),
			num_statements,
	...
	#[test]
	fn concurrent_take_recent_only_one_caller_gets_each_statement() {
		use sp_statement_store::Hash;

		let (store, _temp) = test_store();
		let store = std::sync::Arc::new(store);
		let num_statements = 100usize;

		// Submit statements with distinct account IDs to avoid per-account limits
		for i in 0..num_statements {
			let s = statement(5 + i as u64, 1, None, 10);
			assert_eq!(store.submit(s, StatementSource::Network), SubmitResult::New);
		}

		assert_eq!(store.index.read().recent.len(), num_statements);

		let num_takers = 10usize;
		let barrier = std::sync::Arc::new(std::sync::Barrier::new(num_takers));
		...
		let results: Vec<Vec<(Hash, Statement)>> =
			handles.into_iter().map(|h| h.join().unwrap()).collect();

		// Collect all hashes across all takers
		...
		assert_eq!(
			all_hashes.len(),
			num_statements,
			"Total statements across all takers should be {num_statements}"
		);
		...
		assert_eq!(
			unique.len(),
			all_hashes.len(),
			"Each statement should appear in exactly one taker's result"
		);

		// Subsequent take should be empty
		assert!(store.take_recent_statements().unwrap().is_empty());

		// Statements still exist in the store
		assert_eq!(store.statements().unwrap().len(), num_statements);
	}
}
