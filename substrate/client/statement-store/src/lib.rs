...
impl Store {
	...
	/// Create a new sharded store instance without automatic maintenance.
	/// Creates NUM_SHARDS Store instances, each with its own database at `{path}/shard_{i}`.
	/// Use this for testing or when you want to manually control maintenance.
	#[doc(hidden)]
	pub fn new<Block, Client>(
		path: &std::path::Path,
		options: Options,
	...
	/// Perform periodic store maintenance on all shards
	pub fn maintain(&self) {
