...
//! Shared types, helpers, subxt config, and extension definitions used by the
//! `statement-latency-bench` binary. Accounts are the same deterministic
//! `//StatementClient//{idx}` set that `setup-allowances` provisions.
