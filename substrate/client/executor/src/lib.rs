...
mod tests {
	...
	#[test]
	fn all_host_functions_carry_allocation_metadata() {
		// Every host function the standard client provides must carry the allocation metadata
		// the runtime allocation sanity checker classifies imports by. The metadata is
		// generated by the `#[runtime_interface]` macro, so this can only fail when a
		// hand-written `sp_wasm_interface::Function` implementation that doesn't provide the
		// metadata sneaks into `SubstrateHostFunctions`.
		let missing = sp_io::SubstrateHostFunctions::host_functions()
			.into_iter()
			.filter(|function| {
				function.host_allocates().is_none() || function.abi_epoch().is_none()
			})
			.map(|function| function.name().to_string())
			.collect::<Vec<_>>();
		assert!(missing.is_empty(), "host functions without allocation metadata: {missing:?}");
	}
}
