...
async fn warp_sync_from_genesis_network() -> Result<(), anyhow::Error> {
	log::info!("Spawning network with warp sync");
...
async fn build_network_config() -> Result<NetworkConfig, anyhow::Error> {
	...
	// Network setup:
	// - Nodes:
	//   - alice (validator with warp sync)
	//   - bob (validator with warp sync)
	//   - charlie (full node with warp sync)
	let config = NetworkConfigBuilder::new()
		.with_relaychain(|r| {
			r.with_chain("dev")
				...
				.with_node(|node| {
					node.with_name("alice").validator(true).with_args(vec![("--sync=warp").into()])
				})
				.with_node(|node| {
					node.with_name("bob").validator(true).with_args(vec![(
						"--sync=warp",
						"-lsync=trace",
					)
						.into()])
