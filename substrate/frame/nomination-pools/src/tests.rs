...
mod blacklisted_accounts {
	...
	#[test]
	fn blacklisted_accounts_cannot_join() {
		ExtBuilder::default().build_and_execute(|| {
			// GIVEN
			let alice = 301;
			Currency::set_balance(&alice, 20_000);

			// WHEN alice is blacklisted from participating in pools
			blacklist(&alice);

			// THEN alice cannot join any pool
			assert_noop!(
				Pools::join(RuntimeOrigin::signed(alice), 10, 1),
				Error::<Runtime>::Blacklisted
			);
			...
			// WHEN alice is removed from blacklist
			remove_from_blacklist(&alice);

			// THEN alice can join a pool
			assert_ok!(Pools::join(RuntimeOrigin::signed(alice), 10, 1));

			// WHEN alice is blacklisted while being in a pool
			blacklist(&alice);
