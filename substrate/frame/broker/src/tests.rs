...
/// market and it makes sense to adjust the price to what was paid here. Assuming all renewals
/// were done in the interlude and only normal sales happen in the leadin, renewals will have no
/// effect on price. If there are no cores left for sale on the open markent, renewals will affect
/// price even in the interlude, making sure renewal prices stay in the range of the open market.
...
// We understand that this does not work as intended for leases that expire within
// `region_length` timeslices after calling `start_sales`.
#[test]
...
fn start_sales_sets_correct_core_count() {
	TestExt::new().endow(1, 1000).execute_with(|| {
		...
		Broker::do_start_sales((), 5).unwrap();
...
fn force_reserve_works() {
	...
	TestExt::new().endow(1, 1000).execute_with(|| {
		assert_ok!(Broker::do_start_sales((), 4));
		advance_to(2);
