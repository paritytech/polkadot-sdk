...
mod v6 {
	use super::*;
	use v4::Configuration as ConfigurationV4;
	use v5::{PotentialRenewalRecordOf as PotentialRenewalRecordV5Of, SaleInfo as SaleInfoV5};
	...
	impl<T: Config> UncheckedOnRuntimeUpgrade for MigrateToV6Impl<T> {
		fn on_runtime_upgrade() -> frame_support::weights::Weight {
			...
			<PotentialRenewals<T>>::translate_values::<PotentialRenewalRecordV5Of<T>, _>(|v5| {
				potential_renewals_count.saturating_inc();
				Some(PotentialRenewalRecord { completion: v5.completion })
			});
			log::info!(
				target: LOG_TARGET,
