...
fn query_info_and_fee_details_works() {
	...
	let info = xt.clone().expect_as_ref().get_dispatch_info();
	let ext = xt.encode();
	let len = ext.len() as u32;

	let mut unsigned_xt = UncheckedExtrinsic::<u64, _, (), ()>::new_bare(call);
	let unsigned_xt_info = unsigned_xt.clone().expect_as_ref().get_dispatch_info();
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				TransactionPayment::query_info(xt.clone().expect_as_ref(), len),
				RuntimeDispatchInfo {
			...
			assert_eq!(
				TransactionPayment::query_info(unsigned_xt.clone().expect_as_ref(), len),
				RuntimeDispatchInfo {
			...
			assert_eq!(
				TransactionPayment::query_fee_details(xt.expect_as_ref(), len),
				FeeDetails {
			...
			assert_eq!(
				TransactionPayment::query_fee_details(unsigned_xt.expect_as_ref(), len),
