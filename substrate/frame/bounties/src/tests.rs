...
fn poke_deposit_fails_for_non_existent_bounty() {
	ExtBuilder::default().build_and_execute(|| {
		assert_noop!(Bounties::poke_deposit(RuntimeOrigin::signed(0), 0), Error::<Test>::InvalidIndex);
	});
}
...
fn poke_deposit_fails_for_non_proposer() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_noop!(Bounties::poke_deposit(RuntimeOrigin::signed(1), 0), Error::<Test>::NotProposer);
	});
}
...
fn poke_deposit_fails_for_any_status_other_than_proposed() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_ok!(Bounties::propose_bounty(RuntimeOrigin::signed(proposer), 50, b"12345".to_vec()));
		...
		assert_noop!(Bounties::poke_deposit(RuntimeOrigin::signed(proposer), bounty_id), Error::<Test>::UnexpectedStatus);
...
fn poke_deposit_works_for_deposit_increase() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(last_event(), BountiesEvent::DepositPoked { bounty_id, proposer, old_deposit: deposit, new_deposit });
