...
fn balance_transfer_dispatch_works() {
	...
	pallet_balances::GenesisConfig::<Runtime> { balances: vec![(1, 211)], ..Default::default() }
		...
		.unwrap();
	let mut xt = UncheckedXt::new_signed(call_transfer(2, 69), 1, 1.into(), tx_ext(0, 0));
	let weight = xt.expect_as_ref().get_dispatch_info().total_weight() +
...
fn block_weight_and_size_is_stored_per_tx() {
	let mut xt = UncheckedXt::new_signed(
		RuntimeCall::Balances(BalancesCall::transfer_allow_death { dest: 33, value: 0 }),
		1,
		1.into(),
		tx_ext(0, 0),
	...
	let extension_weight = xt.expect_as_ref().extension_weight();
