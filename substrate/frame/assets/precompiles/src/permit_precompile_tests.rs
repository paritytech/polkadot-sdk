...
fn permit_setup(prefix: u16) -> PermitSetup {
	...
	// The setup mint itself emits a mirrored ERC-20 Transfer log (`Erc20TransferLogs` is the
	// mock's `CallbackHandle`); drop it so `assert_no_contract_event_from` only scopes the
