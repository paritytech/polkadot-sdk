...
pub mod history_depth_reducer {
	...
	/// this release.
	pub struct Migrate<T, OldHistoryDepth, MaxErasToClear>(core::marker::PhantomData<(T, OldHistoryDepth, MaxErasToClear)>);

	fn era_in_history<T: Config>(era: EraIndex) -> bool {
		// Since we remove all era information atomically, we can just check if the following exists
		// to determine if the era is in history.
	...
	impl<T: Config, OldHistoryDepth: Get<EraIndex>, MaxErasToClear: Get<u32>> UncheckedOnRuntimeUpgrade for Migrate<T, OldHistoryDepth, MaxErasToClear> {
		...
		fn pre_upgrade() -> Result<Vec<u8>, TryRuntimeError> {
				...
				OldHistoryDepth::get() > T::HistoryDepth::get(),
				"HistoryDepth should be reduced"
			);
			...
			for era in OldHistoryDepth::get()..T::HistoryDepth::get() {
				...
				if eras_to_remove.len() > MaxErasToClear::get() as usize {
					break;
				}
			}
		...
		fn post_upgrade(state: Vec<u8>) -> Result<(), TryRuntimeError> {
			let eras_to_remove = Vec::<EraIndex>::decode(&mut state.as_slice())
				.expect("the state parameter should be something that was generated by pre_upgrade");
			for era in eras_to_remove {
				ensure!(!era_in_history::<T>(era),"era not cleared");
			}
			Ok(())
		}
	}
