...
mod nomination_staleness {
	...
	// Convenience to reset the staleness parameter_types between assertions.
	...
	fn linear_curve_decays_linearly_to_nonzero_floor() {
		...
		// In grace: full weight.
		assert_eq!(Curve::multiplier(10), Perbill::one());

		// Halfway through decay: 25% + 50% * (1 - 25%) = 25% + 37.5% = 62.5%.
		let m_halfway = Curve::multiplier(10 + 50);
		// active_share = (100 - 50) / 100 = 50%
		// one_minus_floor = 75%
	...
	fn linear_curve_zero_decay_period_clamps_immediately() {
		// Past the grace period with `DecayPeriod = 0`: multiplier == floor immediately.
	...
	fn nominate_resets_submitted_in_even_with_same_targets() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_eq!(after.submitted_in, 5);
		});
	}
	...
	fn bond_extra_does_not_reset_submitted_in() {
		ExtBuilder::default().build_and_execute(|| {
			...
			// `bond_extra` should not refresh the nomination.
	...
	fn chill_removes_nomination_and_renominate_creates_fresh_entry() {
		ExtBuilder::default().build_and_execute(|| {
			...
			// Chill: nomination entry is removed entirely.
			assert_ok!(Staking::chill(RuntimeOrigin::signed(101)));
			assert!(Nominators::<Test>::get(101).is_none());

			// Move to a later era and re-nominate.
			start_active_era(9);
			assert_ok!(Staking::nominate(RuntimeOrigin::signed(101), vec![11, 21]));

			// `submitted_in` is the era of the new `nominate` call, not the original one.
			let after = Nominators::<Test>::get(101).unwrap();
			assert_eq!(after.submitted_in, 9);
		});
	}
	...
	fn snapshot_voter_weight_is_unchanged_with_default_no_op_curve() {
		ExtBuilder::default().build_and_execute(|| {
			// With the default mock parameters (`grace = u32::MAX`), the curve is a no-op.
			start_active_era(50);
			...
			// Find nominator 101 in the snapshot.
			let entry = voters.iter().find(|(who, _, _)| who == &101).expect("101 in snapshot");
			// Weight equals active stake (500 in the genesis mock setup).
	...
	fn snapshot_voter_weight_is_reduced_when_stale() {
		ExtBuilder::default().build_and_execute(|| {
			// Nominator 101 was added at era 0. Configure a curve that fires fast.
			set_curve(2, 4, Perbill::zero());

			// Era 4: 4 eras since `submitted_in = 0`. Past grace (2), 2 eras into decay.
			// multiplier = (4 - 2) / 4 = 50%.
			start_active_era(4);
	...
	fn fully_stale_voter_with_zero_floor_is_excluded_from_snapshot() {
		ExtBuilder::default().build_and_execute(|| {
			...
			// Era 6: 6 eras since `submitted_in = 0` >= grace + decay. multiplier = 0.
			start_active_era(6);
			...
			.unwrap();
			// Nominator 101 is excluded; only validators (11, 21, 31 self-votes) remain.
	...
	fn re_nominating_restores_full_weight() {
		ExtBuilder::default().build_and_execute(|| {
			...
			start_active_era(4);
			// At era 4, 101 is at 50% weight (see `snapshot_voter_weight_is_reduced_when_stale`).

			// Re-affirm in era 4.
			assert_ok!(Staking::nominate(RuntimeOrigin::signed(101), vec![11, 21]));

			// Same era, but `submitted_in` is now 4. multiplier = 1.
			let voters = <Staking as ElectionDataProvider>::electing_voters(
				DataProviderBounds::default(),
				SINGLE_PAGE,
			)
	...
	fn reset_all_nomination_submitted_in_resets_every_nominator() {
		ExtBuilder::default().build_and_execute(|| {
			// Manually scribble an old `submitted_in` for nominator 101 to simulate a
			// long-stale nomination from before the staleness mechanism was enabled.
			Nominators::<Test>::mutate(101, |n| {
				let n = n.as_mut().unwrap();
				n.submitted_in = 0;
			});

			// Move to a non-trivial current era.
			start_active_era(42);
			assert_eq!(CurrentEra::<Test>::get().unwrap(), 42);
			assert_eq!(Nominators::<Test>::get(101).unwrap().submitted_in, 0);

			// Run the helper that a real migration would call.
			let _w = reset_all_nomination_submitted_in::<Test>();

			// Every nominator's `submitted_in` is now the current era.
			for (_, nomination) in Nominators::<Test>::iter() {
				assert_eq!(nomination.submitted_in, 42);
			}
		});
	}
	...
	fn slashing_unaffected() {
		...
		//   - A still-exposed stale nominator is slashed proportional to their (reduced) exposure
		//     value — they are NOT made immune to slashing.
		ExtBuilder::default().build_and_execute(|| {
			Nominators::<Test>::mutate(101, |n| {
				n.as_mut().unwrap().submitted_in = 0;
			});
