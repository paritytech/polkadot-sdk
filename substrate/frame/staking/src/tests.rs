...
mod election_data_provider {
		...
		ExtBuilder::default()
			...
			.build_and_execute(|| {
				...
				assert_ok!(<Staking as ElectionDataProvider>::electing_voters(
					DataProviderBounds::default(), SINGLE_PAGE,
				));
				...
				assert_ok!(<Staking as ElectionDataProvider>::electing_voters(bounds.voters, SINGLE_PAGE));
	...
	fn set_minimum_active_stake_lower_bond_works() {
		...
		ExtBuilder::default().has_stakers(true).nominate(true).build_and_execute(|| {
			...
			let voters_before =
				<Staking as ElectionDataProvider>::electing_voters(DataProviderBounds::default(), SINGLE_PAGE,)
					.unwrap();
	...
	fn only_iterates_max_2_times_max_allowed_len() {
		ExtBuilder::default()
			...
			.build_and_execute(|| {
				...
				assert_eq!(
					Staking::electing_voters(bounds_builder.voters_count(2.into()).build().voters, SINGLE_PAGE,)
						.unwrap()
						.iter()
						.map(|(stash, _, _)| stash)
						.copied()
	...
	fn respects_snapshot_count_limits() {
		ExtBuilder::default()
			...
			.build_and_execute(|| {
				...
				assert_eq!(
					Staking::electing_voters(bounds_builder.voters_count(1.into()).build().voters, SINGLE_PAGE,)
						.unwrap()
						.len(),
					1
				);
				...
				assert_eq!(
					Staking::electing_voters(bounds_builder.voters_count(5.into()).build().voters, SINGLE_PAGE,)
						.unwrap()
						.len(),
					5
				);
				...
				assert_eq!(
					Staking::electing_voters(bounds_builder.voters_count(55.into()).build().voters, SINGLE_PAGE,)
						.unwrap()
						.len(),
					5
				);
...
fn nomination_quota_max_changes_decoding() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			assert_eq!(Staking::electing_voters(unbonded_election, SINGLE_PAGE,).unwrap().len(), 4 + 4);
		});
}
