...
impl SlashingSpans {
	...
	pub(crate) fn end_span(&mut self, now: EraIndex) -> bool {
		...
		self.span_index.saturating_accrue(1);
		true
	}
	...
	fn prune(&mut self, window_start: EraIndex) -> Option<(SpanIndex, SpanIndex)> {
		let old_idx = self
			...
			.position(|span| {
				span.length.map_or(false, |len| span.start.saturating_add(len) <= window_start)
			});

		let earliest_span_index = self.span_index.saturating_sub(self.prior.len() as SpanIndex);
		let pruned = match old_idx {
			Some(o) => {
				self.prior.truncate(o);
				let new_earliest = self.span_index.saturating_sub(self.prior.len() as SpanIndex);
...
impl<'a, T: 'a + Config> InspectingSpans<'a, T> {
	...
	fn compare_and_update_span_slash(
		...
		let reward = if span_record.slashed < slash {
			...
			let difference = slash.saturating_sub(span_record.slashed);
