...
fn basic_minting_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Nfts::mint(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 42, account(1), None));
		...
		assert_ok!(Nfts::mint(RuntimeOrigin::signed_with_basic_filter(account(2)), 1, 69, account(1), None));
...
fn lifecycle_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Nfts::mint(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 70, account(1), None));
		...
		assert_ok!(Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 70, account(2)));
		assert_eq!(Balances::reserved_balance(&account(1)), 8);
		assert_eq!(Balances::reserved_balance(&account(2)), 0);

		assert_ok!(Nfts::set_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 42, bvec![42, 42]));
		assert_eq!(Balances::reserved_balance(&account(1)), 11);
		assert!(ItemMetadataOf::<Test>::contains_key(0, 42));
		assert_ok!(Nfts::set_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 69, bvec![69, 69]));
		...
		assert_noop!(
			Nfts::destroy(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, w),
		...
		assert_ok!(Nfts::burn(RuntimeOrigin::signed_with_basic_filter(account(10)), 0, 42));
		assert_ok!(Nfts::burn(RuntimeOrigin::signed_with_basic_filter(account(20)), 0, 69));
		...
		assert_ok!(Nfts::destroy(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, w));
...
fn destroy_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Nfts::mint(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 42, account(2), None));
		assert_noop!(
			Nfts::destroy(
				RuntimeOrigin::signed_with_basic_filter(account(1)),
				0,
		...
		assert_ok!(Nfts::lock_item_transfer(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 42));
		assert_ok!(Nfts::burn(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42));
...
fn mint_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Nfts::mint(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 43, account(1), None),
		...
		Balances::make_free_balance_be(&account(2), 100);
		assert_noop!(
			Nfts::mint(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 43, account(2), None,),
		...
		assert_noop!(
			Nfts::mint(RuntimeOrigin::signed_with_basic_filter(account(3)), 1, 42, account(3), None),
			Error::<Test>::WitnessRequired
		);
		assert_noop!(
			Nfts::mint(RuntimeOrigin::signed_with_basic_filter(account(2)), 1, 42, account(2), None),
			Error::<Test>::WitnessRequired
		);
		assert_noop!(
			Nfts::mint(
...
fn transfer_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42, account(3)));
		assert_eq!(items(), vec![(account(3), 0, 42)]);
		assert_noop!(
			Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42, account(4)),
			Error::<Test>::NoPermission
		);

		assert_ok!(Nfts::approve_transfer(
			RuntimeOrigin::signed_with_basic_filter(account(3)),
			0,
			42,
			account(2),
			None
		));
		assert_ok!(Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42, account(4)));
		...
		assert_noop!(
			Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(1)), collection_id, 42, account(3)),
...
fn locking_transfer_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Nfts::lock_item_transfer(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 42));
		assert_noop!(
			Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 42, account(2)),
			Error::<Test>::ItemLocked
		);

		assert_ok!(Nfts::unlock_item_transfer(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 42));
		...
		assert_ok!(Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 42, account(2)));
	});
}
...
fn origin_guards_should_work() {
	new_test_ext().execute_with(|| {
		...
		Balances::make_free_balance_be(&account(2), 100);
		assert_ok!(Nfts::set_accept_ownership(RuntimeOrigin::signed_with_basic_filter(account(2)), Some(0)));
		assert_noop!(
			Nfts::transfer_ownership(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, account(2)),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			Nfts::set_team(
		...
		assert_noop!(
			Nfts::lock_item_transfer(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			Nfts::unlock_item_transfer(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			Nfts::mint(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 69, account(2), None),
			Error::<Test>::NoPermission
		);
		assert_ok!(Nfts::mint(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 43, account(2), None));
		assert_noop!(
			Nfts::burn(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 43),
			Error::<Test>::NoPermission
		);
		let w = Nfts::get_destroy_witness(&0).unwrap();
		assert_noop!(
			Nfts::destroy(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, w),
			Error::<Test>::NoPermission
		);
	});
}
...
fn transfer_owner_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(collections(), vec![(account(1), 0)]);
		assert_noop!(
			Nfts::transfer_ownership(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, account(2)),
		...
		assert_ok!(Nfts::transfer_ownership(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, account(2)));
		...
		assert_ok!(Nfts::set_accept_ownership(RuntimeOrigin::signed_with_basic_filter(account(1)), Some(0)));
		assert_noop!(
			Nfts::transfer_ownership(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, account(1)),
			Error::<Test>::NoPermission
		);
		...
		assert_ok!(Nfts::set_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 42, bvec![0u8; 20]));
		assert_ok!(Nfts::set_accept_ownership(RuntimeOrigin::signed_with_basic_filter(account(3)), Some(0)));
		assert_ok!(Nfts::transfer_ownership(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, account(3)));
		...
		assert_noop!(
			Nfts::transfer_ownership(RuntimeOrigin::signed_with_basic_filter(account(3)), 0, account(2)),
			Error::<Test>::Unaccepted
		);
	});
}
...
fn set_team_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Nfts::mint(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42, account(2), None));

		// admin can't transfer/burn items he doesn't own
		assert_noop!(
			Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(3)), 0, 42, account(3)),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			Nfts::burn(RuntimeOrigin::signed_with_basic_filter(account(3)), 0, 42),
			Error::<Test>::NoPermission
		);

		assert_ok!(Nfts::lock_item_transfer(RuntimeOrigin::signed_with_basic_filter(account(4)), 0, 42));
		assert_ok!(Nfts::unlock_item_transfer(RuntimeOrigin::signed_with_basic_filter(account(4)), 0, 42));
		...
		assert_noop!(
			Nfts::lock_item_transfer(RuntimeOrigin::signed_with_basic_filter(account(4)), 0, 42),
			Error::<Test>::NoPermission
		);

		// set all the roles to None
		assert_ok!(Nfts::set_team(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, None, None, None,));
...
fn set_collection_metadata_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Nfts::set_collection_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, bvec![0u8; 20]),
			Error::<Test>::NoPermission,
		);
		assert_ok!(Nfts::force_create(
			RuntimeOrigin::root(),
		...
		assert_noop!(
			Nfts::set_collection_metadata(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, bvec![0u8; 20]),
			Error::<Test>::NoPermission,
		);
		...
		// Cannot over-reserve
		assert_noop!(
			Nfts::set_collection_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, bvec![0u8; 40]),
		...
		assert_noop!(
			Nfts::set_collection_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, bvec![0u8; 15]),
			Error::<Test, _>::LockedCollectionMetadata,
		);
		assert_noop!(
			Nfts::clear_collection_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0),
		...
		assert_noop!(
			Nfts::clear_collection_metadata(RuntimeOrigin::signed_with_basic_filter(account(2)), 0),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			Nfts::clear_collection_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 1),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			Nfts::clear_collection_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0),
...
fn set_item_metadata_should_work() {
	new_test_ext().execute_with(|| {
		...
		// Cannot add metadata to unowned item
		assert_noop!(
			Nfts::set_metadata(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42, bvec![0u8; 20]),
			Error::<Test>::NoPermission,
		);
		...
		// Update deposit
		assert_ok!(Nfts::set_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 42, bvec![0u8; 15]));
		assert_eq!(Balances::free_balance(&account(1)), 13);
		assert_ok!(Nfts::set_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 42, bvec![0u8; 25]));
		assert_eq!(Balances::free_balance(&account(1)), 3);

		// Cannot over-reserve
		assert_noop!(
			Nfts::set_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 42, bvec![0u8; 40]),
			BalancesError::<Test, _>::InsufficientBalance,
		);
		...
		assert_noop!(
			Nfts::set_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 42, bvec![0u8; 15]),
			Error::<Test, _>::LockedItemMetadata,
		);
		assert_noop!(
			Nfts::clear_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 42),
		...
		assert_noop!(
			Nfts::clear_metadata(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42),
			Error::<Test>::NoPermission,
		);
		assert_noop!(
			Nfts::clear_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 1, 42),
			Error::<Test>::NoPermission,
		);
...
fn set_collection_owner_attributes_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Nfts::mint(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 0, account(1), None));
...
fn set_collection_system_attributes_should_work() {
	new_test_ext().execute_with(|| {
		...
		let w = Nfts::get_destroy_witness(&0).unwrap();
		assert_ok!(Nfts::destroy(RuntimeOrigin::signed_with_basic_filter(account(1)), collection_id, w));
...
fn set_item_owner_attributes_should_work() {
	new_test_ext().execute_with(|| {
		...
		// transfer item
		assert_ok!(Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 0, account(3)));
		...
		assert_ok!(Nfts::burn(RuntimeOrigin::signed_with_basic_filter(account(3)), 0, 0));
		assert_eq!(
			attributes(0),
			vec![
...
fn set_attribute_should_respect_lock() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Nfts::mint(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 0, account(1), None));
		assert_ok!(Nfts::mint(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 1, account(1), None));
		...
		assert_ok!(Nfts::set_collection_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, bvec![]));
		assert_ok!(Nfts::lock_collection(
			RuntimeOrigin::signed_with_basic_filter(account(1)),
			0,
...
fn preserve_config_for_frozen_items() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Nfts::burn(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 1));
		assert!(!ItemConfigOf::<Test>::contains_key(0, 1));

		// lock the item and ensure the config stays unchanged
		assert_ok!(Nfts::lock_item_properties(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 0, true, true));
		...
		assert_ok!(Nfts::burn(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 0));
		let config = ItemConfigOf::<Test>::get(0, 0).unwrap();
		assert_eq!(config, expect_config);
...
fn force_update_collection_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Nfts::set_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 42, bvec![0; 20]));
		assert_ok!(Nfts::set_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 69, bvec![0; 20]));
		...
		assert_ok!(Nfts::mint(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 142, account(1), None));
		assert_ok!(Nfts::force_mint(
			RuntimeOrigin::signed_with_basic_filter(account(1)),
			0,
			169,
		...
		assert_ok!(Nfts::set_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 142, bvec![0; 20]));
		assert_ok!(Nfts::set_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0, 169, bvec![0; 20]));
		...
		assert_ok!(Nfts::redeposit(
			RuntimeOrigin::signed_with_basic_filter(account(5)),
			0,
			bvec![0, 42, 50, 69, 100]
		));
		assert_eq!(Balances::reserved_balance(account(1)), 0);

		assert_ok!(Nfts::set_metadata(RuntimeOrigin::signed_with_basic_filter(account(5)), 0, 42, bvec![0; 20]));
		assert_eq!(Balances::reserved_balance(account(5)), 42);

		assert_ok!(Nfts::set_metadata(RuntimeOrigin::signed_with_basic_filter(account(5)), 0, 69, bvec![0; 20]));
...
fn burn_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Nfts::burn(RuntimeOrigin::signed_with_basic_filter(account(5)), 0, 42),
		...
		assert_noop!(
			Nfts::burn(RuntimeOrigin::signed_with_basic_filter(account(0)), 0, 42),
			Error::<Test>::NoPermission
		);

		assert_ok!(Nfts::burn(RuntimeOrigin::signed_with_basic_filter(account(5)), 0, 42));
		assert_ok!(Nfts::burn(RuntimeOrigin::signed_with_basic_filter(account(5)), 0, 69));
		assert_eq!(Balances::reserved_balance(account(1)), 0);
	});
}
...
fn approval_lifecycle_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(3)), 0, 42, account(4)));
		assert_noop!(
			Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(3)), 0, 42, account(3)),
			Error::<Test>::NoPermission
		);
		...
		assert_ok!(Nfts::approve_transfer(
			RuntimeOrigin::signed_with_basic_filter(account(4)),
			0,
			42,
			account(2),
			None
		));
		assert_ok!(Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42, account(2)));
...
fn cancel_approval_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Nfts::cancel_approval(RuntimeOrigin::signed_with_basic_filter(account(2)), 1, 42, account(3)),
			Error::<Test>::UnknownItem
		);
		assert_noop!(
			Nfts::cancel_approval(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 43, account(3)),
			Error::<Test>::UnknownItem
		);
		assert_noop!(
			Nfts::cancel_approval(RuntimeOrigin::signed_with_basic_filter(account(3)), 0, 42, account(3)),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			Nfts::cancel_approval(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42, account(4)),
			Error::<Test>::NotDelegate
		);

		assert_ok!(Nfts::cancel_approval(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42, account(3)));
		assert_noop!(
			Nfts::cancel_approval(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42, account(3)),
			Error::<Test>::NotDelegate
		);
		...
		assert_noop!(
			Nfts::cancel_approval(RuntimeOrigin::signed_with_basic_filter(account(5)), 0, 42, account(3)),
			Error::<Test>::NoPermission
		);

		System::set_block_number(current_block + 3);
		// 5 can cancel the approval since the deadline has passed.
		assert_ok!(Nfts::cancel_approval(RuntimeOrigin::signed_with_basic_filter(account(5)), 0, 42, account(3)));
...
fn approving_multiple_accounts_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(4)), 0, 42, account(6)));
		assert_noop!(
			Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(3)), 0, 42, account(7)),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(5)), 0, 42, account(8)),
			Error::<Test>::NoPermission
		);
	});
}
...
fn approvals_limit_works() {
	new_test_ext().execute_with(|| {
		...
		// the limit is 10
		assert_noop!(
			Nfts::approve_transfer(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42, account(14), None),
...
fn approval_deadline_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(3)), 0, 42, account(4)),
		...
		assert_ok!(Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(6)), 0, 42, account(5)));
	});
}
...
fn cancel_approval_works_with_admin() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Nfts::cancel_approval(RuntimeOrigin::signed_with_basic_filter(account(2)), 1, 42, account(1)),
			Error::<Test>::UnknownItem
		);
		assert_noop!(
			Nfts::cancel_approval(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 43, account(1)),
			Error::<Test>::UnknownItem
		);
		assert_noop!(
			Nfts::cancel_approval(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42, account(4)),
		...
		assert_ok!(Nfts::cancel_approval(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42, account(3)));
		assert_noop!(
			Nfts::cancel_approval(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42, account(1)),
			Error::<Test>::NotDelegate
		);
	});
}
...
fn clear_all_transfer_approvals_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Nfts::clear_all_transfer_approvals(RuntimeOrigin::signed_with_basic_filter(account(3)), 0, 42),
			Error::<Test>::NoPermission
		);

		assert_ok!(Nfts::clear_all_transfer_approvals(RuntimeOrigin::signed_with_basic_filter(account(2)), 0, 42));
		...
		assert_noop!(
			Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(3)), 0, 42, account(5)),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			Nfts::transfer(RuntimeOrigin::signed_with_basic_filter(account(4)), 0, 42, account(5)),
			Error::<Test>::NoPermission
		);
	});
}
...
fn max_supply_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Nfts::set_collection_max_supply(
			RuntimeOrigin::signed_with_basic_filter(user_id.clone()),
			collection_id,
			max_supply
		));
		assert_eq!(
...
fn buy_item_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Nfts::mint(
			RuntimeOrigin::signed_with_basic_filter(user_1.clone()),
			collection_id,
			item_1,
			user_1.clone(),
			None
		...
		// can't buy for less
		assert_noop!(
			Nfts::buy_item(RuntimeOrigin::signed_with_basic_filter(user_2.clone()), collection_id, item_1, 1),
			Error::<Test>::BidTooLow
		);

		// pass the higher price to validate it will still deduct correctly
		assert_ok!(Nfts::buy_item(
			RuntimeOrigin::signed_with_basic_filter(user_2.clone()),
			collection_id,
			item_1,
			price_1 + 1,
		));
		...
		assert_noop!(
			Nfts::buy_item(RuntimeOrigin::signed_with_basic_filter(user_1.clone()), collection_id, item_2, price_2),
			Error::<Test>::NoPermission
		);

		// can't buy when the item is listed for a specific buyer
		assert_noop!(
			Nfts::buy_item(RuntimeOrigin::signed_with_basic_filter(user_2.clone()), collection_id, item_2, price_2),
			Error::<Test>::NoPermission
		);

		// can buy when I'm a whitelisted buyer
		assert_ok!(Nfts::buy_item(
			RuntimeOrigin::signed_with_basic_filter(user_3.clone()),
			collection_id,
			item_2,
			price_2
		));
		...
		assert_noop!(
			Nfts::buy_item(RuntimeOrigin::signed_with_basic_filter(user_2.clone()), collection_id, item_3, price_2),
		...
		// ensure we can't buy an item when the collection or an item are frozen
		{
			...
			assert_noop!(
				buy_item_call.dispatch(RuntimeOrigin::signed_with_basic_filter(user_2.clone())),
				Error::<Test>::ItemsNonTransferable
			);
...
fn create_cancel_swap_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Nfts::cancel_swap(RuntimeOrigin::signed_with_basic_filter(account(2)), collection_id, item_1),
			Error::<Test>::NoPermission
		);
		System::set_block_number(expect_deadline + 1);
		assert_ok!(Nfts::cancel_swap(RuntimeOrigin::signed_with_basic_filter(account(2)), collection_id, item_1));
...
fn pallet_level_feature_flags_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Nfts::buy_item(RuntimeOrigin::signed_with_basic_filter(user_id.clone()), collection_id, item_id, 1),
			Error::<Test>::MethodDisabled
		);
...
fn pre_signed_mints_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Nfts::create(
			RuntimeOrigin::signed_with_basic_filter(user_0.clone()),
			user_1.clone(),
			collection_config_with_all_settings_enabled(),
		));
		...
		assert_ok!(Nfts::burn(RuntimeOrigin::signed_with_basic_filter(user_2.clone()), 0, 0));
...
fn clear_collection_metadata_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Nfts::clear_collection_metadata(RuntimeOrigin::signed_with_basic_filter(account(1)), 0));
		assert_eq!(Collection::<Test>::get(0).unwrap().owner_deposit, 2);
		assert_eq!(Balances::reserved_balance(&account(1)), 12);
