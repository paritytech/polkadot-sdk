...
mod tests {
	...
	fn genesis_voters_can_remove_lock() {
		ExtBuilder::default()
			...
			.build_and_execute(|| {
				...
				assert_ok!(Elections::remove_voter(RuntimeOrigin::signed_with_basic_filter(1)));
				assert_ok!(Elections::remove_voter(RuntimeOrigin::signed_with_basic_filter(2)));
	...
	fn simple_candidate_submission_should_work() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_ok!(submit_candidacy(RuntimeOrigin::signed_with_basic_filter(1)));
			...
			assert_ok!(submit_candidacy(RuntimeOrigin::signed_with_basic_filter(2)));
	...
	fn updating_candidacy_bond_works() {
		ExtBuilder::default().build_and_execute(|| {
			assert_ok!(submit_candidacy(RuntimeOrigin::signed_with_basic_filter(5)));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(5), vec![5], 50));
			assert_eq!(Candidates::<Test>::get(), vec![(5, 3)]);

			// a runtime upgrade changes the bond.
			CANDIDACY_BOND.with(|v| *v.borrow_mut() = 4);

			assert_ok!(submit_candidacy(RuntimeOrigin::signed_with_basic_filter(4)));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(4), vec![4], 40));
	...
	fn candidates_are_always_sorted() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_ok!(submit_candidacy(RuntimeOrigin::signed_with_basic_filter(3)));
	...
	fn dupe_candidate_submission_should_not_work() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_eq!(candidate_ids(), vec![1]);
			assert_noop!(
				submit_candidacy(RuntimeOrigin::signed_with_basic_filter(1)),
	...
	fn member_candidacy_submission_should_not_work() {
		...
		ExtBuilder::default().build_and_execute(|| {
			assert_ok!(submit_candidacy(RuntimeOrigin::signed_with_basic_filter(5)));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![5], 20));
			...
			assert_noop!(submit_candidacy(RuntimeOrigin::signed_with_basic_filter(5)), Error::<Test>::MemberSubmit);
		});
	}
	...
	fn runner_candidate_submission_should_not_work() {
		ExtBuilder::default().desired_runners_up(2).build_and_execute(|| {
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![5, 4], 20));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(1), vec![3], 10));
			...
			assert_noop!(submit_candidacy(RuntimeOrigin::signed_with_basic_filter(3)), Error::<Test>::RunnerUpSubmit);
		});
	}
	...
	fn poor_candidate_submission_should_not_work() {
		ExtBuilder::default().build_and_execute(|| {
			assert_eq!(candidate_ids(), Vec::<u64>::new());
			assert_noop!(
				submit_candidacy(RuntimeOrigin::signed_with_basic_filter(7)),
	...
	fn can_vote_with_custom_stake() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_ok!(submit_candidacy(RuntimeOrigin::signed_with_basic_filter(5)));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![5], 12));
	...
	fn can_update_votes_and_stake() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![5, 4], 15));
			assert_eq!(balances(&2), (18, 2));
	...
	fn updated_voting_bond_works() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_eq!(balances(&2), (20, 0));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![5], 5));
			assert_eq!(balances(&2), (18, 2));
			...
			assert_eq!(balances(&1), (10, 0));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(1), vec![5], 5));
	...
	fn voting_reserves_bond_per_vote() {
		ExtBuilder::default().voter_bond_factor(1).build_and_execute(|| {
			...
			// initial vote.
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![5], 10));
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![5, 3], 18));
			// 2 + 2
			assert_eq!(balances(&2), (16, 4));
			assert_eq!(Voting::<Test>::get(&2).deposit, 4);
			...
			// back to 1 vote.
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![4], 12));
			// 2 + 1
			assert_eq!(balances(&2), (17, 3));
			assert_eq!(Voting::<Test>::get(&2).deposit, 3);
			assert_eq!(has_lock(&2), 12);
	...
	fn cannot_vote_for_no_candidate() {
		ExtBuilder::default().build_and_execute(|| {
			assert_noop!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![], 20), Error::<Test>::NoVotes);
		});
	}
	...
	fn can_vote_for_old_members_even_when_no_new_candidates() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![4, 5], 20));
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(3), vec![4, 5], 10));
		});
	}
	...
	fn prime_works() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(1), vec![4, 3], 10));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![4], 20));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(3), vec![3], 30));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(4), vec![4], 40));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(5), vec![5], 50));
	...
	fn prime_is_kept_if_other_members_leave() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_eq!(members_ids(), vec![4, 5]);
			assert_eq!(PRIME.with(|p| *p.borrow()), Some(5));
			assert_ok!(Elections::renounce_candidacy(RuntimeOrigin::signed_with_basic_filter(4), Renouncing::Member));
	...
	fn prime_is_gone_if_renouncing() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_eq!(members_ids(), vec![4, 5]);
			assert_eq!(PRIME.with(|p| *p.borrow()), Some(5));
			assert_ok!(Elections::renounce_candidacy(RuntimeOrigin::signed_with_basic_filter(5), Renouncing::Member));
		...
		ExtBuilder::default()
			...
			.build_and_execute(|| {
				...
				assert_noop!(
					...
					vote(RuntimeOrigin::signed_with_basic_filter(1), vec![9, 99, 999, 9999], 5),
				...
				assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(1), vec![9, 99, 999, 9999], 5));
				assert_noop!(
					vote(RuntimeOrigin::signed_with_basic_filter(1), vec![9, 99, 999, 9_999, 99_999], 5),
					Error::<Test>::TooManyVotes,
				);
			});
	}
	...
	fn cannot_vote_for_less_than_ed() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_noop!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![4], 1), Error::<Test>::LowBalance);
		})
	}
	...
	fn can_vote_for_more_than_free_balance_but_moot() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![4, 5], 150));
	...
	fn remove_voter_should_work() {
		ExtBuilder::default().voter_bond(8).build_and_execute(|| {
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![5], 20));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(3), vec![5], 30));
	...
	fn non_voter_remove_should_not_work() {
		ExtBuilder::default().build_and_execute(|| {
			assert_noop!(
				Elections::remove_voter(RuntimeOrigin::signed_with_basic_filter(3)),
	...
	fn dupe_remove_should_fail() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_noop!(
				Elections::remove_voter(RuntimeOrigin::signed_with_basic_filter(2)),
				Error::<Test>::MustBeVoter
			);
		});
	}
	...
	fn removed_voter_should_not_be_counted() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_ok!(Elections::remove_voter(RuntimeOrigin::signed_with_basic_filter(4)));
	...
	fn simple_voting_rounds_should_work() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![5], 20));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(4), vec![4], 15));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(3), vec![3], 30));
	...
	fn all_outgoing() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_ok!(Elections::remove_voter(RuntimeOrigin::signed_with_basic_filter(5)));
			assert_ok!(Elections::remove_voter(RuntimeOrigin::signed_with_basic_filter(4)));
	...
	fn defunct_voter_will_be_counted() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(3), vec![4], 30));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(5), vec![5], 50));
	...
	fn only_desired_seats_are_chosen() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![2], 20));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(3), vec![3], 30));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(4), vec![4], 40));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(5), vec![5], 50));
	...
	fn runners_up_should_be_kept() {
		ExtBuilder::default().desired_runners_up(2).build_and_execute(|| {
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![3], 20));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(3), vec![2], 30));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(4), vec![5], 40));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(5), vec![4], 50));
	...
	fn runners_up_should_be_next_candidates() {
		ExtBuilder::default().desired_runners_up(2).build_and_execute(|| {
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(5), vec![5], 10));
	...
	fn seats_should_be_released_when_no_vote() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_ok!(Elections::remove_voter(RuntimeOrigin::signed_with_basic_filter(2)));
			assert_ok!(Elections::remove_voter(RuntimeOrigin::signed_with_basic_filter(3)));
			assert_ok!(Elections::remove_voter(RuntimeOrigin::signed_with_basic_filter(4)));
			assert_ok!(Elections::remove_voter(RuntimeOrigin::signed_with_basic_filter(5)));
	...
	fn incoming_outgoing_are_reported() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(5), vec![4], 8));
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![2], 20));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(1), vec![1], 10));
	...
	fn invalid_votes_are_moot() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(3), vec![3], 30));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(4), vec![4], 40));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(5), vec![10], 50));
	...
	fn runner_up_replacement_maintains_members_order() {
		ExtBuilder::default().desired_runners_up(2).build_and_execute(|| {
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![5], 20));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(4), vec![4], 40));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(5), vec![2], 50));
	...
	fn wrong_renounce_candidacy_should_fail() {
		ExtBuilder::default().build_and_execute(|| {
			assert_noop!(
				Elections::renounce_candidacy(RuntimeOrigin::signed_with_basic_filter(5), Renouncing::Candidate(0)),
				Error::<Test>::InvalidRenouncing,
			);
			assert_noop!(
				Elections::renounce_candidacy(RuntimeOrigin::signed_with_basic_filter(5), Renouncing::Member),
				Error::<Test>::InvalidRenouncing,
			);
			assert_noop!(
				Elections::renounce_candidacy(RuntimeOrigin::signed_with_basic_filter(5), Renouncing::RunnerUp),
				Error::<Test>::InvalidRenouncing,
			);
		})
	}
	...
	fn non_member_renounce_member_should_fail() {
		ExtBuilder::default().desired_runners_up(1).build_and_execute(|| {
			...
			assert_noop!(
				Elections::renounce_candidacy(RuntimeOrigin::signed_with_basic_filter(3), Renouncing::Member),
				Error::<Test>::InvalidRenouncing,
			);
		})
	}
	...
	fn non_runner_up_renounce_runner_up_should_fail() {
		ExtBuilder::default().desired_runners_up(1).build_and_execute(|| {
			...
			assert_noop!(
				Elections::renounce_candidacy(RuntimeOrigin::signed_with_basic_filter(4), Renouncing::RunnerUp),
				Error::<Test>::InvalidRenouncing,
			);
		})
	}
	...
	fn wrong_candidate_count_renounce_should_fail() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_noop!(
				Elections::renounce_candidacy(RuntimeOrigin::signed_with_basic_filter(4), Renouncing::Candidate(2)),
	...
	fn unsorted_runners_up_are_detected() {
		ExtBuilder::default()
			...
			.build_and_execute(|| {
				...
				assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(5), vec![5], 50));
				assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(4), vec![4], 5));
				assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(3), vec![3], 15));
				...
				assert_ok!(submit_candidacy(RuntimeOrigin::signed_with_basic_filter(2)));
				assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![2], 10));
	...
	fn runner_up_to_member_wont_slash() {
		ExtBuilder::default()
			...
			.build_and_execute(|| {
				...
				// swap some votes.
				assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(4), vec![2], 40));
				assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![4], 20));
	...
	fn dupe_vote_is_moot() {
		ExtBuilder::default().desired_members(1).build_and_execute(|| {
			...
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(1), vec![2, 2, 2, 2], 5));
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![2, 2, 2, 2], 20));
	...
	fn remove_defunct_voter_works() {
		ExtBuilder::default().build_and_execute(|| {
			...
			// defunct
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(5), vec![5, 4], 5));
			// defunct
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(4), vec![4], 5));
			// ok
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(3), vec![3], 5));
			// ok
			assert_ok!(vote(RuntimeOrigin::signed_with_basic_filter(2), vec![3, 4], 5));
