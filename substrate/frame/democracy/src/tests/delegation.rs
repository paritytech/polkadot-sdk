...
fn single_proposal_should_work_with_delegation() {
	new_test_ext().execute_with(|| {
		...
		// Delegate first vote.
		assert_ok!(Democracy::delegate(RuntimeOrigin::signed_with_basic_filter(2), 1, Conviction::None, 20));
		let r = 0;
		...
		assert_ok!(Democracy::delegate(RuntimeOrigin::signed_with_basic_filter(3), 1, Conviction::None, 30));
		assert_eq!(tally(r), Tally { ayes: 6, nays: 0, turnout: 60 });

		// Reduce first vote.
		assert_ok!(Democracy::delegate(RuntimeOrigin::signed_with_basic_filter(2), 1, Conviction::None, 10));
		...
		assert_ok!(Democracy::delegate(RuntimeOrigin::signed_with_basic_filter(3), 2, Conviction::None, 30));
		assert_eq!(tally(r), Tally { ayes: 2, nays: 0, turnout: 20 });

		// Main voter cancels their vote
		assert_ok!(Democracy::remove_vote(RuntimeOrigin::signed_with_basic_filter(1), r));
		assert_eq!(tally(r), Tally { ayes: 0, nays: 0, turnout: 0 });

		// First delegator delegates half funds with conviction; nothing changes yet.
		assert_ok!(Democracy::delegate(RuntimeOrigin::signed_with_basic_filter(2), 1, Conviction::Locked1x, 10));
		assert_eq!(tally(r), Tally { ayes: 0, nays: 0, turnout: 0 });
...
fn self_delegation_not_allowed() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			Democracy::delegate(RuntimeOrigin::signed_with_basic_filter(1), 1, Conviction::None, 10),
			Error::<Test>::Nonsense,
		);
	});
}
...
fn cyclic_delegation_should_unwind() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Democracy::delegate(RuntimeOrigin::signed_with_basic_filter(2), 1, Conviction::None, 20));
		assert_ok!(Democracy::delegate(RuntimeOrigin::signed_with_basic_filter(3), 2, Conviction::None, 30));
		assert_ok!(Democracy::delegate(RuntimeOrigin::signed_with_basic_filter(1), 3, Conviction::None, 10));
		let r = 0;
		assert_ok!(Democracy::undelegate(RuntimeOrigin::signed_with_basic_filter(3)));
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(3), r, aye(3)));
		assert_ok!(Democracy::undelegate(RuntimeOrigin::signed_with_basic_filter(1)));
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(1), r, nay(1)));
...
fn single_proposal_should_work_with_vote_and_delegation() {
	...
	new_test_ext().execute_with(|| {
		...
		let r = 0;
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(1), r, aye(1)));
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(2), r, nay(2)));
		assert_eq!(tally(r), Tally { ayes: 1, nays: 2, turnout: 30 });

		// Delegate vote.
		assert_ok!(Democracy::remove_vote(RuntimeOrigin::signed_with_basic_filter(2), r));
		assert_ok!(Democracy::delegate(RuntimeOrigin::signed_with_basic_filter(2), 1, Conviction::None, 20));
...
fn single_proposal_should_work_with_undelegation() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Democracy::delegate(RuntimeOrigin::signed_with_basic_filter(2), 1, Conviction::None, 20));
		assert_ok!(Democracy::undelegate(RuntimeOrigin::signed_with_basic_filter(2)));
...
fn single_proposal_should_work_with_delegation_and_vote() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_eq!(tally(r), Tally { ayes: 3, nays: 0, turnout: 30 });
		assert_ok!(Democracy::undelegate(RuntimeOrigin::signed_with_basic_filter(2)));
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(2), r, aye(2)));
		// Delegated vote is not counted.
		assert_eq!(tally(r), Tally { ayes: 3, nays: 0, turnout: 30 });
	});
}
...
fn conviction_should_be_honored_in_delegation() {
	...
	new_test_ext().execute_with(|| {
		let r = begin_referendum();
		// Delegate and vote.
		assert_ok!(Democracy::delegate(RuntimeOrigin::signed_with_basic_filter(2), 1, Conviction::Locked6x, 20));
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(1), r, aye(1)));
...
fn redelegation_keeps_lock() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Democracy::delegate(RuntimeOrigin::signed_with_basic_filter(2), 3, Conviction::None, 10));
		...
		assert_ok!(Democracy::unlock(RuntimeOrigin::signed_with_basic_filter(2), 2));
		assert_eq!(VotingOf::<Test>::get(2).prior(), &prior_lock);
		assert_eq!(VotingOf::<Test>::get(2).locked_balance(), 20);
