...
fn ooo_inject_referendums_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(1), r2, aye(1)));
		assert_eq!(tally(r2), Tally { ayes: 1, nays: 0, turnout: 10 });

		next_block();

		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(1), r1, aye(1)));
...
fn delayed_enactment_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(4), r, aye(4)));
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(5), r, aye(5)));
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(6), r, aye(6)));
