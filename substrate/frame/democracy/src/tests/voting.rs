...
fn overvoting_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Democracy::vote(RuntimeOrigin::signed_with_basic_filter(1), r, aye(2)),
...
fn split_voting_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Democracy::vote(RuntimeOrigin::signed_with_basic_filter(5), r, v),
			Error::<Test>::InsufficientFunds
		);
		let v = AccountVote::Split { aye: 30, nay: 20 };
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(5), r, v));
...
fn split_vote_cancellation_should_work() {
	new_test_ext().execute_with(|| {
		...
		let v = AccountVote::Split { aye: 30, nay: 20 };
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(5), r, v));
		assert_ok!(Democracy::remove_vote(RuntimeOrigin::signed_with_basic_filter(5), r));
		assert_eq!(tally(r), Tally { ayes: 0, nays: 0, turnout: 0 });
		assert_ok!(Democracy::unlock(RuntimeOrigin::signed_with_basic_filter(5), 5));
...
fn controversial_voting_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(1), r, big_aye(1)));
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(2), r, big_nay(2)));
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(3), r, big_nay(3)));
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(4), r, big_aye(4)));
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(5), r, big_nay(5)));
		assert_ok!(Democracy::vote(RuntimeOrigin::signed_with_basic_filter(6), r, big_aye(6)));
