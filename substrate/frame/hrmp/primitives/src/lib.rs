...
pub enum FailureReason {
	...
	///
	/// On a close or cancel this is a **confirmation wearing a refusal's clothes**: the relay
	/// chain is authoritative for its own map, "no such channel" is the end state those requests
	/// ask for, and it is the only signal the parachain ever gets that a counterparty was
	/// offboarded (the relay chain deletes an outgoing para's channels at the session boundary
	/// without telling anyone). Parachain-side implementations must settle and release deposits
	/// on it, or an offboarded counterparty strands them behind governance forever.
	#[codec(index = 4)]
	NotFound,
...
pub trait ReceiveMigratedChannels {
	...
	///
	/// A record that collides with an existing **system** channel is dropped as redundant, not an
	/// error: the receiving chain opens a deposit-free control channel with every para it takes
	/// over, so a channel that already ran between them arrives twice describing the same thing.
	/// Any other collision fails — landing on a deposit-carrying channel would strand its
	/// deposit — as does a sovereign account that cannot pay; the caller is expected to park such
	/// a record rather than lose it.
