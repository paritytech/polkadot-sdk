...
fn lone_hold_consideration_works() {
	ExtBuilder::default()
		...
		.build_and_execute_with(|| {
			...
			assert_eq!(events(), [RuntimeEvent::Balances(crate::Event::Holding { reason: TestId::Foo, who: who.clone(), amount: 10 }), RuntimeEvent::Balances(crate::Event::Holding { reason: TestId::Foo, who: who.clone(), amount: 5 })]);
