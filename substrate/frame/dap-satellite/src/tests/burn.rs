...
//! Tests for burn flow with DAP Satellite: total issuance preserved, funds go to satellite.
...
#[test]
fn burn_preserves_total_issuance_and_accumulates_in_satellite() {
	new_test_ext().execute_with(|| {
		let satellite = DapSatellitePallet::satellite_account();
		let ed = <Balances as Inspect<_>>::minimum_balance();

		// Given: users have funds, satellite has ED (deactivated at genesis)
