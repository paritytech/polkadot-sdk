...
fn only_root_can_enact_config_change() {
	...
	new_test_ext(1).execute_with(|| {
		...
		let res = Babe::plan_config_change(RuntimeOrigin::none_with_basic_filter(), next_config.clone());

		assert_noop!(res, DispatchError::BadOrigin);

		let res = Babe::plan_config_change(RuntimeOrigin::signed_with_basic_filter(1), next_config.clone());
