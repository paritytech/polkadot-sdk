...
//! Storage migrations for the Identity pallet to v2.
		...
		let authorities: BTreeMap<Suffix<T>, (T::AccountId, u32)> =
			types_v1::UsernameAuthorities::<T>::iter()
				.map(|(account, authority_properties)| {
					(authority_properties.account_id, (account, authority_properties.allocation))
				})
				.collect();
...
impl<T: Config> LazyMigrationV1ToV2<T> {
	pub(crate) fn required_weight(
		...
		match step {
			...
			MigrationState::FinishedPendingUsernames | MigrationState::CleanupAuthorities(_) =>
				T::WeightInfo::migration_v2_cleanup_authority_step(),
			MigrationState::FinishedCleanupAuthorities | MigrationState::CleanupUsernames(_) =>
	...
	pub(crate) fn cleanup_authority_step(maybe_last_key: Option<&Suffix<T>>) -> StepResultOf<T> {
		let mut iter = if let Some(last_key) = maybe_last_key {
	...
	pub(crate) fn cleanup_username_step(maybe_last_key: Option<&Username<T>>) -> StepResultOf<T> {
		let mut iter = if let Some(last_key) = maybe_last_key {
...
impl<T: Config> LazyMigrationV1ToV2<T> {
	...
	pub(crate) fn check_username_cleanup_validity(username: Username<T>, account_id: T::AccountId) {
...
mod tests {
	...
	fn migrate_to_v2() {
		new_test_ext().execute_with(|| {
			...
			assert_eq!(identity_count, count_of_usernames_without_identities + identity_only.len());
