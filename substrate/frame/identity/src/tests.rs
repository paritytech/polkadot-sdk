...
#[test]
fn poke_deposit_increase_works() {
	new_test_ext().execute_with(|| {
		let ten = ten();
		let id_deposit = id_deposit(&ten);
		let sub_deposit: u64 = <<Test as Config>::SubAccountDeposit as Get<u64>>::get();
		...
		// Free balance reduced by the identity deposit and the deposit for one sub
		assert_eq!(Balances::free_balance(10), 1000 - id_deposit - sub_deposit);
		// New registration deposit is now `id_deposit`
		assert_eq!(
		...
		// New subs deposit is `sub_deposit`
		assert_eq!(Identity::subs_of(10), (sub_deposit, vec![20].try_into().unwrap()));
	});
}

#[test]
fn poke_deposit_decrease_works() {
	new_test_ext().execute_with(|| {
		let ten = ten();
		let id_deposit = id_deposit(&ten);
		let sub_deposit: u64 = <<Test as Config>::SubAccountDeposit as Get<u64>>::get();
		let inflated_sub_deposit = sub_deposit * 10;
		SubsOf::<Test>::insert::<&u64, (u64, BoundedVec<u64, ConstU32<2>>)>(
			&10,
			(inflated_sub_deposit, vec![20].try_into().unwrap()),
		);
		SuperOf::<Test>::insert(&20, (&10, Data::Raw(vec![1; 1].try_into().unwrap())));
		// Set a custom registration with some inflated deposit
		let inflated_id_deposit = id_deposit * 10;
		IdentityOf::<Test>::insert(
			&10,
			Registration {
				judgements: BoundedVec::default(),
				deposit: inflated_id_deposit,
				info: ten.clone(),
			},
		);
		assert!(Identity::identity(10).is_some());
		...
		assert_eq!(Balances::free_balance(10), 1000);
		// Simulate the deposits
		Balances::make_free_balance_be(&10, 1000 + inflated_id_deposit + inflated_sub_deposit);
		assert_ok!(Balances::reserve(&10, inflated_id_deposit + inflated_sub_deposit));

		// Poke
		assert_ok!(Identity::poke_deposit(RuntimeOrigin::signed(1), 10));

		// Free balance increased by the difference between the inflated deposits and the actual
		// deposits
		assert_eq!(
			Balances::free_balance(10),
			1000 + (inflated_id_deposit - id_deposit) + (inflated_sub_deposit - sub_deposit)
		);
		assert_eq!(Balances::reserved_balance(10), id_deposit + sub_deposit);
		// New registration deposit is now `id_deposit`
		assert_eq!(
			Identity::identity(&10),
			Some(Registration {
...
fn poke_deposit_decrease_insufficient_reserved() {
	new_test_ext().execute_with(|| {
		...
		// Balance is untouched
		assert_eq!(Balances::free_balance(10), 1000);
		// Ensure the reserved amount is 0 so that `unreserve`` will not succeed
		assert_eq!(Balances::free_balance(10), 1000);

		// Poke - this should panic in the test because of a `debug_assert` that the entire deposit
		// is successfully unreserved
		let _ = Identity::poke_deposit(RuntimeOrigin::signed(1), 10);
	});
}
