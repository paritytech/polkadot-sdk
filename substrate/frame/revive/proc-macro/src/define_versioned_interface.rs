...
mod tests {
	...
	use super::{handle_define_versioned_interface, DefineVersionedInterfaceInput};
	...
	fn generated_code_contains_expected_items() {
		...
		assert_eq!(
			...
			output using the `update_define_versioned_interface_expanded_fixture` test."
		);
	...
	fn update_define_versioned_interface_expanded_fixture() {
		let generated = expand_fixture_input().to_string();

		let fixture_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
			.join("tests")
			.join("assets")
			.join("define_versioned_interface.expanded.rs");
	...
	fn parse_fixture() -> syn::File {
		syn::parse_file(include_str!("../tests/assets/define_versioned_interface.expanded.rs"))
			.unwrap()
	}
	...
	fn expand_fixture_input() -> TokenStream2 {
		handle_define_versioned_interface(
			parse2::<DefineVersionedInterfaceInput>(fixture_input()).unwrap(),
		)
		.unwrap()
	}
	...
	fn fixture_input() -> TokenStream2 {
		quote! {
			#[doc = "latest input payload"]
			#[derive(Clone, Debug, PartialEq)]
			pub struct QueryVersionedInputPayloadV4<'a, T: Clone + Default + PartialEq, const N: usize> {
				pub key: T,
				pub borrowed: Option<&'a T>,
				pub bytes: [u8; N],
			}

			#[doc = "latest output payload"]
			#[derive(Clone, Debug, PartialEq)]
			pub(crate) struct QueryVersionedOutputPayloadV4<R: Clone + PartialEq, E: Clone + PartialEq> {
				pub value: Option<R>,
				pub error: Option<E>,
			}

			#[doc = "base input payload"]
			#[derive(Clone, Debug, Eq, PartialEq)]
			pub struct QueryVersionedInputPayloadV3<'a, T: Clone + PartialEq> {
				pub key: &'a T,
			}

			#[doc = "base output payload"]
			#[derive(Debug, PartialEq)]
			pub struct QueryVersionedOutputPayloadV3<R: Clone + PartialEq> {
				pub value: Option<R>,
			}
		}
	}
