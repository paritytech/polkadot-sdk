	...
	fn call(
		...
		match input {
			...
			ISystemCalls::toAccountId(ISystem::toAccountIdCall { input }) => {
				env.gas_meter_mut().charge_weight_token(RuntimeCosts::ToAccountId)?;
			...
			ISystemCalls::minimumBalance(ISystem::minimumBalanceCall {}) => {
				env.gas_meter_mut().charge_weight_token(RuntimeCosts::MinimumBalance)?;
			...
			ISystemCalls::weightLeft(ISystem::weightLeftCall {}) => {
				env.gas_meter_mut().charge_weight_token(RuntimeCosts::WeightLeft)?;
				let ref_time = env.gas_meter().weight_left().unwrap_or_default().ref_time();
				let proof_size = env.gas_meter().weight_left().unwrap_or_default().proof_size();
