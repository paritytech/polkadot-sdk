...
use crate::{
	...
	tracing::if_tracing,
	vm::{
		BackendCosts, CodeInfo, InterpreterBackend, RuntimeCosts, StorageAccessKind,
...
impl<T: Config> Pallet<T> {
		...
		CallOf<T>: SetWeightLimit,
	{
		...
		#[cfg(revive_jit)]
		sp_virtualization::Module::clear_cache();
	...
	pub fn code(address: &H160) -> Vec<u8> {
		...
		match info.account_type {
			AccountType::Contract(contract) => pristine_code::get::<T>(&contract.code_hash)
				.map(|code| code.into())
				.unwrap_or_default(),
