	...
	E: Executable<T>,
{
	...
	fn transfer<S: State>(
		...
		let value_balance_with_dust = BalanceWithDust::<BalanceOf<T>>::from_value::<T>(value)
			.map_err(|_| Error::<T>::BalanceConversionFailed)?;
		if value_balance_with_dust.is_zero() {
			return Ok(());
		}
		...
		// Emit a synthetic ERC-20 Transfer log for ETH value transfers when
		// trace_transfers is enabled (per ERC-7528 / eth_simulateV1 spec).
		if exec_config.trace_transfers {
			...
			block_storage::capture_ethereum_log(&emitter, &data, &topics, false);
			Contracts::<T>::deposit_event(Event::ContractEmitted {
				contract: emitter,
				data: data.to_vec(),
				topics,
			});
	...
	E: Executable<T>,
{
	...
	fn deposit_event(&mut self, topics: Vec<H256>, data: Vec<u8>) {
		...
		block_storage::capture_ethereum_log(&contract, &data, &topics, true);
