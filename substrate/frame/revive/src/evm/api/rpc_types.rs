...
impl GenericTransaction {
	...
	/// Returns `true` when the transaction's fields look like those of a simple EOA-to-EOA
	/// value transfer: a non-pallet `to` address, empty calldata, no access list,
	/// no EIP-7702 authorization list, no EIP-4844 blob payload, and no blob gas fee.
	pub fn has_simple_transfer_fields(&self) -> bool {
		self.to.is_some_and(|to| to != crate::RUNTIME_PALLETS_ADDR) &&
			self.input.is_empty() &&
