...
/// The system precompile's `terminate` must not destroy a delegated EOA.
/// Only the EVM SELFDESTRUCT opcode (with EIP-6780 semantics) is allowed.
#[test]
fn precompile_blocked_for_delegated_eoa() {
	use crate::{storage::AccountInfo, tests::eip7702::DelegationTestSetup};
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		setup.authorize(contract.addr);

		// Call terminate via the system precompile — should revert
		let result = builder::bare_call(setup.signer.address)
			...
			.build_and_unwrap_result();
		assert!(result.did_revert(), "terminate precompile should revert for delegated EOA");
