...
use crate::{
	...
	test_utils::{builder::Contract, ALICE, ALICE_ADDR, DJANGO, DJANGO_ADDR},
	tests::{
		builder,
...
// #[test_case(FixtureType::Solc)]
#[test_case(FixtureType::Resolc)]
fn self_destruct_by_syscall_tracing_works(fixture_type: FixtureType) {
	use crate::{
		evm::{CallTrace, CallType, PrestateTrace, PrestateTracer, PrestateTracerConfig, Tracer},
		tests::test_utils::get_contract,
		tracing::trace,
		CallTracer, Trace, U256,
	};

	let (binary, _code_hash) = compile_module_with_type("Terminate", fixture_type).unwrap();
