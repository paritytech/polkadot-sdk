...
/// A deployed contract and the hash of its runtime code, which for EVM is not what we compiled.
...
struct ReadsAndWrites {
	...
	/// Keys it does not track, named by their storage item.
...
/// Returns what the left touched and the right did not, subtracting field by field.
...
fn label_key(key: &[u8]) -> String {
	use frame_support::{storage::transactional::TRANSACTION_LEVEL_KEY, traits::StorageInfoTrait};
	if key == TRANSACTION_LEVEL_KEY {
		return "transaction level".to_string();
	}
...
fn reads_and_writes_of_call(
	...
	let (mut ext, caller, target) = state_with_contracts(caller_code, target_code);
...
fn a_transfer_touches_what_it_declares(dust: bool) {
	...
	let (mut ext, from, to) = state_with_contracts(&caller_code, &target_code);
