...
mod tests {
	...
	use crate::{
		...
		storage::{AccountInfo, ContractInfo, StorageMapValueOf, StorageValueOf},
	...
	fn seed_code_upload(hash: H256, owner: AccountId32, deposit: u128) {
		...
		CodeInfoOf::<Test>::insert(
			hash,
			StorageMapValueOf::<CodeInfoOf<Test>>::new(CodeInfo::<Test>::new_with_deposit(
				owner, deposit,
			)),
		);
	}
	...
	fn seed_contract(address: H160, code_hash: H256, storage_deposit: u128) {
		...
		AccountInfoOf::<Test>::insert(
			address,
			StorageMapValueOf::<AccountInfoOf<Test>>::new(AccountInfo::<Test> {
				account_type: AccountType::Contract(info),
				dust: 0,
			}),
		);
	...
	fn phase_three_rewrites_legacy_deletion_queue_entries() {
		...
		ExtBuilder::default().genesis_config(None).build().execute_with(|| {
			...
			DeletionQueueCounter::<Test>::set(StorageValueOf::<DeletionQueueCounter<Test>>::new(
				q.clone(),
			));
			let _ = &mut q;
