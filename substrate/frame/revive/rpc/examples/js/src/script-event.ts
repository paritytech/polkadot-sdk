...
async function call(address: string, abi: InterfaceAbi) {
	...
	const tx = (await contract.triggerEvent()) as TransactionResponse;
	console.log("Call transaction hash:", tx.hash);
	const receipt = await tx.wait();
	if (receipt) {
		console.log("Transaction logs:", receipt.logs[0].toJSON());
	}
}

try {
	const res = await compile({
		["Event.sol"]: { content: readFileSync("./contracts/Event.sol", "utf8") },
	});

	const out = res.contracts["Event.sol"]["EventExample"];
	const {
		abi,
		evm: {
