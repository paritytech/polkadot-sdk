...
use crate::{
	ClientError, H160,
	subxt_client::{
		self, SrcChainConfig, runtime_types::pallet_revive_types::storage as revive_storage_types,
	},
};
use subxt::{OnlineClient, storage::Storage};

type ContractInfo = revive_storage_types::types::contract::ContractInfoV1<u128>;
...
impl StorageApi {
	...
	pub async fn get_contract_info(
		...
		let revive_storage_types::types::contract::AccountTypeV1::Contract(contract_info) =
			info.account_type
		else {
			return Err(ClientError::ContractNotFound);
		};
