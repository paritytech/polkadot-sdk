...
fn can_pause_specific_call() {
	new_test_ext().execute_with(|| {
		assert_ok!(call_transfer(1, 1).dispatch(RuntimeOrigin::signed_with_basic_filter(0)));

		assert_ok!(TxPause::pause(
			RuntimeOrigin::signed_with_basic_filter(mock::PauseOrigin::get()),
			full_name::<Test>(b"Balances", b"transfer_allow_death")
		));

		assert_err!(
			call_transfer(2, 1).dispatch(RuntimeOrigin::signed_with_basic_filter(2)),
			frame_system::Error::<Test>::CallFiltered
		);
		assert_ok!(call_transfer_keep_alive(3, 1).dispatch(RuntimeOrigin::signed_with_basic_filter(3)));
	});
}
...
fn can_pause_all_calls_in_pallet_except_on_whitelist() {
	new_test_ext().execute_with(|| {
		...
		assert_err!(
			batch_call.clone().dispatch(RuntimeOrigin::signed_with_basic_filter(0)),
			frame_system::Error::<Test>::CallFiltered
		);
	});
}
...
fn can_unpause_specific_call() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(TxPause::unpause(
			RuntimeOrigin::signed_with_basic_filter(mock::UnpauseOrigin::get()),
			full_name::<Test>(b"Balances", b"transfer_allow_death"),
		));
		assert_ok!(call_transfer(4, 1).dispatch(RuntimeOrigin::signed_with_basic_filter(0)));
	});
}
...
fn can_filter_balance_in_proxy_when_paused() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Proxy::proxy(RuntimeOrigin::signed_with_basic_filter(2), 1, None, Box::new(call_transfer(1, 1))));
		System::assert_last_event(
...
fn fails_to_pause_unpausable_call_when_other_call_is_paused() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(call_transfer_keep_alive(3, 1).dispatch(RuntimeOrigin::signed_with_basic_filter(3)));
		assert_err!(
			call_transfer(2, 1).dispatch(RuntimeOrigin::signed_with_basic_filter(0)),
			frame_system::Error::<Test>::CallFiltered
		);
	});
}
