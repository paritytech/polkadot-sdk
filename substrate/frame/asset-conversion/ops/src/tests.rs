...
fn migrate_pool_account_id() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(PoolAssets::balance(lp_token.clone(), &user), 216);
		...
		let total_issuance_lp_token = PoolAssets::total_issuance(lp_token.clone());
		...
		assert_eq!(PoolAssets::balance(lp_token.clone(), &prior_pool_account), 100);
		...
		assert_eq!(PoolAssets::balance(lp_token.clone(), &new_pool_account), 100);
