...
use crate::{
	...
	storage::meter::Diff,
	wasm::{CodeInfo, Determinism, Environment, WasmBlob, BYTES_PER_PAGE},
	...
	T: Config,
{
	...
	let builder = E::define().unwrap();
	// .map_err(|err| {
	// 	log::debug!(target: LOG_TARGET, "New code rejected on wasmi instantiation: {:?}", err);
	...
	WasmBlob::<T>::instantiate::<E, _>(builder, module, (), schedule).map_err(|err| {
