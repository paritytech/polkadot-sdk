...
use crate::{engine::on_chain::EncodeScope, ReturnFlags};
use scale::Encode;
...
mod sys {
	...
	extern "C" {
		...
		pub fn code_hash(account_ptr: *const u8, out_ptr: *mut u8, out_len_ptr: *mut u32) -> u32;
...
pub fn debug_message(message: &str) {
	...
	if unsafe { DEBUG_ENABLED || FIRST_RUN } {
		...
		if !matches!(ReturnCode(ret_val).into(), Err(super::Error::LoggingDisabled)) {
			// SAFETY: safe because executing in a single threaded context
...
pub fn ecdsa_to_eth_address(pubkey: &[u8; 33], output: &mut [u8; 20]) -> Result {
	let ret_val = unsafe { sys::ecdsa_to_eth_address(pubkey.as_ptr(), output.as_mut_ptr()) };
	ReturnCode(ret_val).into()
}
