...
mod sys {
	...
	extern "C" {
		...
		pub fn weight_to_fee(gas: u64, output_ptr: Ptr32Mut<[u8]>, output_len_ptr: Ptr32Mut<u32>);
		pub fn gas_left(output_ptr: Ptr32Mut<[u8]>, output_len_ptr: Ptr32Mut<u32>);
		pub fn value_transferred(output_ptr: Ptr32Mut<[u8]>, output_len_ptr: Ptr32Mut<u32>);
		...
		pub fn hash_keccak_256(input_ptr: Ptr32<[u8]>, input_len: u32, output_ptr: Ptr32Mut<[u8]>);
		pub fn hash_blake2_256(input_ptr: Ptr32<[u8]>, input_len: u32, output_ptr: Ptr32Mut<[u8]>);
		pub fn hash_blake2_128(input_ptr: Ptr32<[u8]>, input_len: u32, output_ptr: Ptr32Mut<[u8]>);
		pub fn hash_sha2_256(input_ptr: Ptr32<[u8]>, input_len: u32, output_ptr: Ptr32Mut<[u8]>);
...
pub fn clear_storage(key: &[u8]) -> Option<u32> {
	let ret_code = unsafe { sys::clear_storage(Ptr32::from_slice(key), key.len() as u32) };
	ret_code.into()
}
...
pub fn storage_contains(key: &[u8]) -> Option<u32> {
	let ret_code = unsafe { sys::contains_storage(Ptr32::from_slice(key), key.len() as u32) };
	ret_code.into()
}
...
pub fn input(output: &mut &mut [u8]) {
	let mut output_len = output.len() as u32;
	{
		unsafe { sys::input(Ptr32Mut::from_slice(output), Ptr32Mut::from_ref(&mut output_len)) };
	}
	extract_from_slice(output, output_len as usize);
}
...
pub fn call_runtime(call: &[u8]) -> Result {
	let ret_code = unsafe { sys::call_runtime(Ptr32::from_slice(call), call.len() as u32) };
	ret_code.into()
}
...
pub fn debug_message(message: &str) {
	...
	if unsafe { DEBUG_ENABLED || FIRST_RUN } {
		...
		let ret_code = unsafe { sys::debug_message(Ptr32::from_slice(bytes), bytes.len() as u32) };
...
pub fn own_code_hash(output: &mut [u8]) {
	let mut output_len = output.len() as u32;
	unsafe { sys::own_code_hash(Ptr32Mut::from_slice(output), Ptr32Mut::from_ref(&mut output_len)) }
}
