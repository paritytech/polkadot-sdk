...
unsafe impl GlobalAlloc for BumpAllocator {
	...
	unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
		...
		match INNER.as_mut().expect("We just set the value above; qed").alloc(layout) {
...
mod fuzz_tests {
	...
	fn should_allocate_arbitrary_sized_bytes(n: usize) -> TestResult {
		...
		assert_eq!(inner.upper_limit, expected_limit, "The upper bound of our heap doesn't match.");
	...
	fn should_allocate_arbitrary_byte_sequences(sequence: Vec<isize>) -> TestResult {
		...
		let pages_required =
			sequence.iter().fold(0, |acc, &x| acc + required_pages(x as usize).unwrap());
