...
pub trait HostFn {
    /// Returns the number of times specified contract exists on the call stack. Delegated
    /// calls are not counted as separate calls.
    ///
    /// # Parameters
    ///
    ...
    ///
    /// If the available space in `output` is less than the size of the value a trap is
    /// triggered.
    ///
    /// # Parameters
    ///
    ...
    /// # Parameters
    ///
    /// - `code_hash`: The code hash of the dependency. Should be decodable as an
    ///   `T::Hash`. Traps otherwise.
    ...
    /// `flags` set to [`CallFlags::ALLOW_REENTRY`]. See the newer version for
    /// documentation.
    ...
    /// - `callee`: The address of the callee. Should be decodable as an `T::AccountId`.
    ///   Traps otherwise.
    ...
    ///   `Option<T::Balance>`. Traps otherwise. Passing `None` means setting no specific
    ///   limit for the call, which implies storage usage up to the limit of the parent
    ///   call.
    /// - `value`: The value to transfer into the contract. Should be decodable as a
    ///   `T::Balance`. Traps otherwise.
    /// - `input`: The input data buffer used to call the contract.
    /// - `output`: A reference to the output data buffer to write the call output buffer.
    ...
    /// stated otherwise.
    ///
    /// - [CalleeReverted][`crate::ReturnErrorCode::CalleeReverted]: Output buffer is
    ///   returned.
    ...
    /// triggered.
    ///
    /// If this is a top-level call (i.e. initiated by an extrinsic) the origin address of
    /// the extrinsic will be returned. Otherwise, if this call is initiated by
    /// another contract then the address of the contract will be returned.
    ///
    /// If there is no address associated with the caller (e.g. because the caller is
    /// root) then it traps with `BadOrigin`.
    ///
    /// # Parameters
    ///
    ...
    /// stack.
    ///
    /// Prefer this over [`is_contract()`][`Self::is_contract`] when checking whether your
    /// contract is being called by a contract or a plain account. The reason is that
    /// it performs better since it does not need to do any storage lookups.
    ///
    /// # Return
    ///
    ...
    ///
    /// Note that only the origin of the call stack can be root. Hence this function
    /// returning `true` implies that the contract is being called by the origin.
    ///
    ...
    /// the exception of the return type. Still a valid thing to call when not interested
    /// in the return value.
    ...
    /// # Parameters
    ///
    /// - `account_id`: The address of the contract.Should be decodable as an
    ...
    /// Checks whether there is a value stored under the given key.
    ///
    /// The key length must not exceed the maximum defined by the contracts module
    /// parameter.
    ///
    /// # Parameters
    /// - `key`: The storage key.
    ...
    /// # Note
    ///
    /// Even though no action is taken when debug message recording is disabled there is
    /// still a non trivial overhead (and weight cost) associated with calling this
    /// function. Contract languages should remove calls to this function (either at
    /// runtime or compile time) when not being executed as an RPC. For example, they
    /// could allow users to disable logging through compile time flags (cargo
    /// features) for on-chain deployment. Additionally, the return value of this
    /// function can be cached in order to prevent further calls at runtime.
    ...
    /// # Errors
    ///
    /// An error means that the call wasn't successful and no output buffer is returned
    ...
    /// Deposit a contract event with the data buffer and optional list of topics. There
    /// is a limit on the maximum number of topics specified by `event_topics`.
    ///
    /// There should not be any duplicates in `topics`.
    ///
    /// # Parameters
    ///
    /// - `topics`: The topics list encoded as `Vec<T::Hash>`. It can't contain
    ///   duplicates.
    ...
    /// Instantiate a contract with the specified code hash.
    ///
    /// This function creates an account and executes the constructor defined in the code
    /// specified by the code hash.
    ///
    /// # Parameters
    ///
    ...
    ///   `T::Balance`. Traps otherwise.
    /// - `input`: The input data buffer.
    /// - `address`: A reference to the address buffer to write the address of the
    ///   contract. If `None` is provided then the output buffer is not copied.
    ...
    /// # Errors
    ///
    /// Please consult the [ReturnErrorCode][`crate::ReturnErrorCode`] enum declaration
    /// for more information on those errors. Here we only note things specific to
    /// this function.
    ///
    ...
    /// # Parameters
    ///
    /// - `account_id`: The address to check. Should be decodable as an `T::AccountId`.
    ///   Traps otherwise.
    ///
    /// # Return
    ///
    ...
    /// Returns the number of times the currently executing contract exists on the call
    /// stack in addition to the calling instance.
    ///
    /// # Return
    ///
    ...
    /// # Note
    ///
    /// There are a couple of important considerations which must be taken into account
    /// when using this API:
    ///
    ...
    ///
    /// 2. Contracts using this API can't be assumed as having deterministic addresses.
    ///    Said another
    /// way, when using this API you lose the guarantee that an address always identifies
    /// a specific code hash.
    ///
    ...
    /// [`set_code_hash()`][`Self::set_code_hash`] and the next caller would use the
    /// old code.
    ///
    /// # Parameters
    ///
    /// - `code_hash`: The hash of the new code. Should be decodable as an `T::Hash`.
    ///   Traps otherwise.
    ///
    /// # Errors
    ///
    ...
    /// exception of the return type. Still a valid thing to call  for fixed sized storage
    /// key, when not interested in the return value.
    ...
    /// Set the value at the given key in the contract storage.
    ///
    /// The key and value lengths must not exceed the maximums defined by the contracts
    /// module parameters.
    ///
    /// # Parameters
    ///
    ...
    fn sr25519_verify(signature: &[u8; 64], message: &[u8], pub_key: &[u8; 32])
        -> Result;
    ...
    /// # Parameters
    ///
    /// - `account_id`: The address of the account to transfer funds to. Should be
    ///   decodable as an `T::AccountId`. Traps otherwise.
    /// - `value`: The value to transfer. Should be decodable as a `T::Balance`. Traps
    ///   otherwise.
    ///
    /// # Errors
    ///
    ...
    /// buffer. The data is encoded as `T::Balance`.
    ///
    /// If the available space in `output` is less than the size of the value a trap is
    /// triggered.
    ///
    ...
    /// works with *ref_time* Weight only. It is recommended to switch to the latest
    ...
    /// This is equivalent to dispatching `pallet_xcm::execute` through call_runtime,
    /// except that the function is called directly instead of being dispatched.
    ///
    /// # Parameters
    ///
    ...
    /// # Return
    ///
    /// Returns `Error::Success` when the XCM execution attempt is successful. When the
    /// XCM execution fails, `ReturnCode::XcmExecutionFailed` is returned
    #[deprecated(
        note = "Unstable function. Behaviour can change without further notice. Use only for testing."
    )]
    ...
    /// This is equivalent to dispatching `pallet_xcm::send` through `call_runtime`,
    /// except that the function is called directly instead of being dispatched.
    ///
    /// # Parameters
    ///
    ...
    /// # Return Value
    ///
    /// Returns `ReturnCode::Success` when the query was successfully created. When the
    /// query creation fails, `ReturnCode::XcmQueryFailed` is returned.
    #[deprecated(
        note = "Unstable function. Behaviour can change without further notice. Use only for testing."
    )]
