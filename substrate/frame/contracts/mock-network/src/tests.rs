...
fn instantiate_test_contract(name: &str) -> AccountId {
    ...
    ParaA::execute_with(|| {
        ...
        parachain::Assets::mint_into(0u32.into(), &contract_addr, INITIAL_BALANCE)
            .unwrap();
    });
    Relay::execute_with(|| {
        let sovereign_account =
            parachain_account_sovereign_account_id(1u32, contract_addr.clone());
...
fn test_xcm_execute() {
    ...
    ParaA::execute_with(|| {
        ...
        // Check if the funds are subtracted from the account of Alice and added to the
        // account of Bob.
        let initial = INITIAL_BALANCE;
        assert_eq!(ParachainBalances::free_balance(BOB), initial + amount);
        assert_eq!(
            ParachainBalances::free_balance(&contract_addr),
            initial - amount
        );
    });
}
...
fn test_xcm_execute_incomplete() {
    ...
    ParaA::execute_with(|| {
        ...
        let message: Xcm<()> = Xcm(vec![
            WithdrawAsset(vec![(Here, amount).into()].into()),
            // This will fail as the contract does not have enough balance to complete
            // both withdrawals.
...
fn test_xcm_execute_filtered_call() {
    ...
    ParaA::execute_with(|| {
        ...
        let call =
            parachain::RuntimeCall::System(frame_system::Call::remark { remark: vec![] });
        ...
        assert_err!(
            result.result,
            frame_system::Error::<parachain::Runtime>::CallFiltered
        );
    });
}
...
fn test_xcm_execute_reentrant_call() {
    ...
    ParaA::execute_with(|| {
        let transact_call =
            parachain::RuntimeCall::Contracts(pallet_contracts::Call::call {
...
fn test_xcm_send() {
    ...
    Relay::execute_with(|| {
        ...
        assert_eq!(
            relay_chain::Balances::locks(&parachain_account_sovereign_account_id(
                1,
                contract_addr
            )),
            vec![BalanceLock {
...
fn test_xcm_query() {
    ...
    ParaA::execute_with(|| {
        let match_querier = Location::from(AccountId32 {
            network: None,
            id: ALICE.into(),
        });
...
fn test_xcm_take_response() {
    ...
    ParaA::execute_with(|| {
        ...
        let query_id =
            ParachainPalletXcm::new_query(responder, 1u32.into(), querier.clone());
