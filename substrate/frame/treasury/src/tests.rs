...
/// Tests for BurnDestination = BurnToDap configuration.
/// This module creates a separate mock runtime with DAP integration, reusing
/// most configuration from the parent `Test` runtime.
mod burn_to_dap_tests {
	use super::*;
	use frame_support::traits::{NeverEnsureOrigin, OnInitialize};

	type Block = frame_system::mocking::MockBlock<TestWithDap>;

	frame_support::construct_runtime!(
		pub enum TestWithDap {
			System: frame_system,
			Balances: pallet_balances,
			Treasury: treasury,
			Dap: pallet_dap,
	...
	// Reuse frame_system and pallet_balances configs from parent (same as Test)
	#[derive_impl(frame_system::config_preludes::TestDefaultConfig)]
	impl frame_system::Config for TestWithDap {
		type AccountId = u128;
		type Lookup = IdentityLookup<Self::AccountId>;
		type Block = Block;
		type AccountData = pallet_balances::AccountData<u64>;
	...
	#[derive_impl(pallet_balances::config_preludes::TestDefaultConfig)]
	impl pallet_balances::Config for TestWithDap {
		type AccountStore = System;
	}
	...
	impl pallet_dap::Config for TestWithDap {
		type Currency = Balances;
		type PalletId = DapPalletId;
	}

	// Treasury config: same as Test except BurnDestination redirects to DAP
	impl Config for TestWithDap {
		type PalletId = TreasuryPalletId;
		type Currency = Balances;
		type RejectOrigin = frame_system::EnsureRoot<u128>;
		type RuntimeEvent = RuntimeEvent;
		type SpendPeriod = ConstU64<2>;
		type Burn = Burn;
		type BurnDestination = pallet_dap::BurnToDap<TestWithDap, Balances>;
		type WeightInfo = ();
		type SpendFunds = ();
		type MaxApprovals = ConstU32<100>;
	...
	fn go_to_block_dap(n: u64) {
		<TestWithDap as Config>::BlockNumberProvider::set_block_number(n);
		<Treasury as OnInitialize<u64>>::on_initialize(n);
	}

	#[test]
	fn treasury_burn_redirected_to_dap_buffer() {
		ExtBuilder::default().build().execute_with(|| {
			let dap_buffer = pallet_dap::Pallet::<TestWithDap>::buffer_account();

			pallet_dap::Pallet::<TestWithDap>::ensure_buffer_account_exists();

			let initial_total_issuance = pallet_balances::TotalIssuance::<TestWithDap>::get();

			// Given: Treasury has 1025 (1024 pot + 1 ED), DAP buffer is empty
			Balances::make_free_balance_be(&Treasury::account_id(), 1025);
			assert_eq!(Treasury::pot(), 1024);
			assert_eq!(Balances::free_balance(&dap_buffer), 0);

			// When: first spend period passes
			go_to_block_dap(2);
			// Then: 50% burned (512), pot = 512, DAP = 512
			assert_eq!(Treasury::pot(), 512);
			assert_eq!(Balances::free_balance(&dap_buffer), 512);

			// When: second spend period passes
			go_to_block_dap(4);
			// Then: 50% of 512 burned (256), pot = 256, DAP = 512 + 256 = 768
			assert_eq!(Treasury::pot(), 256);
			assert_eq!(Balances::free_balance(&dap_buffer), 768);

			// When: third spend period passes
			go_to_block_dap(6);
			// Then: 50% of 256 burned (128), pot = 128, DAP = 768 + 128 = 896
			assert_eq!(Treasury::pot(), 128);
			assert_eq!(Balances::free_balance(&dap_buffer), 896);

			// And: total issuance is preserved throughout
			assert_eq!(
				pallet_balances::TotalIssuance::<TestWithDap>::get(),
				initial_total_issuance + 1024
			);
		});
	}
}
