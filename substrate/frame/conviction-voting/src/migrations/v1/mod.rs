...
impl<T: Config<I>, I: 'static> SteppedMigration for SteppedMigrationV1<T, I> {
	...
	fn step(
		...
		let required = <T as Config<I>>::WeightInfo::step_to_v1();
		...
		loop {
			...
			if let Some((account, class, value)) = iter.next() {
				...
				match value {
					...
					v0::Voting::Delegating(v0::Delegating {
						balance,
						target,
						conviction,
						delegations,
	...
	fn post_upgrade(prev: Vec<u8>) -> Result<(), frame_support::sp_runtime::TryRuntimeError> {
		...
		// Decodes.
		let prev_vec =
			Vec::<(T::AccountId, v0::ClassOf<T, I>, v0::VotingOf<T, I>)>::decode(&mut &prev[..])
