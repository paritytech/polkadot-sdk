...
fn decode_impl(
	...
	let decode_impl_single = {
		...
		quote! {
			let #name =
			<
				_fepsp::alloc::vec::Vec<(_fepsp::codec::Compact<#voter_type>, _fepsp::codec::Compact<#target_type>)>
				as
				_fepsp::codec::Decode
			>::decode(value)?;
			let #name = #name
				.into_iter()
				.map(|(v, t)| (v.0, t.0))
				.collect::<_fepsp::alloc::vec::Vec<_>>();
		}
	};
...
fn encode_impl(ident: &syn::Ident, count: usize) -> TokenStream2 {
	...
	quote!(
		impl _fepsp::codec::Encode for #ident {
			fn encode(&self) -> _fepsp::alloc::vec::Vec<u8> {
				let mut r = vec![];
				#encode_impl_single
				#encode_impl_rest
				r
