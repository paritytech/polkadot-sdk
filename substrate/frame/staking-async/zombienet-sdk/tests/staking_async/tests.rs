...
use crate::staking_async::{
	activate_ah_client, assethub as assethub_metadata, build_network_config,
	polkadot as polkadot_metadata, set_validator_count,
};
						...
						if event.pallet_name() == "StakingNextAhClient" {
							match event.variant_name() {
								"ValidatorSetReceived" => {
									let ev = event.as_event::<polkadot_metadata::staking_next_ah_client::events::ValidatorSetReceived>()?.expect("ValidatorSetReceived event should be parsed");
						...
						if event.pallet_name() == "StakingNextRcClient" {
							match event.variant_name() {
								"SessionReportReceived" => {
									let ev = event.as_event::<assethub_metadata::staking_next_rc_client::events::SessionReportReceived>()?.expect("SessionReportReceived event should be parsed");
							...
							match event.variant_name() {
								"PagedElectionProceeded" => {
									...
									let ev = event.as_event::<assethub_metadata::staking::events::PagedElectionProceeded>()?.expect("PagedElectionProceeded event should be parsed");
