			...
			rc_block = rc_blocks_sub.next() => {
				if let Some(block) = rc_block {
					...
					for event in block.events().await?.iter() {
						let event = event?;
						if event.pallet_name() == "StakingAsyncAhClient" {
							match event.variant_name() {
								...
								_ => {
									log!(debug,
										"ON RC: block: {}; pallet: StakingAsyncAhClient;  Event: {:?}; Fields: {:?}",
										block.number(),
										event.variant_name(),
										event.field_values()?
									);
			...
			ah_block = ah_blocks_sub.next() => {
				if let Some(block) = ah_block {
					...
					for event in block.events().await?.iter() {
						...
						} else if event.pallet_name() == "StakingAsyncAhClient" {
							assert!(false, "StakingAsyncAhClient event on AH client");
						}
						else {
...
/// Sets `Mode` in `StakingAsyncAhClient` pallet to `Active`.
