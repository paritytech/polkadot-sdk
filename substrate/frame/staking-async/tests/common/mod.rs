...
const LOG_TARGET: &str = "staking-async::tests::common";
...
pub trait TestEventHandler {
	fn on_ah_client_event(
		&mut self,
		event: AhClientEvent,
		stop: &mut Option<tokio::sync::oneshot::Sender<()>>,
	);
						...
						if event.pallet_name() == "StakingNextAhClient" {
							match event.variant_name() {
								"ValidatorSetReceived" => {
									...
									event_handler.on_ah_client_event(ev, &mut notify_stop);
								},
								_ => {
									log::debug!(
										target: LOG_TARGET,
						...
						if event.pallet_name() == "StakingNextRcClient" {
							match event.variant_name() {
								"SessionReportReceived" => {
									...
									event_handler.on_rc_client_event(ev, &mut notify_stop);
								},
								_ => {
									log::debug!(
										target: LOG_TARGET,
							...
							log::debug!( target: LOG_TARGET, "block: {} Event for pallet: {:?}; variant: {:?}; fields: {:?}", block.number(), event.pallet_name(), event.variant_name(), event.field_values()?);
						}
					}
				}
			}
			_ = &mut stop => {
				log::info!(target: LOG_TARGET, "Event loop stopped");
				break
			}
		}
	}
...
pub async fn build_network_config() -> Result<NetworkConfig, anyhow::Error> {
	...
	log::info!(target: LOG_TARGET, "Using images: {images:?}");
