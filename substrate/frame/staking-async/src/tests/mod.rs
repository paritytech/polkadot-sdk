...
mod hold_migration {
	...
	fn migrate_currency_force_withdraw_exceeds_active_drains_unlocking() {
		...
		ExtBuilder::default().existential_deposit(1).has_stakers(false).build_and_execute(|| {
			let alice = 300;
			// Bond alice with a small active stake and one unlocking chunk.
			let _ = asset::set_stakeable_balance::<Test>(&alice, 1000);
			assert_ok!(Staking::bond(RuntimeOrigin::signed(alice), 500, RewardDestination::Staked));
