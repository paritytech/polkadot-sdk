...
use sp_runtime::{
    impl_tx_ext_default,
    traits::{
        AtLeast32BitUnsigned, Bounded, CheckedAdd, Dispatchable, DispatchInfoOf, DispatchOriginOf, Member, One, SaturatedConversion, Saturating, Zero,
        TransactionExtension, ValidateResult,
    },
    transaction_validity::{InvalidTransaction, ValidTransaction},
};
use frame_support::{pallet_prelude::*, traits::{EnsureOrigin, Get}};
...
where
    Origin: Into<Result<frame_system::RawOrigin<Origin::AccountId>, Origin>>
		+ From<frame_system::RawOrigin<Origin::AccountId>> + Clone,
...
pub mod pallet {
    ...
    use sp_runtime::traits::{Dispatchable, One, Hash};
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn check_and_execute_proposal(
            ...
            mut proposal_info: ProposalInfo<T::Hash, BlockNumberFor<T>, T::OriginId, T::MaxApprovals>,
        ) -> DispatchResult {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn propose(
            ...
            ensure!(!<Proposals<T>>::contains_key(&proposal_hash, &origin_id),
        ...
        pub fn approve(
            ...
            let mut proposal_info = <Proposals<T>>::get(&call_hash, &origin_id)
                ...
                Err(e) => match e {
                    DispatchError::Module(module_error) => {
                        if module_error.index == <Self as PalletInfoAccess>::index() as u8 {
                            let insufficient_approvals_index = Self::error_index(Error::<T>::InsufficientApprovals);
    ...
    pub type Approvals<T: Config> = StorageDoubleMap<
        _,
        Blake2_128Concat, (T::Hash, T::OriginId),
        Blake2_128Concat, T::OriginId,
        bool,
        ValueQuery,
    >;
