...
#[test]
fn burn_extrinsic_preserves_total_issuance_and_accumulates_in_buffer() {
...
#[test]
fn direct_burn_from_credits_buffer_and_preserves_total_issuance() {
	new_test_ext().execute_with(|| {
		// Given
		let buffer = DapPallet::buffer_account();
		let ed = <Balances as Inspect<_>>::minimum_balance();
		...
		assert_eq!(Balances::free_balance(buffer), ed + 50);
