...
fn re_insert_unchanged_priority_no_op() {
	build_and_execute(|| {
		insert(1, 100, 50);
		let steps = LinkedList::re_insert(1, 100, 50, None, None).unwrap();
		assert_eq!(steps, 0);
...
fn re_insert_in_place_when_position_still_valid() {
	build_and_execute(|| {
		...
		let steps = LinkedList::re_insert(1, 200, 30, None, None).unwrap();
		assert_eq!(steps, 0);
...
fn re_insert_priority_increase_moves_toward_head() {
	build_and_execute(|| {
		...
		assert_ok!(LinkedList::re_insert(1, 3, 95, prev, next));
...
fn re_insert_priority_decrease_moves_toward_tail() {
	build_and_execute(|| {
		...
		assert_ok!(LinkedList::re_insert(1, 1, 5, prev, next));
...
fn re_insert_unknown_errors() {
	build_and_execute(|| {
		assert_storage_noop!(assert!(matches!(
			LinkedList::re_insert(1, 100, 50, None, None),
...
fn re_insert_slow_path_failure_leaves_storage_untouched() {
	build_and_execute(|| {
		...
		assert_storage_noop!(assert!(matches!(
			LinkedList::re_insert(1, 1, 5, None, Some(1)),
