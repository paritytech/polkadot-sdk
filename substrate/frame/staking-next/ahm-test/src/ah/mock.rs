...
construct_runtime! {
	pub enum Runtime {
		...
		Staking: pallet_staking_next,
		RcClient: pallet_staking_next_rc_client,
...
type MaxVotesPerVoter = pallet_staking_next::MaxNominationsOf<Runtime>;
parameter_types! {
...
impl pallet_staking_next::Config for Runtime {
	...
	type NominationsQuota = pallet_staking_next::FixedNominationsQuota<16>;

	type VoterList = pallet_staking_next::UseNominatorsAndValidatorsMap<Self>;
...
impl pallet_staking_next_rc_client::SendToRelayChain for DeliverToRelay {
	type AccountId = AccountId;

	fn validator_set(report: pallet_staking_next_rc_client::ValidatorSetReport<Self::AccountId>) {
		if let Some(mut local_queue) = LocalQueue::get() {
			...
			shared::in_rc(|| {
				...
				pallet_staking_next_ah_client::Pallet::<crate::rc::Runtime>::validator_set(
					origin,
					report.clone(),
				)
				.unwrap();
...
pub enum OutgoingMessages {
	ValidatorSet(pallet_staking_next_rc_client::ValidatorSetReport<AccountId>),
}
...
impl ExtBuilder {
	...
	pub fn build(self) -> TestState {
		...
		pallet_balances::GenesisConfig::<Runtime> { balances, ..Default::default() }
			...
			.unwrap();
		pallet_staking_next::GenesisConfig::<Runtime> {
			stakers,
			validator_count: 4,
...
pub(crate) fn staking_events_since_last_call() -> Vec<pallet_staking_next::Event<T>> {
