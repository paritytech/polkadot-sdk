...
//!
//! The counter-part for this pallet is `pallet-staking-next-ah-client` on the relay chain.
//!
...
//!
//! After queuing, they are passed to pallet-staking-next via [`AHStakingInterface`].
//!
//! The calls are:
//!
...
//! Moreover, pallet-staking-next no longer has a notion of timestamp locally, and only relies in the
...
//! `pallet-staking-next` should rotate sessions/eras going forward.
//!
//! ### Synchronous Model
//!
...
/// Our communication trait of `pallet-staking-next-rc-client` -> `pallet-staking-next`.
///
/// This is merely a shorthand to avoid tightly-coupling the staking pallet to this pallet. It
/// limits what we can say to `pallet-staking-next` to only these functions.
...
/// The communication trait of `pallet-staking-next` -> `pallet-staking-next-rc-client`.
