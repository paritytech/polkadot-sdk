...
fn inherent_expand() {
	...
	assert!(InherentData::new().check_extrinsics(&block.into_lazy_block()).ok());
	...
	assert!(InherentData::new().check_extrinsics(&block.into_lazy_block()).fatal_error());
	...
	assert!(inherent.check_extrinsics(&block.into_lazy_block()).fatal_error());
