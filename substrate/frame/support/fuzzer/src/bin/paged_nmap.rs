...
//! paged-storage-nmap-fuzzer`. `honggfuzz` CLI options can be used by setting `HFUZZ_RUN_ARGS`,
	...
	TestExternalities::default().execute_with(|| {
		...
		for op in ops.into_iter() {
			...
			*total += op.op.exec_map(op.key);
			...
			assert_eq!(NMap::len((op.key,)), *total as u64);

			// The queue removes the metadata when empty.
			if *total == 0 {
