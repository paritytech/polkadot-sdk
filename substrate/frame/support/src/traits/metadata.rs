...
/// 1. An current storage version
/// 2. An on-chain storage version
///
/// The current storage version is the version of the pallet as defined in the local development
...
/// correctly during a migration so that it matches the current storage version.
pub trait GetStorageVersion {
	...
	/// case that they wish to compare the current storage version to the on-chain storage version.
	type CurrentStorageVersion;

	/// Returns the current storage version as specified in the
