...
impl<'a> Default for StorageNoopGuard<'a> {
	fn default() -> Self {
		Self {
			storage_root: crate::__private::storage_root(),
...
impl<'a> StorageNoopGuard<'a> {
	...
	pub fn from_error_message(error_message: &'a str) -> Self {
		Self { storage_root: crate::__private::storage_root(), error_message }
	}
...
impl<'a> Drop for StorageNoopGuard<'a> {
	fn drop(&mut self) {
		...
		assert_eq!(crate::__private::storage_root(), self.storage_root, "{}", self.error_message,);
	}
}
