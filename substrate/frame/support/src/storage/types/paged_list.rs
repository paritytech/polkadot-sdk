...
use crate::{
	...
	storage::{
		lists::*,
		types::paged_nmap::{StoragePagedNMapMeta, StoragePagedNMapPrefix},
		EncodeLikeTuple, KeyLenOf, StorageEntryMetadataBuilder, StoragePrefixedContainer,
		TupleToEncodedIter,
	},
	...
	MaxPages: Get<Option<u32>>,
{
	fn storage_info() -> Vec<StorageInfo> {
		vec![StorageInfo {
			pallet_name: <Self as StoragePrefixedContainer>::pallet_prefix().to_vec(),
...
pub(crate) mod mock {
	pub use super::*;
	pub use crate::storage::types::paged_nmap::*;
...
mod tests {
	...
	fn append_works() {
		TestExternalities::default().execute_with(|| {
			...
			assert_eq!(List::vec(), (0..1000).collect::<Vec<_>>());
		});
	}
	...
	fn simple_drain_works() {
		TestExternalities::default().execute_with(|| {
			...
			assert_eq!(List::take(), (0..1000).collect::<Vec<_>>());

			assert_eq!(List::meta(), Default::default());

			// all gone
			assert_eq!(List::vec(), Vec::<u32>::new());
	...
	fn partial_drain_works() {
		TestExternalities::default().execute_with(|| {
			...
			// 50 gone, 50 to go
			assert_eq!(List::vec(), (50..100).collect::<Vec<_>>());
		});
	}
	...
	fn drain_append_iter_works() {
		TestExternalities::default().execute_with(|| {
			for r in 1..=100 {
				...
				assert_eq!(List::vec(), (0..12).cycle().take(r * 12).collect::<Vec<_>>());
