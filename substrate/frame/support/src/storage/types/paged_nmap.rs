...
pub struct Page<K, KArg, V> {
	...
	pub(crate) values: SkipIter<sp_std::vec::IntoIter<V>>,
...
pub struct StoragePagedNMapIterator<Prefix, Key, KArg, Value, HeapSize, MaxPages> {
	...
	/// Whether all visited pages should be removed.
	drain: bool,
	/// The cached metadata.
	meta: StoragePagedNMapMeta<Prefix, Key, KArg, Value, HeapSize, MaxPages>,
	/// The key that we are iterating on.
	key: KArg,
}
	...
	HeapSize: Get<u32>,
{
	...
	/// Return the elements of the map as a vector.
	#[cfg(feature = "std")]
	pub fn vec<KArg: EncodeLikeTuple<Key::KArg> + TupleToEncodedIter + Clone>(
		key: KArg,
	) -> Vec<Value> {
		<Self as frame_support::storage::StorageKeyedList<_, _>>::iter(key).collect()
	}

	/// Return and remove the elements of the map as a vector.
	#[cfg(feature = "std")]
	pub fn take<KArg: EncodeLikeTuple<Key::KArg> + TupleToEncodedIter + Clone>(
		key: KArg,
	) -> Vec<Value> {
...
{
	fn pallet_prefix() -> &'static [u8] {
		StoragePagedNMapPrefix::<Prefix>::pallet_prefix()
	}
	...
	Value: FullCodec + scale_info::StaticTypeInfo,
{
	fn build_metadata(docs: Vec<&'static str>, entries: &mut Vec<StorageEntryMetadataIR>) {
		...
		let entry = StorageEntryMetadataIR {
			...
			default: None::<Value>.encode(), // FAIL-CI
			docs,
		};
...
pub(crate) mod mock {
	pub use super::*;
	pub use crate::pallet_prelude::NMapKey;
	use codec::Compact;
	pub use frame_support::{parameter_types, Blake2_128Concat};
...
mod tests {
	use super::mock::*;
	use crate::storage::lists::StorageKeyedList;
	use codec::Encode;
	...
	fn append_works() {
		TestExternalities::default().execute_with(|| {
			...
			assert_eq!(NMap::vec((123,)), (0..1000).collect::<Vec<_>>());
		});
	}
	...
	fn simple_drain_works() {
		TestExternalities::default().execute_with(|| {
			let _g = crate::StorageNoopGuard::default(); // All in all a No-Op
			NMap::append_many((123,), 0..1000);

			assert_eq!(NMap::take((123,)), (0..1000).collect::<Vec<_>>());

			assert_eq!(NMap::meta((123,)), Default::default());

			// all gone
			assert_eq!(NMap::vec((123,)), Vec::<u32>::new());
	...
	fn partial_drain_works() {
		TestExternalities::default().execute_with(|| {
			...
			// 50 gone, 50 to go
			assert_eq!(NMap::vec((123,)), (50..100).collect::<Vec<_>>());
		});
	}
	...
	fn drain_append_iter_works() {
		TestExternalities::default().execute_with(|| {
			for r in 1..=100 {
				...
				assert_eq!(NMap::vec((123,)), (0..12).cycle().take(r * 12).collect::<Vec<_>>());
	...
	fn append_storage_layout() {
		TestExternalities::default().execute_with(|| {
			...
			let vec = Vec::<u32>::decode(&mut &raw[..]).unwrap();
			assert_eq!(vec.len(), 5, "First page contains 5");

			let key = page_key::<Prefix, KeyGen, Key>((123,), 1);
			let raw = sp_io::storage::get(&key).expect("Page should be present");
			let vec = Vec::<u32>::decode(&mut &raw[..]).unwrap();
			assert_eq!(vec.len(), 4, "Second page contains 4");
