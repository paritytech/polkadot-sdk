...
pub fn expand_outer_task(
	...
	for decl in pallet_decls {
		if let Some(_) = decl.find_part("Task") {
			...
			// TODO: `pallet` will probably not be needed when `Task` is generated by macro

			from_impls.push(quote! {
				impl From<#path::pallet::Task<#runtime_name>> for RuntimeTask {
					fn from(hr: #path::pallet::Task<#runtime_name>) -> Self {
			...
			task_variants.push(quote! {
				#[codec(index = #index)]
				#variant_name(#path::pallet::Task<#runtime_name>),
			});
