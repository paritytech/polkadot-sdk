...
pub fn expand_genesis_config(def: &mut Def) {
	let Some(genesis_config) = &def.genesis_config else {
		return;
	};
	...
	let genesis_config_item =
		&mut def.item.content.as_mut().expect(msg!("Checked by def parser")).1
			[genesis_config.index];
	...
	match genesis_config_item {
		...
		_ => unreachable!("{}", msg!("Checked by genesis_config parser")),
	}
}
