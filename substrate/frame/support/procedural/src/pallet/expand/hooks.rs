...
pub fn expand_hooks(def: &mut Def) -> proc_macro2::TokenStream {
	...
	quote::quote_spanned!(span =>
			...
			for #pallet_ident<#type_use_gen> #where_clause
		{
			...
			fn pre_upgrade() -> Result<#frame_support::__private::alloc::vec::Vec<u8>, #frame_support::sp_runtime::TryRuntimeError> {
				<
					Self
					as
					#frame_support::traits::Hooks<#frame_system::pallet_prelude::BlockNumberFor::<T>>
			...
			#[cfg(feature = "try-runtime")]
			fn post_upgrade(state: #frame_support::__private::alloc::vec::Vec<u8>) -> Result<(), #frame_support::sp_runtime::TryRuntimeError> {
