	...
	let consts = config_consts.chain(extra_consts).map(|const_| {
		...
		quote::quote!({
			#frame_support::__private::metadata_ir::PalletConstantMetadataIR {
				...
				docs: #frame_support::__private::alloc::vec![ #( #doc ),* ],
			}
		})
	});
	...
	quote::quote!(
		impl<#type_impl_gen> #pallet_ident<#type_use_gen> #completed_where_clause{
			...
			pub fn pallet_constants_metadata()
				-> #frame_support::__private::alloc::vec::Vec<#frame_support::__private::metadata_ir::PalletConstantMetadataIR>
			{
				#frame_support::__private::alloc::vec![ #( #consts ),* ]
			}
		}
	)
}
