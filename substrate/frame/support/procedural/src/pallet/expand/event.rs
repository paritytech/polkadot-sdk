...
pub fn expand_event(def: &mut Def) -> proc_macro2::TokenStream {
	...
	let deposit_event = if let Some(deposit_event) = &event.deposit_event {
		if event.capture_for_chain_batch {
			generate_deposit_event_with_capture(def, event, deposit_event, &maybe_allow_attrs)
		} else {
			let event_use_gen = &event.gen_kind.type_use_gen(event.attr_span);
...
fn generate_deposit_event_with_capture(
	...
	maybe_allow_attrs: &[syn::Attribute],
...
fn generate_capture_arms(
	...
	let mut fields_by_variant: BTreeMap<&syn::Ident, Vec<&CapturedField>> = BTreeMap::new();
	...
	for (variant_ident, fields) in fields_by_variant {
		...
		let capture_calls: Vec<_> = field_idents
			.iter()
			...
			.map(|(field_ident, field_name)| {
				quote::quote! {
					#frame_support::dispatch_context::capture_event_field(
						...
						#field_name,
						#frame_support::__private::codec::Encode::encode(#field_ident),
					);
				}
			})
			.collect();
