...
pub fn expand_donkey(def: &mut Def) -> proc_macro2::TokenStream {
	let donkey =
		if let Some(donkey) = &def.donkey_field { donkey } else { return Default::default() };
	...
	// 		impl<#type_impl_gen> #frame_support::sp_runtime::BuildStorage for
	// #gen_cfg_ident<#gen_cfg_use_gen> #where_clause 		{
	// 			fn assimilate_storage(&self, storage: &mut #frame_support::sp_runtime::Storage) ->
