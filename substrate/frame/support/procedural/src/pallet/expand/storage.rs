...
pub fn expand_storages(def: &mut Def) -> proc_macro2::TokenStream {
		...
		quote::quote_spanned!(storage.attr_span =>
			#(#cfg_attrs)*
			{
				<#full_ident as #frame_support::storage::StorageEntryMetadataBuilder>::build_metadata(
					#frame_support::__private::alloc::vec![
						#( #docs, )*
					],
					&mut entries,
				);
	...
	let try_decode_entire_state = {
		...
		quote::quote!(
			...
			{
				fn try_decode_entire_state() -> Result<usize, #frame_support::__private::alloc::vec::Vec<#frame_support::traits::TryDecodeEntireStorageError>> {
					...
					let mut errors = #frame_support::__private::alloc::vec::Vec::new();
			...
			#completed_where_clause
		{
			...
			pub fn storage_metadata() -> #frame_support::__private::metadata_ir::PalletStorageMetadataIR {
				#frame_support::__private::metadata_ir::PalletStorageMetadataIR {
					...
					entries: {
						#[allow(unused_mut)]
						let mut entries = #frame_support::__private::alloc::vec![];
						#( #entries_builder )*
						entries
					},
				}
