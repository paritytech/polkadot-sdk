...
/// ## Specification:
///
/// Authorize process comes with 4 macros:
///
/// 2 attributes on top the authorized call function:
///
...
/// 1 proc-macro call:
///
/// * `ensure_authorized_origin!($origin)` - generates the check of the origin of the call. The
///   `AuthorizeCall` transaction extension replaces the `None` origin to the origin specific for
///   the authorized call. Then during the dispatch of the call, this check will ensure that only
///   this call-specific origin is accepted, and return a dispatch error otherwise.
///
///   NOTE: The origin specific for the authorized call is an origin variant generated by the macro.
///   E.g. for the call with authorization `foo` and `bar`, the macro will generate 2 variants:
///   ```
...
///   }
///   ```
///   This aggregated type is inserted into the pallet's origin.
///
/// 1 optional attribute inside the origin enum:
///
...
/// Allows to define the place to generate the specific origin for authorized call.
///
/// See [`frame_support_procedural::authorize`] for more information on authorized works.
///
/// # Example:
///
/// ```
...
/// ```
#[proc_macro_attribute]
pub fn authorized_call(_: TokenStream, _: TokenStream) -> TokenStream {
	pallet_macro_stub()
}

/// Macro to be used as first statement inside a call with `#[pallet::authorize]` attribute.
///
/// The macro will expand to a check that the origin is the one authorized for the call.
/// And return a dispatch error otherwise.
///
/// See [`authorize`] for more information about call authorization.
///
/// # Example
///
/// ```
...
/// ```
#[proc_macro]
pub fn ensure_authorized_origin(input: TokenStream) -> TokenStream {
	let error = syn::Error::new(
		proc_macro2::Span::call_site(),
		"Invalid usage, macro must be use as the first statement in the block of \
		a call method with `#[pallet::authorize]` attribute. E.g.:
		```
		#[pallet::call_index(0)]
		#[pallet::authorize(|arg1| Ok(Default::default()))]
		pub fn my_call_function(origin: OriginFor<T>, arg1: u32) -> DispatchResult {
			ensure_authorized_origin!(origin);
			Ok(())
		}
		```
