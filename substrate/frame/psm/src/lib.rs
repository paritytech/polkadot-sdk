...
/// Minimal PSM interface for external consumers on stable2603.
///
/// Upstream master exposes `frame_support::traits::tokens::stable::PsmInterface`;
/// that trait is not present on stable2603, so we define a local shape here.
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		/// Asset id of the internal asset, used to read its decimals via
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn add_external_asset(origin: OriginFor<T>, asset_id: T::AssetId) -> DispatchResult {
			...
			let internal_decimals = InternalDecimals::<T>::get().ok_or(Error::<T>::Unexpected)?;
				...
				T::Fungibles::decimals(T::InternalAssetId::get()) == internal_decimals,
