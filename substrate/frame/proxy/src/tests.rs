...
#[test]
fn force_add_proxy_requires_root() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			Proxy::force_add_proxy(RuntimeOrigin::signed(1), 1, 2, ProxyType::Any, 0),
			DispatchError::BadOrigin,
		);
		assert_noop!(
			Proxy::force_add_proxy(RuntimeOrigin::none(), 1, 2, ProxyType::Any, 0),
			DispatchError::BadOrigin,
		);
	});
}
...
#[test]
fn force_remove_proxy_requires_root() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			Proxy::force_remove_proxy(RuntimeOrigin::signed(1), 1, 2, ProxyType::Any, 0),
			DispatchError::BadOrigin,
		);
		assert_noop!(
			Proxy::force_remove_proxy(RuntimeOrigin::none(), 1, 2, ProxyType::Any, 0),
			DispatchError::BadOrigin,
		);
	});
}
...
#[test]
fn force_remove_proxies_requires_root() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			Proxy::force_remove_proxies(RuntimeOrigin::signed(1), 1),
			DispatchError::BadOrigin,
		);
		assert_noop!(
			Proxy::force_remove_proxies(RuntimeOrigin::none(), 1),
			DispatchError::BadOrigin,
		);
	});
}
...
#[test]
fn force_remove_announcements_requires_root() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			Proxy::force_remove_announcements(RuntimeOrigin::signed(1), 1),
			DispatchError::BadOrigin,
		);
		assert_noop!(
			Proxy::force_remove_announcements(RuntimeOrigin::none(), 1),
			DispatchError::BadOrigin,
		);
	});
}
...
#[test]
fn force_add_proxy_works() {
	new_test_ext().execute_with(|| {
		assert_eq!(Balances::free_balance(1), 10);
		assert_eq!(proxy_hold_balance(&1), 0);

		// Force add proxy as root
		assert_ok!(Proxy::force_add_proxy(RuntimeOrigin::root(), 1, 2, ProxyType::Any, 0));

		// Verify proxy was added
		let (proxies, deposit) = Proxies::<Test>::get(1);
		assert_eq!(proxies.len(), 1);
		assert_eq!(proxies[0].delegate, 2);
		assert_eq!(proxies[0].proxy_type, ProxyType::Any);
		assert_eq!(proxies[0].delay, 0);

		// Verify deposit was held
		assert_eq!(proxy_hold_balance(&1), deposit);

		// Verify event was emitted
		assert!(System::events().iter().any(|record| matches!(
			record.event,
			RuntimeEvent::Proxy(Event::ForceProxyAdded {
				delegator: 1,
				delegatee: 2,
				proxy_type: ProxyType::Any,
				delay: 0,
...
#[test]
fn force_remove_proxy_works() {
	new_test_ext().execute_with(|| {
		// Setup: add a proxy first
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed(1), 2, ProxyType::Any, 0));
		let initial_deposit = proxy_hold_balance(&1);
		assert!(initial_deposit > 0);

		// Force remove proxy as root
		assert_ok!(Proxy::force_remove_proxy(RuntimeOrigin::root(), 1, 2, ProxyType::Any, 0));
		...
		// Verify deposit was released
		assert_eq!(proxy_hold_balance(&1), 0);

		// Verify event was emitted
		assert!(System::events().iter().any(|record| matches!(
			record.event,
			RuntimeEvent::Proxy(Event::ForceProxyRemoved {
				delegator: 1,
				delegatee: 2,
				proxy_type: ProxyType::Any,
				delay: 0,
...
#[test]
fn force_remove_proxies_works() {
	new_test_ext().execute_with(|| {
		// Setup: add multiple proxies
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed(1), 2, ProxyType::Any, 0));
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed(1), 3, ProxyType::JustTransfer, 5));
		let initial_deposit = proxy_hold_balance(&1);
		assert!(initial_deposit > 0);

		let (initial_proxies, _) = Proxies::<Test>::get(1);
		let proxy_count = initial_proxies.len() as u32;

		// Force remove all proxies as root
		assert_ok!(Proxy::force_remove_proxies(RuntimeOrigin::root(), 1));
		...
		assert!(System::events().iter().any(|record| matches!(
			record.event,
			RuntimeEvent::Proxy(Event::ForceProxiesCleared {
				delegator: 1,
				proxy_count: count,
				released: amount,
			}) if count == proxy_count && amount == initial_deposit
		)));
	});
}

#[test]
fn force_remove_announcements_works() {
	new_test_ext().execute_with(|| {
		// Setup: add proxy and make announcement
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed(1), 2, ProxyType::Any, 1));
		let call_hash = BlakeTwo256::hash_of(&RuntimeCall::Balances(
			pallet_balances::Call::transfer_allow_death { dest: 1, value: 1000 },
		));
		assert_ok!(Proxy::announce(RuntimeOrigin::signed(2), 1, call_hash));

		// Verify announcement was created
		let (announcements, initial_deposit) = Announcements::<Test>::get(2);
		assert_eq!(announcements.len(), 1);
		assert!(initial_deposit > 0);

		// Force remove announcements as root
		assert_ok!(Proxy::force_remove_announcements(RuntimeOrigin::root(), 2));

		// Verify announcements were removed
		let (announcements, deposit) = Announcements::<Test>::get(2);
		...
		assert!(System::events().iter().any(|record| matches!(
			record.event,
			RuntimeEvent::Proxy(Event::ForceAnnouncementsCleared {
				delegate: 2,
...
#[test]
fn force_add_proxy_handles_deposits_correctly() {
	new_test_ext().execute_with(|| {
		let initial_balance = Balances::free_balance(1);
		let initial_hold = proxy_hold_balance(&1);

		// Force add proxy
		assert_ok!(Proxy::force_add_proxy(RuntimeOrigin::root(), 1, 2, ProxyType::Any, 0));

		// Verify deposit was properly calculated and held
		let expected_deposit = Proxy::deposit(1);
		assert_eq!(proxy_hold_balance(&1), initial_hold + expected_deposit);
		assert_eq!(Balances::free_balance(1), initial_balance - expected_deposit);

		// Add another proxy to test deposit accumulation
		assert_ok!(Proxy::force_add_proxy(RuntimeOrigin::root(), 1, 3, ProxyType::JustTransfer, 5));

		let expected_total_deposit = Proxy::deposit(2);
		assert_eq!(proxy_hold_balance(&1), expected_total_deposit);
		assert_eq!(Balances::free_balance(1), initial_balance - expected_total_deposit);
	});
}

#[test]
fn force_remove_proxies_releases_deposits_correctly() {
	new_test_ext().execute_with(|| {
		// Setup: add proxies with deposits
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed(1), 2, ProxyType::Any, 0));
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed(1), 3, ProxyType::JustTransfer, 5));

		let initial_free = Balances::free_balance(1);
		let held_amount = proxy_hold_balance(&1);
		assert!(held_amount > 0);

		// Force remove all proxies
		assert_ok!(Proxy::force_remove_proxies(RuntimeOrigin::root(), 1));

		// Verify all deposits were released back to free balance
		assert_eq!(proxy_hold_balance(&1), 0);
		assert_eq!(Balances::free_balance(1), initial_free + held_amount);
	});
}

#[test]
fn force_remove_announcements_releases_deposits_correctly() {
	new_test_ext().execute_with(|| {
		// Setup: add proxy and announcements
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed(1), 2, ProxyType::Any, 1));

		let call_hash1 = BlakeTwo256::hash_of(&RuntimeCall::Balances(
			pallet_balances::Call::transfer_allow_death { dest: 1, value: 100 },
		));
		let call_hash2 = BlakeTwo256::hash_of(&RuntimeCall::Balances(
			pallet_balances::Call::transfer_allow_death { dest: 1, value: 200 },
		));

		assert_ok!(Proxy::announce(RuntimeOrigin::signed(2), 1, call_hash1));
		assert_ok!(Proxy::announce(RuntimeOrigin::signed(2), 1, call_hash2));

		let initial_free = Balances::free_balance(2);
		let held_amount =
			hold_balance(&2, RuntimeHoldReason::Proxy(HoldReason::AnnouncementDeposit));
		assert!(held_amount > 0);

		// Force remove announcements
		assert_ok!(Proxy::force_remove_announcements(RuntimeOrigin::root(), 2));

		// Verify all announcement deposits were released
		assert_eq!(hold_balance(&2, RuntimeHoldReason::Proxy(HoldReason::AnnouncementDeposit)), 0);
		assert_eq!(Balances::free_balance(2), initial_free + held_amount);
	});
}
