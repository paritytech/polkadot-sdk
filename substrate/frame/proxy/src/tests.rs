...
fn announcement_works() {
	new_test_ext().execute_with(|| {
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed_with_basic_filter(1), 3, ProxyType::Any, 1));
		...
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed_with_basic_filter(2), 3, ProxyType::Any, 1));
		assert_eq!(Balances::reserved_balance(3), 0);

		assert_ok!(Proxy::announce(RuntimeOrigin::signed_with_basic_filter(3), 1, [1; 32].into()));
		...
		assert_ok!(Proxy::announce(RuntimeOrigin::signed_with_basic_filter(3), 2, [2; 32].into()));
		let announcements = Announcements::<Test>::get(3);
		assert_eq!(
			announcements.0,
			vec![
		...
		assert_noop!(
			Proxy::announce(RuntimeOrigin::signed_with_basic_filter(3), 2, [3; 32].into()),
			Error::<Test>::TooMany
		);
	});
}
...
fn remove_announcement_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(Proxy::remove_announcement(RuntimeOrigin::signed_with_basic_filter(3), 1, [0; 32].into()), e);
		assert_ok!(Proxy::remove_announcement(RuntimeOrigin::signed_with_basic_filter(3), 1, [1; 32].into()));
		let announcements = Announcements::<Test>::get(3);
		assert_eq!(
			announcements.0,
...
fn reject_announcement_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Proxy::announce(RuntimeOrigin::signed_with_basic_filter(3), 2, [2; 32].into()));
		let e = Error::<Test>::NotFound;
		assert_noop!(Proxy::reject_announcement(RuntimeOrigin::signed_with_basic_filter(1), 3, [0; 32].into()), e);
		let e = Error::<Test>::NotFound;
		assert_noop!(Proxy::reject_announcement(RuntimeOrigin::signed_with_basic_filter(4), 3, [1; 32].into()), e);
		assert_ok!(Proxy::reject_announcement(RuntimeOrigin::signed_with_basic_filter(1), 3, [1; 32].into()));
		let announcements = Announcements::<Test>::get(3);
		assert_eq!(
			announcements.0,
			vec![Announcement { real: 2, call_hash: [2; 32].into(), height: 1 }]
...
fn announcer_must_be_proxy() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			Proxy::announce(RuntimeOrigin::signed_with_basic_filter(2), 1, H256::zero()),
			Error::<Test>::NotProxy
		);
	});
}
...
fn calling_proxy_doesnt_remove_announcement() {
	new_test_ext().execute_with(|| {
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed_with_basic_filter(1), 2, ProxyType::Any, 0));

		let call = Box::new(call_transfer(6, 1));
		let call_hash = BlakeTwo256::hash_of(&call);

		assert_ok!(Proxy::announce(RuntimeOrigin::signed_with_basic_filter(2), 1, call_hash));
		assert_ok!(Proxy::proxy(RuntimeOrigin::signed_with_basic_filter(2), 1, None, call));
...
fn delayed_requires_pre_announcement() {
	new_test_ext().execute_with(|| {
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed_with_basic_filter(1), 2, ProxyType::Any, 1));
		let call = Box::new(call_transfer(6, 1));
		let e = Error::<Test>::Unannounced;
		assert_noop!(Proxy::proxy(RuntimeOrigin::signed_with_basic_filter(2), 1, None, call.clone()), e);
		let e = Error::<Test>::Unannounced;
		assert_noop!(Proxy::proxy_announced(RuntimeOrigin::signed_with_basic_filter(0), 2, 1, None, call.clone()), e);
		let call_hash = BlakeTwo256::hash_of(&call);
		assert_ok!(Proxy::announce(RuntimeOrigin::signed_with_basic_filter(2), 1, call_hash));
		frame_system::Pallet::<Test>::set_block_number(2);
		assert_ok!(Proxy::proxy_announced(RuntimeOrigin::signed_with_basic_filter(0), 2, 1, None, call.clone()));
	});
}
...
fn proxy_announced_removes_announcement_and_returns_deposit() {
	new_test_ext().execute_with(|| {
		...
		let call = Box::new(call_transfer(6, 1));
		let call_hash = BlakeTwo256::hash_of(&call);
		assert_ok!(Proxy::announce(RuntimeOrigin::signed_with_basic_filter(3), 1, call_hash));
		assert_ok!(Proxy::announce(RuntimeOrigin::signed_with_basic_filter(3), 2, call_hash));
		// Too early to execute announced call
		let e = Error::<Test>::Unannounced;
		assert_noop!(Proxy::proxy_announced(RuntimeOrigin::signed_with_basic_filter(0), 3, 1, None, call.clone()), e);

		frame_system::Pallet::<Test>::set_block_number(2);
		assert_ok!(Proxy::proxy_announced(RuntimeOrigin::signed_with_basic_filter(0), 3, 1, None, call.clone()));
		let announcements = Announcements::<Test>::get(3);
...
fn filtering_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed_with_basic_filter(1), 2, ProxyType::Any, 0));
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed_with_basic_filter(1), 3, ProxyType::JustTransfer, 0));
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed_with_basic_filter(1), 4, ProxyType::JustUtility, 0));

		let call = Box::new(call_transfer(6, 1));
		assert_ok!(Proxy::proxy(RuntimeOrigin::signed_with_basic_filter(2), 1, None, call.clone()));
		System::assert_last_event(ProxyEvent::ProxyExecuted { result: Ok(()) }.into());
		assert_ok!(Proxy::proxy(RuntimeOrigin::signed_with_basic_filter(3), 1, None, call.clone()));
		System::assert_last_event(ProxyEvent::ProxyExecuted { result: Ok(()) }.into());
		assert_ok!(Proxy::proxy(RuntimeOrigin::signed_with_basic_filter(4), 1, None, call.clone()));
		System::assert_last_event(
...
fn add_remove_proxies_works() {
	new_test_ext().execute_with(|| {
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed_with_basic_filter(1), 2, ProxyType::Any, 0));
		assert_noop!(
			Proxy::add_proxy(RuntimeOrigin::signed_with_basic_filter(1), 2, ProxyType::Any, 0),
		...
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed_with_basic_filter(1), 3, ProxyType::Any, 0));
		assert_eq!(Balances::reserved_balance(1), 4);
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed_with_basic_filter(1), 4, ProxyType::JustUtility, 0));
		assert_eq!(Balances::reserved_balance(1), 5);
		assert_noop!(
			Proxy::add_proxy(RuntimeOrigin::signed_with_basic_filter(1), 4, ProxyType::Any, 0),
			Error::<Test>::TooMany
		);
		assert_noop!(
			Proxy::remove_proxy(RuntimeOrigin::signed_with_basic_filter(1), 3, ProxyType::JustTransfer, 0),
			Error::<Test>::NotFound
		);
		assert_ok!(Proxy::remove_proxy(RuntimeOrigin::signed_with_basic_filter(1), 4, ProxyType::JustUtility, 0));
		System::assert_last_event(
		...
		assert_eq!(Balances::reserved_balance(1), 4);
		assert_ok!(Proxy::remove_proxy(RuntimeOrigin::signed_with_basic_filter(1), 3, ProxyType::Any, 0));
		assert_eq!(Balances::reserved_balance(1), 3);
		System::assert_last_event(
			ProxyEvent::ProxyRemoved {
				delegator: 1,
		...
		assert_ok!(Proxy::remove_proxy(RuntimeOrigin::signed_with_basic_filter(1), 2, ProxyType::Any, 0));
		assert_eq!(Balances::reserved_balance(1), 2);
		System::assert_last_event(
			ProxyEvent::ProxyRemoved {
				delegator: 1,
		...
		assert_ok!(Proxy::remove_proxy(RuntimeOrigin::signed_with_basic_filter(1), 2, ProxyType::JustTransfer, 0));
		...
		assert_noop!(
			Proxy::add_proxy(RuntimeOrigin::signed_with_basic_filter(1), 1, ProxyType::Any, 0),
...
fn cannot_add_proxy_without_balance() {
	new_test_ext().execute_with(|| {
		assert_ok!(Proxy::add_proxy(RuntimeOrigin::signed_with_basic_filter(5), 3, ProxyType::Any, 0));
		assert_eq!(Balances::reserved_balance(5), 2);
		assert_noop!(
			Proxy::add_proxy(RuntimeOrigin::signed_with_basic_filter(5), 4, ProxyType::Any, 0),
...
fn proxying_works() {
	new_test_ext().execute_with(|| {
		...
		let call = Box::new(call_transfer(6, 1));
		assert_noop!(
			Proxy::proxy(RuntimeOrigin::signed_with_basic_filter(4), 1, None, call.clone()),
			Error::<Test>::NotProxy
		);
		assert_noop!(
			Proxy::proxy(RuntimeOrigin::signed_with_basic_filter(2), 1, Some(ProxyType::Any), call.clone()),
			Error::<Test>::NotProxy
		);
		assert_ok!(Proxy::proxy(RuntimeOrigin::signed_with_basic_filter(2), 1, None, call.clone()));
		System::assert_last_event(ProxyEvent::ProxyExecuted { result: Ok(()) }.into());
		...
		assert_ok!(RuntimeCall::Proxy(super::Call::new_call_variant_proxy(1, None, call.clone()))
			.dispatch(RuntimeOrigin::signed_with_basic_filter(2)));
		System::assert_last_event(
			ProxyEvent::ProxyExecuted { result: Err(SystemError::CallFiltered.into()) }.into(),
		);
		assert_ok!(Proxy::proxy(RuntimeOrigin::signed_with_basic_filter(3), 1, None, call.clone()));
...
fn pure_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Proxy::create_pure(RuntimeOrigin::signed_with_basic_filter(1), ProxyType::Any, 0, 0));
		...
		assert_ok!(Proxy::create_pure(RuntimeOrigin::signed_with_basic_filter(1), ProxyType::JustTransfer, 0, 0));
		assert_ok!(Proxy::create_pure(RuntimeOrigin::signed_with_basic_filter(1), ProxyType::Any, 0, 1));
		let anon2 = Proxy::pure_account(&2, &ProxyType::Any, 0, None);
		assert_ok!(Proxy::create_pure(RuntimeOrigin::signed_with_basic_filter(2), ProxyType::Any, 0, 0));
		assert_noop!(
			Proxy::create_pure(RuntimeOrigin::signed_with_basic_filter(1), ProxyType::Any, 0, 0),
			Error::<Test>::Duplicate
		);
		...
		let call = Box::new(call_transfer(6, 1));
		assert_ok!(Balances::transfer_allow_death(RuntimeOrigin::signed_with_basic_filter(3), anon, 5));
		assert_eq!(Balances::free_balance(6), 0);
		assert_ok!(Proxy::proxy(RuntimeOrigin::signed_with_basic_filter(1), anon, None, call));
		System::assert_last_event(ProxyEvent::ProxyExecuted { result: Ok(()) }.into());
		assert_eq!(Balances::free_balance(6), 1);
		...
		assert_ok!(Proxy::proxy(RuntimeOrigin::signed_with_basic_filter(2), anon2, None, call.clone()));
		...
		assert_noop!(
			Proxy::kill_pure(RuntimeOrigin::signed_with_basic_filter(1), 1, ProxyType::Any, 0, 1, 0),
			Error::<Test>::NoPermission
		);
		assert_eq!(Balances::free_balance(1), 1);
		assert_ok!(Proxy::proxy(RuntimeOrigin::signed_with_basic_filter(1), anon, None, call.clone()));
		assert_eq!(Balances::free_balance(1), 3);
		assert_noop!(
			Proxy::proxy(RuntimeOrigin::signed_with_basic_filter(1), anon, None, call.clone()),
			Error::<Test>::NotProxy
		);

		// Actually kill the pure proxy.
		assert_ok!(Proxy::kill_pure(RuntimeOrigin::signed_with_basic_filter(anon), 1, ProxyType::Any, 0, 1, 0));
		System::assert_last_event(
...
fn poke_deposit_works_for_proxy_deposits() {
	new_test_ext().execute_with(|| {
		...
		let result = Proxy::poke_deposit(RuntimeOrigin::signed_with_basic_filter(1));
...
fn poke_deposit_works_for_announcement_deposits() {
	new_test_ext().execute_with(|| {
		...
		let result = Proxy::poke_deposit(RuntimeOrigin::signed_with_basic_filter(3));
		assert_ok!(result.as_ref());
		assert_eq!(result.unwrap().pays_fee, Pays::No);
...
fn poke_deposit_handles_insufficient_balance() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Proxy::poke_deposit(RuntimeOrigin::signed_with_basic_filter(5)),
...
fn poke_deposit_updates_both_proxy_and_announcement_deposits() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Proxy::announce(RuntimeOrigin::signed_with_basic_filter(2), 1, [1; 32].into()));
		let announcements = Announcements::<Test>::get(2);
		assert_eq!(
			announcements.0,
			vec![Announcement { real: 1, call_hash: [1; 32].into(), height: 1 }]
		...
		let result = Proxy::poke_deposit(RuntimeOrigin::signed_with_basic_filter(2));
		assert_ok!(result.as_ref());
		assert_eq!(result.unwrap().pays_fee, Pays::No);
...
fn poke_deposit_fails_for_unsigned_origin() {
	new_test_ext().execute_with(|| {
		assert_noop!(Proxy::poke_deposit(RuntimeOrigin::none_with_basic_filter()), DispatchError::BadOrigin,);
	});
}
