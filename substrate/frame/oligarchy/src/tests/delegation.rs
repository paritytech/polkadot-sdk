...
fn single_proposal_should_work_with_delegation() {
	new_test_ext().execute_with(|| {
		...
		// Delegate first vote.
		assert_ok!(Oligarchy::delegate(RuntimeOrigin::signed(2), 1, Conviction::None, 20));
		let r = 0;
		...
		assert_ok!(Oligarchy::delegate(RuntimeOrigin::signed(3), 1, Conviction::None, 30));
		assert_eq!(tally(r), Tally { ayes: 6, nays: 0, turnout: 60 });

		// Reduce first vote.
		assert_ok!(Oligarchy::delegate(RuntimeOrigin::signed(2), 1, Conviction::None, 10));
		...
		assert_ok!(Oligarchy::delegate(RuntimeOrigin::signed(3), 2, Conviction::None, 30));
		...
		assert_ok!(Oligarchy::delegate(RuntimeOrigin::signed(2), 1, Conviction::Locked1x, 10));
		assert_eq!(tally(r), Tally { ayes: 0, nays: 0, turnout: 0 });
...
fn self_delegation_not_allowed() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			Oligarchy::delegate(RuntimeOrigin::signed(1), 1, Conviction::None, 10),
			Error::<Test>::Nonsense,
		);
	});
}
...
fn cyclic_delegation_should_unwind() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Oligarchy::delegate(RuntimeOrigin::signed(2), 1, Conviction::None, 20));
		assert_ok!(Oligarchy::delegate(RuntimeOrigin::signed(3), 2, Conviction::None, 30));
		assert_ok!(Oligarchy::delegate(RuntimeOrigin::signed(1), 3, Conviction::None, 10));
		let r = 0;
		assert_ok!(Oligarchy::undelegate(RuntimeOrigin::signed(3)));
		assert_ok!(Oligarchy::vote(RuntimeOrigin::signed(3), r, aye(3)));
		assert_ok!(Oligarchy::undelegate(RuntimeOrigin::signed(1)));
...
fn single_proposal_should_work_with_vote_and_delegation() {
	...
	new_test_ext().execute_with(|| {
		...
		let r = 0;
		assert_ok!(Oligarchy::vote(RuntimeOrigin::signed(1), r, aye(1)));
		assert_ok!(Oligarchy::vote(RuntimeOrigin::signed(2), r, nay(2)));
...
fn single_proposal_should_work_with_undelegation() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Oligarchy::delegate(RuntimeOrigin::signed(2), 1, Conviction::None, 20));
		assert_ok!(Oligarchy::undelegate(RuntimeOrigin::signed(2)));
...
fn conviction_should_be_honored_in_delegation() {
	...
	new_test_ext().execute_with(|| {
		let r = begin_referendum();
		// Delegate and vote.
		assert_ok!(Oligarchy::delegate(RuntimeOrigin::signed(2), 1, Conviction::Locked6x, 20));
		assert_ok!(Oligarchy::vote(RuntimeOrigin::signed(1), r, aye(1)));
...
fn redelegation_keeps_lock() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Oligarchy::delegate(RuntimeOrigin::signed(2), 3, Conviction::None, 10));
