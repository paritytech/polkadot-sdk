...
fn multisig_deposit_is_taken_and_returned() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Balances::transfer_allow_death(RuntimeOrigin::signed_with_basic_filter(1), multi, 5));
		assert_ok!(Balances::transfer_allow_death(RuntimeOrigin::signed_with_basic_filter(2), multi, 5));
		assert_ok!(Balances::transfer_allow_death(RuntimeOrigin::signed_with_basic_filter(3), multi, 5));
...
fn cancel_multisig_returns_deposit() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Multisig::cancel_as_multi(RuntimeOrigin::signed_with_basic_filter(1), 3, vec![2, 3], now(), hash));
...
fn cancel_multisig_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Multisig::cancel_as_multi(RuntimeOrigin::signed_with_basic_filter(2), 3, vec![1, 3], now(), hash),
			Error::<Test>::NotOwner,
		);
		assert_ok!(Multisig::cancel_as_multi(RuntimeOrigin::signed_with_basic_filter(1), 3, vec![2, 3], now(), hash),);
	});
...
fn multisig_filters() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Multisig::as_multi_threshold_1(RuntimeOrigin::signed_with_basic_filter(1), vec![2], call.clone()),
...
fn poke_deposit_fails_for_threshold_less_than_two() {
	new_test_ext().execute_with(|| {
		...
		let hash = blake2_256(&call.encode());
		assert_noop!(
			Multisig::poke_deposit(RuntimeOrigin::signed_with_basic_filter(1), 0, vec![2], hash),
			Error::<Test>::MinimumThreshold,
		);
		assert_noop!(
			Multisig::poke_deposit(RuntimeOrigin::signed_with_basic_filter(1), 1, vec![2], hash),
			Error::<Test>::MinimumThreshold,
		);
	});
}
...
fn poke_deposit_fails_for_too_incorrect_number_of_signatories() {
	new_test_ext().execute_with(|| {
		...
		let hash = blake2_256(&call.encode());
		assert_noop!(
			Multisig::poke_deposit(RuntimeOrigin::signed_with_basic_filter(1), 2, vec![], hash),
			Error::<Test>::TooFewSignatories,
		);
		assert_noop!(
			Multisig::poke_deposit(RuntimeOrigin::signed_with_basic_filter(1), 2, vec![2, 3, 4, 5], hash),
			Error::<Test>::TooManySignatories,
		);
	});
}
...
fn poke_deposit_fails_for_non_existent_multisig() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			Multisig::poke_deposit(RuntimeOrigin::signed_with_basic_filter(1), 2, vec![2, 3], [0; 32]),
			Error::<Test>::NotFound
		);
	});
}
...
fn poke_deposit_charges_fee_when_deposit_unchanged() {
	new_test_ext().execute_with(|| {
		...
		let result =
			Multisig::poke_deposit(RuntimeOrigin::signed_with_basic_filter(1), threshold, other_signatories, hash);
...
fn poke_deposit_handles_insufficient_balance() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Multisig::poke_deposit(RuntimeOrigin::signed_with_basic_filter(4), threshold, other_signatories, hash),
