...
use crate::{
	migrations::{
		v2,
		v2::{weights, weights::WeightInfo as _},
	},
	mock::{
...
fn lazy_migration_works() {
	new_test_ext().execute_with(|| {
		...
		for i in 0..1024 {
			let multi_account = multi_account_id(&[i, 2, 3][..], 2);
			let call = call_transfer(6, i).encode();
			let hash = blake2_256(&call);
			let val = OldMultisig { when: now(), deposit: 100, depositor: i, approvals: &[i] };
		...
		for i in 0..1024 {
			assert_eq!(crate::MyMap::<T>::get(i).unwrap().maybe_when, Some(now()));
		}
	});
}
