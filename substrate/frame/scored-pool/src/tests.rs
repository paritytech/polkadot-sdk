...
fn submit_candidacy_must_not_work() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			ScoredPool::submit_candidacy(RuntimeOrigin::signed_with_basic_filter(99)),
			pallet_balances::Error::<Test, _>::InsufficientBalance,
		);
		assert_noop!(
			ScoredPool::submit_candidacy(RuntimeOrigin::signed_with_basic_filter(40)),
...
fn submit_candidacy_works() {
	new_test_ext().execute_with(|| {
		...
		// when
		assert_ok!(ScoredPool::submit_candidacy(RuntimeOrigin::signed_with_basic_filter(who)));
...
fn scoring_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ScoredPool::score(RuntimeOrigin::signed_with_basic_filter(ScoreOrigin::get()), who, index, score));
...
fn kicking_works_only_for_authorized() {
	new_test_ext().execute_with(|| {
		let who = 40;
		let index = find_in_pool(who).expect("entity must be in pool") as u32;
		assert_noop!(ScoredPool::kick(RuntimeOrigin::signed_with_basic_filter(99), who, index), BadOrigin);
	});
}
...
fn kicking_works() {
	new_test_ext().execute_with(|| {
		...
		// when
		let index = find_in_pool(who).expect("entity must be in pool") as u32;
		assert_ok!(ScoredPool::kick(RuntimeOrigin::signed_with_basic_filter(KickOrigin::get()), who, index));
...
fn unscored_entities_must_not_be_used_for_filling_members() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ScoredPool::submit_candidacy(RuntimeOrigin::signed_with_basic_filter(15)));
...
fn refreshing_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ScoredPool::submit_candidacy(RuntimeOrigin::signed_with_basic_filter(who)));
		let index = find_in_pool(who).expect("entity must be in pool") as u32;
		assert_ok!(ScoredPool::score(RuntimeOrigin::signed_with_basic_filter(ScoreOrigin::get()), who, index, 99));
...
fn refreshing_happens_every_period() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(ScoredPool::score(RuntimeOrigin::signed_with_basic_filter(ScoreOrigin::get()), 15, index, 99));
		assert_eq!(ScoredPool::members(), vec![20, 40]);
...
fn withdraw_candidacy_must_only_work_for_members() {
	new_test_ext().execute_with(|| {
		...
		let index = 0;
		assert_noop!(
			ScoredPool::withdraw_candidacy(RuntimeOrigin::signed_with_basic_filter(who), index),
...
fn oob_index_should_abort() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			ScoredPool::withdraw_candidacy(RuntimeOrigin::signed_with_basic_filter(who), oob_index),
			Error::<Test, _>::InvalidIndex
		);
		assert_noop!(
			ScoredPool::score(RuntimeOrigin::signed_with_basic_filter(ScoreOrigin::get()), who, oob_index, 99),
			Error::<Test, _>::InvalidIndex
		);
		assert_noop!(
			ScoredPool::kick(RuntimeOrigin::signed_with_basic_filter(KickOrigin::get()), who, oob_index),
			Error::<Test, _>::InvalidIndex
		);
	});
}
...
fn index_mismatches_should_abort() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			ScoredPool::score(RuntimeOrigin::signed_with_basic_filter(ScoreOrigin::get()), who, index, 99),
			Error::<Test, _>::WrongAccountIndex
		);
		assert_noop!(
			ScoredPool::kick(RuntimeOrigin::signed_with_basic_filter(KickOrigin::get()), who, index),
			Error::<Test, _>::WrongAccountIndex
		);
	});
}
...
fn withdraw_unscored_candidacy_must_work() {
	new_test_ext().execute_with(|| {
		...
		// when
		let index = find_in_pool(who).expect("entity must be in pool") as u32;
		assert_ok!(ScoredPool::withdraw_candidacy(RuntimeOrigin::signed_with_basic_filter(who), index));
	...
	new_test_ext().execute_with(|| {
		for i in [1, 2, 3, 4, 6] {
			...
			assert_ok!(ScoredPool::score(
				RuntimeOrigin::signed_with_basic_filter(ScoreOrigin::get()),
				who,
				index,
				99
			));
		...
		assert_noop!(
			ScoredPool::submit_candidacy(RuntimeOrigin::signed_with_basic_filter(8)),
