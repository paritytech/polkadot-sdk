...
fn existential_weight_async<T: pallet_staking_async::Config>(
	total_issuance: u128,
	minimum_balance: u128,
) -> VoteWeight {
	use sp_staking::currency_to_vote::CurrencyToVote;
...
/// Inner implementation shared by [`generate_thresholds`] and [`generate_thresholds_async`].
fn generate_thresholds_inner<T: frame_system::Config>(
	n_bags: usize,
	output: &Path,
	total_issuance: u128,
	minimum_balance: u128,
...
pub fn generate_thresholds<T: pallet_staking::Config>(
	...
	let ew = existential_weight::<T>(total_issuance, minimum_balance);
	generate_thresholds_inner::<T>(n_bags, output, total_issuance, minimum_balance, ew)
}
...
pub fn generate_thresholds_async<T: pallet_staking_async::Config>(
	...
	let ew = existential_weight_async::<T>(total_issuance, minimum_balance);
	generate_thresholds_inner::<T>(n_bags, output, total_issuance, minimum_balance, ew)
}
