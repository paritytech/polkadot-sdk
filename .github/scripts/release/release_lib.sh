...
upload_s3_release() {
  ...
  aws s3 ls "s3://releases.parity.io/$product/${version}/" --recursive --human-readable --summarize || true
  echo "Content to be uploaded:"
  artifacts="artifacts/$product/"
  ls "$artifacts"
  aws s3 sync --acl public-read "$artifacts" "s3://releases.parity.io/$product/${version}/"
  echo "Uploaded files:"
  aws s3 ls "s3://releases.parity.io/$product/${version}/" --recursive --human-readable --summarize
  echo "✅ The release should be at https://releases.parity.io/$product/${version}"
}
