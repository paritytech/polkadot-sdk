#!/usr/bin/env bash
# Register pair 0 (DOT/USD) and set its endpoints on a running zombienet,
# wrapped in sudo. On westend-local the sudo key is Alice (//Alice).
#
...
polkadot-js-api --ws "$WS_URL" --seed "$SUDO_SEED" --sudo \
  tx.priceOracle.setActiveEndpoints "$PAIR_ID" "$ENDPOINTS"
