...
fn query_trusted_api_works() {
    ...
    sp_io::TestExternalities::default().execute_with(|| {
        ...
        let test_cases: Vec<TestCase> = vec![
            ...
            // 	asset: Asset{ id: AssetId(Location::new(11, [GeneralIndex(3212122222), PalletInstance(50)]).into()), fun: Fungible(100) }.into(),
            // 	location: Location::new(11, [GeneralIndex(3212122222), PalletInstance(50)]).into(),
            // 	expected: Err(Error::VersionedLocationConversionFailed),
            // },
        ];

        for tc in test_cases {
            let res = runtime_api.is_trusted_reserve(H256::zero(), tc.asset.clone(), tc.location.clone());
...
fn query_trusted_teleporter_works() {
    ...
    sp_io::TestExternalities::default().execute_with(|| {
        ...
        for tc in test_cases {
            let res = runtime_api.is_trusted_teleporter(H256::zero(), tc.asset.clone(), tc.location.clone());
            let inner_res = res.unwrap_or_else(|e| {
                panic!("Test case '{}' failed with ApiError: {:?}", tc.name, e);
            });
