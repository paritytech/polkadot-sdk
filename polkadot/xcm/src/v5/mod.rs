...
pub enum Instruction<Call> {
	...
	/// Publish data to the relay chain for other parachains to subscribe to.
	///
	/// This instruction allows parachains to publish key-value data pairs to the relay chain
	/// which can then be subscribed to by other parachains through the Subscribe instruction.
	/// The data is stored in child tries on the relay chain indexed by the publisher's ParaId.
	///
	...
	/// - Overflow: If data exceeds bounds
	Publish {
		data: PublishData
	},
	...
	/// Errors:
	/// - BadOrigin: If origin is not a valid parachain
	/// - TooManySubscriptions: If subscriber has reached maximum subscription limit
