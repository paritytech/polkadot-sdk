...
#[test]
fn computed_origin_works_with_execute_with_origin() {
	let mut message = Xcm::<()>(vec![
		ExecuteWithOrigin {
			origin: Some(AccountId32 { id: [0u8; 32], network: None }.into()),
			xcm: Xcm::<()>(vec![
	...
	assert_eq!(should_execute, Ok(()));

	let mut new_message = Xcm::<()>(vec![
	...
	>::should_execute(
		&Parent.into(),
		invalid_message.inner_mut(),
		Weight::from_parts(100, 100),
		&mut props(Weight::zero()),
	);
	assert_eq!(should_execute, Err(ProcessMessageError::BadFormat));
}
