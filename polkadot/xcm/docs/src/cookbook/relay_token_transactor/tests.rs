...
use super::{
    network::{MockNet, Relay, ParaA, ALICE, BOB, CENTS, INITIAL_BALANCE},
    relay_chain,
    parachain,
};
...
fn reserve_asset_transfers_work() {
    ...
    Relay::execute_with(|| {
        ...
        let beneficiary: MultiLocation = AccountId32 { id: BOB.clone().into(), network: Some(NetworkId::Polkadot) }.into();
    ...
    ParaA::execute_with(|| {
        ...
        let beneficiary: MultiLocation = AccountId32 { id: ALICE.clone().into(), network: Some(NetworkId::Polkadot) }.into();
    ...
    Relay::execute_with(|| {
        assert_eq!(relay_chain::Balances::free_balance(&ALICE), INITIAL_BALANCE - 50 * CENTS + 25 * CENTS);
    });
}
