...
/// Test for Emitting Events on `DispatchError`
#[test]
fn emit_event_on_dispatch_error() {
	let balances = vec![
		(ALICE, INITIAL_BALANCE),
		(ParaId::from(OTHER_PARA_ID).into_account_truncating(), INITIAL_BALANCE),
	];
	new_test_ext_with_balances(balances).execute_with(|| {
		...
		let message = Xcm(vec![
			ReserveAssetDeposited((Parent, SEND_AMOUNT).into()),
			// Introduce an error by using an invalid instruction
			WithdrawAsset((Parent, 0).into()), // Invalid amount
			DepositAsset { assets: AllCounted(1).into(), beneficiary: sender.clone() },
		]);
		...
		// Ensure the send fails
		assert_noop!(result, crate::Error::<Test>::SendFailure);

		// Check if the error event is emitted
		let events = System::events();
		assert!(
			events.iter().any(|record| matches!(
				record.event,
				RuntimeEvent::XcmPallet(crate::Event::Failed { .. })
			)),
			"Expected a Failed event to be emitted"
		);
	});
}

/// Test for Emitting Events on Query Response
#[test]
fn emit_event_on_query_response() {
	let balances = vec![
		(ALICE, INITIAL_BALANCE),
		(ParaId::from(OTHER_PARA_ID).into_account_truncating(), INITIAL_BALANCE),
	];
	new_test_ext_with_balances(balances).execute_with(|| {
		...
		// Check if the response event is emitted
		let events = System::events();
		assert!(
			events.iter().any(|record| matches!(
				record.event,
			...
			"Expected a ResponseReady event to be emitted"
		);
	});
}
