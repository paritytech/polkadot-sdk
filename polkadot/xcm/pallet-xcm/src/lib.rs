...
impl<T: Config> Pallet<T> {
	...
	pub fn do_try_state() -> Result<(), TryRuntimeError> {
		...
		// Validate that SafeXcmVersion is set correctly according to the XCM version support rule
		// Only validate this when there's no active migration (i.e., in a stable state)
		if !CurrentMigration::<T>::exists() {
			let latest = CurrentXcmVersion::get();
			let expected_safe_version = latest.saturating_sub(migration::SUPPORTED_VERSIONS_COUNT);
			let stored_safe_version = SafeXcmVersion::<T>::get();

			// SafeXcmVersion should be set to latest - SUPPORTED_VERSIONS_COUNT
			ensure!(
				stored_safe_version == Some(expected_safe_version),
				TryRuntimeError::Other(
					"SafeXcmVersion must be set to latest - SUPPORTED_VERSIONS_COUNT"
				)
			);
		}
