...
impl ClaimQueueState {
	...
	pub fn get_pending_at(&self, relay_parent: &Hash) -> VecDeque<ParaId> {
	...
	/// TODO
	pub fn get_all_for_para_at(&self, relay_parent: &Hash, para_id: &ParaId) -> usize {
		let window = self.get_window(relay_parent);
	...
	pub fn all_assignments<'a>(&'a self) -> impl Iterator<Item = &'a ParaId> + 'a {
		self.block_state
			.iter()
	...
	pub fn free_slots(&self) -> Vec<ParaId> {
		self.block_state
			.iter()
