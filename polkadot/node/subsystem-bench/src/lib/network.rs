...
use polkadot_node_network_protocol::{self as net_protocol, VersionedValidation};
...
impl NetworkMessage {
	...
	pub fn size(&self) -> usize {
		match &self {
			NetworkMessage::MessageFromPeer(_, VersionedValidation::V3(message)) =>
				message.encoded_size(),
			NetworkMessage::MessageFromNode(_peer_id, VersionedValidation::V3(message)) =>
				message.encoded_size(),
