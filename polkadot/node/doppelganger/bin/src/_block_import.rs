...
use sc_consensus::block_import::{BlockImport, BlockCheckParams, ImportResult, BlockImportParams};
use log::debug;
	...
	BI: BlockImport<Block> + Send + Sync,
{
	...
	async fn import_block(
        ...
        if block_params.with_state() {
			debug!(target: LOG_TARGET, "Block param wirh state, with header {:?}", block_params.header);
			if let StateAction::ApplyChanges(sc_consensus::StorageChanges::Import(ref mut imported_state)) = block.state_action {
				if std::env::var("ZOMBIE_OVERRIDE").is_ok() {
					...
					let overrides = get_overrides();
					{
						...
						let mut trie = TrieDBMutBuilderV1::<HashingFor<Block>>::new(&mut mdb, &mut state_root).build();

						for key_value_storage_level in imported_state.state.0.iter_mut() {
							println!("state_root: {:?}", key_value_storage_level.state_root);
							println!("parent_storage_keys: {:?}", key_value_storage_level.parent_storage_keys);


							for kv in key_value_storage_level.key_values.iter_mut() {

								if let Some (override_value) = overrides.get(&kv.0) {
									println!("Overriding key: {}", sp_core::hexdisplay::HexDisplay::from(&kv.0));
									println!("old value: {}", sp_core::hexdisplay::HexDisplay::from(&kv.1));
									kv.1 = override_value.clone();
									println!("new value: {}", sp_core::hexdisplay::HexDisplay::from(&kv.1));
								}

								trie.insert(&kv.0, &kv.1)
								.expect("TrieMut::insert should not fail");
							}
						}
					}
        ...
        return self.inner.import_block(block_params).await
    }
}
