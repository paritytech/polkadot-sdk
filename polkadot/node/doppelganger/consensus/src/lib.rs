...
use sp_runtime::traits::{Block as BlockT, Header, PhantomData, HashingFor};
	...
	async fn import_block(
		...
		if block.with_state() {
			...
			)) = block.state_action
			{
				...
				let executor: WasmExecutor<(cumulus_primitives_proof_size_hostfunction::storage_proof_size::HostFunctions,
					sp_io::SubstrateHostFunctions,)> = WasmExecutor::builder().build();
			...
			if let Ok(zombie_info_path) = env::var("ZOMBIE_INFO_PATH") {
				let err_msg = format!("write to 'ZOMBIE_INFO_PATH'= {zombie_info_path} should work");
				tokio::fs::write(zombie_info_path, format!("{number}")).await.expect(&err_msg);
			} else {
				// send to stdout
				io::stdout().write_all(&format!("{number}").as_bytes()).expect("write to stdout should work");
			}
...
async fn get_overrides() -> OverrideKeys {
	let override_keys: OverrideKeys = if let Ok(overrides_path) = std::env::var(format!("ZOMBIE_RC_OVERRIDES_PATH")) {
...
async fn get_overrides_para(_id: u32) -> OverrideKeys {
	let override_keys: OverrideKeys = if let Ok(overrides_path) = std::env::var(format!("ZOMBIE_PARA_OVERRIDES_PATH")) {
		let content: HashMap<String, HashMap<String, String>> = serde_json::from_str(&tokio::fs::read_to_string(&overrides_path).await.expect(&format!("Overrides path 'ZOMBIE_PARA_OVERRIDES_PATH' ({overrides_path}) should be valid. qed"))).expect("Should be a valid json");
		let mut overrides: HashMap<Vec<u8>, Vec<u8>> = Default::default();
		let mut injects: HashMap<Vec<u8>, Vec<u8>> = Default::default();
...
fn subtract_free_balance_from_state(data: &Vec<u8>, amount:u128) -> Vec<u8> {
	let mut account_info = AccountInfo::<u32, AccountData<u128>>::decode(&mut data.as_slice()).unwrap();
...
mod tests {
	...
	fn account_info_should_works() {
        ...
        let  account: AccountInfo<u32, AccountData<u128>> = AccountInfo::<u32, AccountData<u128>>::decode(&mut data.as_slice()).unwrap();
        println!("{account:?}");
		let new_account_data = subtract_free_balance_from_state(&data, 10_000_000_000_000_u128);
		let  new_account: AccountInfo<u32, AccountData<u128>> = AccountInfo::<u32, AccountData<u128>>::decode(&mut new_account_data.as_slice()).unwrap();
		println!("{new_account:?}");
	}
