	...
	Network: sc_network::NetworkBackend<Block, <Block as BlockT>::Hash>,
{
	...
	pub fn build(self) -> Result<NewFull, Error> {
		...
		let warp_sync_config_to_use = if let Ok(zombie_target_header_path) =
			std::env::var("ZOMBIE_TARGET_HEADER_PATH")
		{
			let header_content = std::fs::read_to_string(&zombie_target_header_path)
				.expect("env var ZOMBIE_TARGET_HEADER_PATH should be valid. qed");
