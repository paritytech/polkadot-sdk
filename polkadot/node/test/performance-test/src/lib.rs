...
pub fn measure_pvf_prepare(wasm_code: &[u8]) -> Result<Duration, PerfCheckError> {
	...
	let blob = polkadot_node_core_pvf_common::executor_intf::prevalidate(code.as_ref())
		.map_err(PerfCheckError::from)?;
	polkadot_node_core_pvf_common::executor_intf::prepare(blob, &ExecutorParams::default())
		.map_err(PerfCheckError::from)?;
