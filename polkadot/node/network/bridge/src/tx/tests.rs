...
use polkadot_node_network_protocol::{
	...
	v1 as protocol_v1, v3 as protocol_v3, ObservedRole, VersionedCollation, VersionedValidation,
};
			...
			virtual_overseer
				.send(FromOrchestra::Communication {
					msg: NetworkBridgeTxMessage::SendValidationMessage(
						vec![peer],
						VersionedValidation::V3(message_v1.clone()),
					),
				})
				.timeout(TIMEOUT)
				.await
			...
			virtual_overseer
				.send(FromOrchestra::Communication {
					msg: NetworkBridgeTxMessage::SendCollationMessage(
						vec![peer],
						VersionedCollation::V1(message_v1.clone()),
					),
				})
				.await;
