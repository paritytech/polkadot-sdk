...
async fn connect_and_declare_collator(
	...
	let wire_message = match version {
		CollationVersion::V1 =>
			VersionedCollation::V1(protocol_v1::CollatorProtocolMessage::Declare(
				collator.public(),
				para_id,
				collator.sign(&protocol_v1::declare_signature_payload(&peer)),
			)),
		CollationVersion::V2 =>
			VersionedCollation::V2(protocol_v2::CollatorProtocolMessage::Declare(
				collator.public(),
				para_id,
				collator.sign(&protocol_v1::declare_signature_payload(&peer)),
			)),
...
async fn advertise_collation(
	...
	let wire_message = match candidate {
		Some((candidate_hash, parent_head_data_hash)) =>
			VersionedCollation::V2(protocol_v2::CollatorProtocolMessage::AdvertiseCollation {
				relay_parent,
				candidate_hash,
				parent_head_data_hash,
			}),
		None => VersionedCollation::V1(protocol_v1::CollatorProtocolMessage::AdvertiseCollation(
			relay_parent,
		)),
	};
	overseer_send(
