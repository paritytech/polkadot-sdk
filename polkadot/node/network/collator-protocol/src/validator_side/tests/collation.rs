...
fn cant_add_more_than_claim_queue() {
	...
	let assignments = vec![para_a.clone(), para_b.clone(), para_a.clone()];
	...
	assert!(!collations.is_collations_limit_reached(relay_parent_mode, para_a.clone(), 0));
	collations.note_fetched(para_a.clone());
	// and `para_b` is not affected
	assert!(!collations.is_collations_limit_reached(relay_parent_mode, para_b.clone(), 0));
	...
	// one fetch for b
	assert!(!collations.is_collations_limit_reached(relay_parent_mode, para_b.clone(), 0));
	collations.note_fetched(para_b.clone());
...
fn pending_fetches_are_counted() {
	...
	// first collation for `para_a` is in the limit
	assert!(!collations.is_collations_limit_reached(relay_parent_mode, para_a.clone(), 1));
	collations.note_fetched(para_a.clone());
