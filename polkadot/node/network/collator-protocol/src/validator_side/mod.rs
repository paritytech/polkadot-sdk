...
async fn notify_collation_seconded(
	...
	let wire_message = match version {
		CollationVersion::V1 =>
			VersionedCollation::V1(protocol_v1::CollationProtocol::CollatorProtocol(
...
async fn process_incoming_peer_message<Context>(
	...
	match msg {
		VersionedCollation::V1(V1::Declare(collator_id, para_id, signature)) |
		VersionedCollation::V2(V2::Declare(collator_id, para_id, signature)) => {
		...
		VersionedCollation::V1(V1::AdvertiseCollation(relay_parent)) =>
			if let Err(err) =
		...
		VersionedCollation::V1(V1::CollationSeconded(..)) |
		VersionedCollation::V2(V2::CollationSeconded(..)) => {
			gum::warn!(
				target: LOG_TARGET,
				peer_id = ?origin,
