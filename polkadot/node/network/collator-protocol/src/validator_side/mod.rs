...
impl State {
	...
	async fn handle_our_view_change<Sender: CollatorProtocolSenderTrait>(
		...
		// TODO: we need to extract bumps from the new leaves
		self.peer_manager.process_bumps(vec![]);

		let maybe_disconnected_peers =
			self.peer_manager.scheduled_paras_update(self.collation_manager.assignments());
	...
	fn handle_connected(&mut self, peer_id: PeerId) {
		let disconnected = self.peer_manager.try_accept(peer_id);
