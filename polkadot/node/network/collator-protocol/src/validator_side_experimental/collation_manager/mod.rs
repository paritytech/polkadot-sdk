...
use crate::{
	...
	validator_side_experimental::{
		common::{
			...
			PeerAdvertisement, ProspectiveCandidate, Score, SecondingRejectionInfo,
			FAILED_FETCH_SLASH,
		},
...
/// Decides which advertised collations to fetch, drives those fetches, and tracks what has
/// been fetched so far.
///
/// Fetch selection is reputation-driven ([`Self::pick_best_advertisement`]); a candidate hash
/// only becomes authoritative once its collation is fetched and verified
/// ([`PerSchedulingParent::fetched_collations`]).
pub struct CollationManager {
	...
	// A fetched collation blocked from seconding because we don't have its parent's head
	// data. Seconding needs it; normally prospective-parachains supplies it, but it can't until
	// the parent is itself seconded. We keep the collation here, tagged with the parent it
	// waits on (`(para_id, parent_head_data_hash)`), and retry once a candidate with that
	// output head gets seconded.
	//
	// Only pre-elastic-scaling collators reach this path. Elastic-scaling collators send the
	// parent head data inline (`CollationWithParentHeadData`), so we second them immediately.
	// Remove this once those older collators are unsupported.
	//
	// At most one blocked collation is kept, and it must not count towards claim-queue
	// capacity (see `set_blocked_collation`). One suffices: a pre-elastic-scaling collator
	// produces one candidate per relay parent (~6s apart), so under normal operation the
	// blocked collation unblocks long before a second would arrive.
	blocked_collation: Option<(BlockedCollationId, FetchedCollation)>,

	// State per scheduling parent: our group's assigned core there, the peer advertisements
	// seen, and the collations already fetched.
	...
	// Clock for time reads (V3 scheduling-parent slot validation).
	clock: Arc<dyn Clock>,
...
impl CollationManager {
	...
	pub async fn update_view<Sender: CollatorProtocolSenderTrait>(
		...
		self.per_scheduling_parent = std::mem::take(&mut self.per_scheduling_parent)
			...
			.filter_map(|(sp, per_sp)| {
				...
				for peer_adv in per_sp.all_advertisements() {
		...
		// Drop the blocked collation if its scheduling parent is no longer tracked.
		if let Some((_, c)) = &self.blocked_collation {
			if !self.per_scheduling_parent.contains_key(&c.scheduling_parent()) {
				self.blocked_collation = None;
			}
		}
	...
	/// right away is fine — it stays kept in `peer_advertisements` until a claim-queue slot
	/// opens up.
	...
	pub async fn try_accept_advertisement<Sender: CollatorProtocolSenderTrait>(
		...
		if let Some(ProspectiveCandidate { candidate_hash, .. }) = prospective_candidate {
			// A candidate we already hold (fetched and verified) needs no further copies: the
			// same candidate yields the same outcome, so a re-fetch is never useful once it is
			// recorded. Rejecting here also spares the `CanSecond` round trip below.
			if per_sp.fetched_collations.contains_key(&candidate_hash) {
				return Err(AdvertisementError::Duplicate);
			}
		}

		// We don't dedup against *in-flight* fetches here — the fetch planner does that.
		// Keeping a duplicate of an in-flight candidate is cheap and gives us a fallback if
		// that fetch fails. The peer rate-limit (`accept_attempts`) counts every advertisement
		// attempt regardless, so peers can't spam past their cap.
		...
		per_sp.add_advertisement(peer_adv);
	...
	>(
		...
		for lc_idx in 0..leaf_core_cqs.len() {
			...
			for idx in (0..cq_len).rev() {
				...
				let Some(peer_adv) =
					self.pick_best_advertisement(para_id, candidate_sps, &connected_rep_query_fn)
				else {
					continue;
				};
				...
				let req = self.fetching.launch(&peer_adv, create_timer_fn());
				requests.push(req);
				...
				for lc in leaf_core_cqs.iter_mut() {
					lc.reserve_slot(&peer_adv.scheduling_parent(), para_id);
				}
			}
		}
	...
	pub async fn note_fetched<Sender: CollatorProtocolSenderTrait>(
		...
		let peer_adv = res.0;
		let mut reject_info = SecondingRejectionInfo::from(&peer_adv);

		self.fetching.note_completed(&peer_adv.advertisement);

		let Some(per_sp) = self.per_scheduling_parent.get_mut(&peer_adv.scheduling_parent()) else {
			gum::debug!(
				target: LOG_TARGET,
				hash = ?peer_adv.scheduling_parent(),
		...
		per_sp.remove_advertisement(&peer_adv);
		...
		match process_collation_fetch_result(
			...
			Ok(fetched_collation) => {
				...
				// Populate the rejection info now that the candidate hash is known (for logging).
	...
	/// Frees the slot consumed by a previously-fetched candidate. Called when backing reports
	/// it invalid. After this, capacity at `scheduling_parent` for `para_id` increases by one.
	/// Returns the peer id of the fetcher if the slot was actually held.
	///
	/// Note: A blocked collation is never recorded (see `can_begin_seconding`), so there is nothing
	/// to release for it; it leaves `blocked_collation` on unblock or when its scheduling
	/// parent leaves view.
	pub fn release_slot(
		&mut self,
	...
	pub async fn note_seconded<Sender: CollatorProtocolSenderTrait>(
		...
		let Some(fetched_collation) = self.take_blocked_collation(&BlockedCollationId {
			para_id: *para_id,
		...
		let Some(per_sp) = self.per_scheduling_parent.get(&fetched_collation.scheduling_parent())
		else {
			return (peer_id, vec![]);
		};
		let can_second = self
		...
		(peer_id, vec![can_second])
	}

	/// Peer-advertisements at `sp` for `para_id` that are not yet fetched and not currently
	/// being fetched.
	fn eligible_advertisements<'a>(
		&'a self,
		sp: Hash,
		para_id: ParaId,
	) -> impl Iterator<Item = PeerAdvertisement> + 'a {
		...
		// V1 ads carry no candidate hash. Once we've fetched *any* V1 collation for this
		// `(sp, para)`, we don't need another — `fetched_collations` is keyed by the receipt
		// hash, so we scan its values for this para. (V1 in-flight dedup is handled by the
		// `Advertisement`-keyed `fetching.contains` below: the V1 `Advertisement` is the
		// same struct for every co-advertiser at one `(sp, para)`, so one in-flight fetch
		// blocks them all.)
		let v1_already_fetched =
			per_sp.fetched_collations.values().any(|info| info.para_id == para_id);

		let fetching = &self.fetching;
		Either::Right(per_sp.all_advertisements().filter(move |peer_adv| {
			if peer_adv.para_id() != para_id {
				return false;
			}
			// In-flight dedup. `Advertisement`-keyed: if any peer's fetch for this
			// `Advertisement` is in flight, every co-advertiser's ad is filtered out here.
			if fetching.contains(&peer_adv.advertisement) {
				return false;
			}
			match peer_adv.advertisement.prospective_candidate {
	...
	/// Picks the best (= highest-scored) peer-advertisement for `para_id` among `candidate_sps`.
	///
	/// When several peers advertise the same `Advertisement`, this is also where reputation
	/// arbitration happens: each `PeerAdvertisement` contributes one entry to the candidate
	/// set, and the rep-best wins (subject to the global ordering).
	fn pick_best_advertisement<RepQueryFn: Fn(&PeerId, &ParaId) -> Option<Score>>(
		...
		let advertisements: BTreeSet<AcceptedAdvertisement> = candidate_sps
			.flat_map(|sp| {
				self.eligible_advertisements(sp, para_id).filter_map(|peer_adv| {
					Some(AcceptedAdvertisement {
						score: connected_rep_query_fn(&peer_adv.peer_id, &peer_adv.para_id())?,
						peer_adv,
					})
				})
			})
		...
		advertisements.first().map(|best| best.peer_adv)
	}
	...
	///
	/// `may_block` is `true` on the initial fetch and `false` on an unblock retry (so a
	/// still-blocked retry isn't kept again — an unblock retry is one-shot).
	async fn can_begin_seconding<Sender: CollatorProtocolSenderTrait>(
		...
		// Another copy of this candidate was already verified and recorded (a concurrent fetch
		// of the same candidate from a different carrier). The first verified copy is the one
		// dispatched to backing, and its fetcher is the one credited or slashed; a later copy
		// is dropped, unslashed, without touching the record.
		if self
			.per_scheduling_parent
			.get(&scheduling_parent)
			.map_or(false, |per_sp| per_sp.fetched_collations.contains_key(&candidate_hash))
		{
			gum::debug!(
				target: LOG_TARGET,
				?candidate_hash,
				...
				?para_id,
				peer_id = ?fetched_collation.peer_id,
				"Candidate already fetched and verified from another peer; dropping this copy",
			);
			return CanSecond::No(None, reject_info);
		}
		...
		.await
		{
			...
			Err(SecondingError::BlockedOnParent(parent)) => {
				...
				// Keep the collation to retry once its parent is seconded. A blocked collation
				// is unverified (we could not reconstruct, let alone check, its PVD) and may
				// never become seconding-able, so it is NOT recorded (unlike the `Ok` arm): it
				// must not consume claim-queue capacity, or a peer could withhold parent head
				// data to occupy capacity it never fills.
				//
				// `may_block` is false on the unblock retry — a retry that is still blocked is
				// dropped rather than kept again.
				if may_block {
					self.set_blocked_collation(parent, fetched_collation);
				}
	...
	/// Keep a blocked collation (its parent isn't seconded, so its PVD can't be
	/// reconstructed) to retry once the parent is seconded.
	///
	/// At most one is kept: if one is already blocked the new collation is dropped (no slash —
	/// keeping it is only a re-fetch optimization). See the `blocked_collation` field docs
	/// for why this legacy path exists and why one suffices.
	fn set_blocked_collation(&mut self, parent_head_data_hash: Hash, collation: FetchedCollation) {
		if self.blocked_collation.is_some() {
			gum::debug!(
				target: LOG_TARGET,
				?parent_head_data_hash,
				para_id = ?collation.candidate_receipt.descriptor.para_id(),
				candidate_hash = ?collation.candidate_receipt.hash(),
				"Already holding a blocked collation; dropping this one (will rely on re-fetch)",
			);
			return;
		}
		let id = BlockedCollationId {
			para_id: collation.candidate_receipt.descriptor.para_id(),
			parent_head_data_hash,
		};
		self.blocked_collation = Some((id, collation));
	}

	/// Take the blocked collation if it is the one waiting on `id`.
	fn take_blocked_collation(&mut self, id: &BlockedCollationId) -> Option<FetchedCollation> {
		match &self.blocked_collation {
			Some((blocked_id, _)) if blocked_id == id => {
				self.blocked_collation.take().map(|(_, c)| c)
			},
			_ => None,
		}
	}
	...
	/// credit if it is later included, or slash if it turns out invalid. The first verified
	/// copy wins: an existing record is never overwritten, so attribution can't be shifted to
	/// a later carrier of the same candidate. No-op if the scheduling parent has since left
	/// view.
	fn record_fetched_collation(&mut self, fetched_collation: &FetchedCollation) {
		...
		per_sp
			.fetched_collations
			.entry(fetched_collation.candidate_receipt.hash())
			.or_insert(FetchedCollationInfo {
				peer_id: fetched_collation.peer_id,
				para_id: fetched_collation.candidate_receipt.descriptor.para_id(),
			});
	}
	...
	#[cfg(test)]
	pub fn advertisements(&self) -> BTreeSet<PeerAdvertisement> {
		self.per_scheduling_parent
			.values()
			.flat_map(|per_sp| per_sp.all_advertisements())
			.collect()
	}
}
...
/// A peer-advertisement paired with the per-(peer, para) score used for fetch arbitration.
/// Supports ordering.
///
/// Ordering priority: score (descending), then peer-advertisement as a deterministic
/// tiebreaker. Higher scores come first so that `BTreeSet::first()` returns the best.
#[derive(PartialEq, Eq)]
...
impl Ord for AcceptedAdvertisement {
	fn cmp(&self, other: &Self) -> std::cmp::Ordering {
		other
			...
			.then_with(|| self.peer_adv.cmp(&other.peer_adv))
	}
}
...
impl PerSchedulingParent {
	...
	/// Every advertisement at this scheduling parent.
	fn all_advertisements(&self) -> impl Iterator<Item = PeerAdvertisement> + '_ {
		self.peer_advertisements.iter().flat_map(|(peer_id, list)| {
			list.advertisements.iter().map(move |advertisement| PeerAdvertisement {
	...
	/// Whether `peer_adv` may be kept; pair with `add_advertisement` after the caller's
	/// async backing check. Bumps `accept_attempts` even on rejection, so a peer can't spam
	/// past its cap with bad advertisements.
	fn can_keep_advertisement(
		&mut self,
		peer_adv: PeerAdvertisement,
		max_assignments: usize,
	) -> std::result::Result<(), AdvertisementError> {
		let peer_advertisements = self.peer_advertisements.entry(peer_adv.peer_id).or_default();

		peer_advertisements.accept_attempts += 1;

		if peer_advertisements.accept_attempts > max_assignments {
			return Err(AdvertisementError::PeerLimitReached);
		}

		if peer_advertisements.advertisements.contains(&peer_adv.advertisement) {
			return Err(AdvertisementError::Duplicate);
		}
	...
	fn add_advertisement(&mut self, peer_adv: PeerAdvertisement) {
		self.peer_advertisements
			.entry(peer_adv.peer_id)
			.or_default()
			.advertisements
			.insert(peer_adv.advertisement);
	}

	fn remove_advertisement(&mut self, peer_adv: &PeerAdvertisement) {
		if let Some(advertisements) = self.peer_advertisements.get_mut(&peer_adv.peer_id) {
			advertisements.advertisements.remove(&peer_adv.advertisement);
		}
	}
...
/// advertisements. Same peer reasserting the same advertisement is a no-op for
	...
	match res {
		...
		Ok(request_v2::CollationFetchingResponse::Collation(candidate_receipt, pov)) => {
			...
			Ok(FetchedCollation::new(
				...
				pov,
				None,
				peer_adv.advertisement.prospective_candidate.map(|p| p.parent_head_data_hash),
				peer_adv.peer_id,
			))
		},
...
mod tests {
	...
	fn accepted_advertisement_ordering() {
		...
		let make_adv = |peer_id: PeerId| PeerAdvertisement {
			advertisement: Advertisement {
				scheduling_parent,
				para_id,
		...
		{
			let high_score = AcceptedAdvertisement { peer_adv: adv_1, score: score(100) };
			let low_score = AcceptedAdvertisement { peer_adv: adv_2, score: score(50) };

			assert_eq!(high_score.cmp(&low_score), Ordering::Less,);
			assert_eq!(low_score.cmp(&high_score), Ordering::Greater);
		}

		// Same score, different peers - falls back to peer-advertisement comparison.
		{
			let acc_1 = AcceptedAdvertisement { peer_adv: adv_1, score: score(100) };
			let acc_2 = AcceptedAdvertisement { peer_adv: adv_2, score: score(100) };

			// Result depends on peer-advertisement Ord, but must be consistent and not Equal.
			let cmp_result = acc_1.cmp(&acc_2);
			assert_ne!(cmp_result, Ordering::Equal);
			assert_eq!(acc_2.cmp(&acc_1), cmp_result.reverse());
		}

		// Same peer-advertisement, same score - should be Equal.
		{
			let acc_1 = AcceptedAdvertisement { peer_adv: adv_1, score: score(100) };
			let acc_2 = AcceptedAdvertisement { peer_adv: adv_1, score: score(100) };
		...
		// BTreeSet ordering - first() should return highest score.
		{
			...
			let advertisements = [
				AcceptedAdvertisement { peer_adv: adv_1, score: score(50) },
				AcceptedAdvertisement { peer_adv: adv_2, score: score(200) },
				AcceptedAdvertisement { peer_adv: adv_3, score: score(100) },
				AcceptedAdvertisement { peer_adv: adv_4, score: score(150) },
			]
			.into_iter()
			.collect::<BTreeSet<_>>();
	...
	fn pick_best_advertisement_works() {
		...
		// Single advertisement.
		{
			...
			collation_manager
				...
				.get_mut(&scheduling_parent)
				.unwrap()
				.add_advertisement(make_adv(peer_a));
		...
		// Multiple advertisements - picks highest score.
		{
			...
			per_sp.add_advertisement(make_adv(peer_a));
			per_sp.add_advertisement(make_adv(peer_b));
			per_sp.add_advertisement(make_adv(peer_c));
		...
		// Unknown scheduling parent - returns None.
		{
			let collation_manager = new_collation_manager_instance();
			let get_rep = |_: &PeerId, _: &ParaId| Some(score(100));
	...
	// At most one blocked collation is kept: further ones are dropped rather than
	// accumulated, and the first one stays. This bounds the memory a peer can tie up by
	// advertising collations whose parents never get seconded.
	#[test]
	fn at_most_one_blocked_collation_is_kept() {
		...
		// Block more distinct collations than we keep.
		let mut first_hash = None;
		for i in 0..4u8 {
			let ccr = dummy_committed_candidate_receipt_v2(Hash::repeat_byte(i));
			let fetched = FetchedCollation::new(
				ccr.to_plain(),
				PoV { block_data: polkadot_node_primitives::BlockData(vec![i]) },
				None,
				None,
				PeerId::random(),
			);
			first_hash.get_or_insert(fetched.candidate_receipt.hash());
			manager.set_blocked_collation(Hash::repeat_byte(0xf0 | i), fetched);
		}

		let kept = manager
			.blocked_collation
			.as_ref()
			.map(|(_, c)| c.candidate_receipt.hash())
			.expect("exactly one blocked collation must be kept");
		assert_eq!(Some(kept), first_hash, "the first blocked collation must be the one kept");
	}
}
