...
//! Asset-Hub permissionless-collator stop-gap protections (mod `ah_stop_gap` upstream).
//!
//! These scenarios use the legacy adapter's `LegacyValidatorConfig` to inject
//! `invulnerables` (and, where needed, a non-default `collator_protocol_hold_off`); the
//! experimental side has its own permissionless gating that's not exercised here.
//!
//! Mirrors:
//! * `permissionless_collators_are_rejected_when_connection_limit_is_hit`
//! * `invulnerable_collations_are_preferred_over_permissionless_ones`
