...
pub enum DisputeSenderMessage {
	...
	ActiveDisputesReady(JfyiErrorResult<BTreeSet<(SessionIndex, CandidateHash, DisputeStatus)>>),
}
...
async fn get_active_disputes<Sender>(
	sender: &mut Sender,
) -> JfyiErrorResult<BTreeSet<(SessionIndex, CandidateHash, DisputeStatus)>>
where
