...
pub(crate) struct RequestResultCache {
	...
	client_features: LruMap<SessionIndex, vstaging::ClientFeatures>,
}
...
impl RequestResultCache {
	...
	pub(crate) fn client_features(
		&mut self,
		session_index: SessionIndex,
	) -> Option<vstaging::ClientFeatures> {
		self.client_features.get(&session_index).copied()
	}

	pub(crate) fn cache_client_features(
		&mut self,
		session_index: SessionIndex,
		features: vstaging::ClientFeatures,
	) {
		self.client_features.insert(session_index, features);
	}
...
pub(crate) enum RequestResult {
	...
	ClientFeatures(SessionIndex, vstaging::ClientFeatures),
}
