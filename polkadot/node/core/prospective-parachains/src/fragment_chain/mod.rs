...
impl CandidateStorage {
	...
	fn mark_backed(&mut self, candidate_hash: &CandidateHash) {
		if let Some(entry) = self.by_candidate_hash.get_mut(candidate_hash) {
			gum::trace!(target: LOG_TARGET,  candidate_hash = ?candidate_hash.0, "Candidate marked as backed");
			entry.state = CandidateState::Backed;
		} else {
			gum::trace!(target: LOG_TARGET,  candidate_hash = ?candidate_hash.0, "Candidate not found while marking as backed");
		}
	}
						...
						match f {
							...
							Err(e) => {
								gum::debug!(
									...
									err = ?e,
									?relay_parent,
									candidate_hash = ?candidate.candidate_hash.0,
