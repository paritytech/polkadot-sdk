...
impl RelayChainScope {
	...
	pub fn new(
		...
		Ok(RelayChainScope { scheduling_parent, scheduling_ancestors: ancestors })
	}
...
impl FragmentChain {
	...
	fn check_potential_lightweight(
		...
		if !relay_chain_scope.contains_scheduling_ancestor(&scheduling_parent) {
	...
	fn check_potential(
		...
		let Some((is_unconnected, constraints)) = self.check_potential_lightweight(
			relay_chain_scope,
