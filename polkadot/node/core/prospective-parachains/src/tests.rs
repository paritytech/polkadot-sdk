...
fn check_backable_query_multiple_candidates() {
	...
	let view = test_harness(|mut virtual_overseer| async move {
		...
		let (candidate_b, candidate_hash_b) = make_and_back_candidates!(&candidate_a, 2);
		let (candidate_c, candidate_hash_c) = make_and_back_candidates!(&candidate_a, 3);
		let (_candidate_d, candidate_hash_d) = make_and_back_candidates!(&candidate_b, 4);
		let (_candidate_e, candidate_hash_e) = make_and_back_candidates!(&candidate_b, 5);
		let (candidate_f, candidate_hash_f) = make_and_back_candidates!(&candidate_b, 6);
		let (_candidate_g, candidate_hash_g) = make_and_back_candidates!(&candidate_f, 7);
		let (candidate_h, candidate_hash_h) = make_and_back_candidates!(&candidate_c, 8);
		let (candidate_i, candidate_hash_i) = make_and_back_candidates!(&candidate_h, 9);
		let (_candidate_j, candidate_hash_j) = make_and_back_candidates!(&candidate_i, 10);

		// Should not get any backable candidates for the other para.
		get_backable_candidates(&mut virtual_overseer, &leaf_a, 2.into(), vec![], 1, vec![]).await;
		get_backable_candidates(&mut virtual_overseer, &leaf_a, 2.into(), vec![], 5, vec![]).await;
		get_backable_candidates(
			&mut virtual_overseer,
			&leaf_a,
			2.into(),
