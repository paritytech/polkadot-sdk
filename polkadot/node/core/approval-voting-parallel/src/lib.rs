...
where
{
	tracing::info!(target: LOG_TARGET, "Starting approval distribution workers");
	...
	tracing::info!(target: LOG_TARGET, "Starting approval voting workers");
		...
		futures::select! {
			next_msg = ctx.recv().fuse() => {
				let next_msg = match next_msg {
					Ok(msg) => msg,
					Err(err) => {
						tracing::info!(target: LOG_TARGET, ?err, "Approval voting parallel subsystem received an error");
						return Err(err);
					}
				};
...
async fn handle_get_approval_signatures<Context>(
	...
	if let Err(err) = ctx.spawn("approval-voting-gather-signatures", Box::pin(gather_signatures)) {
		tracing::warn!(target: LOG_TARGET, "Failed to spawn gather signatures task: {:?}", err);
	}
}
