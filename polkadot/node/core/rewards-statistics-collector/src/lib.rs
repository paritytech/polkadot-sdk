...
use polkadot_node_subsystem_util::{
	request_node_features, request_session_index_for_child, request_session_info,
};
...
impl PerSessionView {
	fn new(
		...
		Self { authorities_ids, credentials, validators_tallies: HashMap::new(), node_features }
	}
}
...
async fn extract_activated_leaf_info(
	...
	let new_session_info = if !view.per_session.contains_key(&session_idx) {
		...
		let (tx, rx) = oneshot::channel();
		let validators =
			runtime_api_request(sender, relay_hash, RuntimeApiRequest::Validators(tx), rx).await?;

		let signing_credentials =
			polkadot_node_subsystem_util::signing_key_and_index(&validators, keystore).map(
				|(validator_key, validator_index)| SigningCredentials {
					validator_key,
					validator_index,
				},
			);
...
fn aggregate_finalized_approvals_stats(
	...
	for (session_idx, session_view) in &view.per_session {
		let total_approvals: u32 =
			session_view.validators_tallies.values().map(|tally| tally.approvals).sum();
		let total_no_shows: u32 =
			session_view.validators_tallies.values().map(|tally| tally.no_shows).sum();
	...
	match view.latest_finalized_session {
		Some(latest_fin_session) if latest_fin_session < current_fin_session => {
			...
			let removed_sessions =
				view.per_session.keys().filter(|k| **k < current_fin_session).count();

			view.per_session
				.retain(|session_index, _| *session_index >= current_fin_session);
...
async fn sign_and_submit_approvals_tallies(
	...
	let payload = ApprovalStatistics(
		session_index.clone(),
		credentials.validator_index,
		approvals_tallies.clone(),
	);
