...
pub mod well_known_keys {
	...
	/// Published data roots from the broadcaster pallet (StorageMap prefix).
	///
	/// This is the prefix for the `PublishedDataRoots` StorageMap.
	/// To access individual entries, use `published_data_root(para_id)` instead.
	pub const BROADCASTER_PUBLISHED_DATA_ROOTS: &[u8] =
		&hex!["6aca18c1f7576767ccb238db4ccaedf239166324ac7ea24c870f96ab961f9654"];

	/// Get the storage key for a specific parachain's published data root.
	///
	/// Returns the key to access `PublishedDataRoots[para_id]` which contains
	/// the child trie root hash (32 bytes) for that publisher.
	pub fn published_data_root(para_id: Id) -> Vec<u8> {
		para_id.using_encoded(|para_id: &[u8]| {
			BROADCASTER_PUBLISHED_DATA_ROOTS
				.iter()
