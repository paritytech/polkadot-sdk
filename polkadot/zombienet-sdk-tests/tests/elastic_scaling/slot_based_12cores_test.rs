...
async fn slot_based_12cores_test() -> Result<(), anyhow::Error> {
		...
		.with_parachain(|p| {
			p.with_id(2300)
				...
				.with_default_args(vec![
					...
					// ("-lparachain=debug,consensus::common::find_potential_parents=trace,
					// aura::cumulus=trace").into(),
				])
				.with_collator(|n| n.with_name("collator-elastic"))
		})
		.with_global_settings(|global_settings| global_settings.with_base_dir("/tmp/es-demo/"))
		.build()
		.map_err(|e| {
	...
	// candidates.
	assert_finality_lag(¶_node.wait_client().await?, 60).await?;
