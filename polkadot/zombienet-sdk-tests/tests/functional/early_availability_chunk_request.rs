...
async fn early_availability_chunk_request_test() -> Result<(), anyhow::Error> {
	...
	assert_para_throughput(&relay_client, 30, [(ParaId::from(2000), 25..31)].into_iter().collect())
		.await?;
	...
	log::info!("Slow candidates fetched: {late}");
	log::info!("Early fetched candidates got onchain: {early_got_onchain}");

	assert!(early > late, "Expected early fetched candidates > late");
	assert!(early_got_onchain > late, "Expected early fetched candidates got onchain > late");
	Ok(())
}
