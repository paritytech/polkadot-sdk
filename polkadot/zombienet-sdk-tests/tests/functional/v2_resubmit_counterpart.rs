...
//! V2-descriptor counterparts of the `v4_resubmit_*` tests, for throughput comparison.
//!
//! Same topology as the v4 suite (rococo-local, 6 validators, one parachain elastic-scaled to
//! 3 cores, lookahead 5, max_validators_per_core 2, group_rotation_frequency 4, 100-block
//! window), but with the `elastic-scaling` (velocity-3, V2 descriptor) runtime, V2 descriptors
//! only (node-feature bit 3) and the standard collator protocol. Expected ranges are kept wide —
//! the point is measuring the counted candidates, not gating on them.
...
#[rstest]
#[case::single(1, false, "elastic-scaling")]
#[case::single_glutton(1, true, "elastic-scaling")]
#[case::three(3, false, "elastic-scaling")]
...
async fn v2_resubmit_counterpart(
				...
				.with_genesis_overrides(json!({
					"configuration": {
						"config": {
							...
							// V2 descriptors only (bit 3).
							"node_features": {"bits": 8, "data": [0b00001000]},
	...
	log::info!("Para 2900 elastic-scaled to 3 cores (V2 descriptors, chain {chain})");
	...
	log::info!("V2 resubmit counterpart ({num_collators} collators, glutton={glutton}) finished");
	Ok(())
}
