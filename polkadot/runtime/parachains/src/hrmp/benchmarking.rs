	...
	<T as frame_system::Config>::RuntimeOrigin: From<crate::Origin>,
{
	...
	Hrmp::<T>::process_hrmp_open_channel_requests(&configuration::ActiveConfig::<Test>::get());
...
mod benchmarks {
	...
	fn hrmp_init_open_channel() {
		...
		let ed = T::Currency::minimum_balance();
		let deposit: BalanceOf<T> = configuration::ActiveConfig::<Test>::get()
			.hrmp_sender_deposit
			.unique_saturated_into();
		register_parachain_with_balance::<T>(sender_id, deposit + ed);
		register_parachain_with_balance::<T>(recipient_id, deposit + ed);

		let capacity = configuration::ActiveConfig::<Test>::get().hrmp_channel_max_capacity;
		let message_size = configuration::ActiveConfig::<Test>::get().hrmp_channel_max_message_size;
	...
	fn force_open_hrmp_channel(
		...
		let ed = T::Currency::minimum_balance();
		let sender_deposit: BalanceOf<T> = configuration::ActiveConfig::<Test>::get()
			.hrmp_sender_deposit
			.unique_saturated_into();
