...
approval_voting_params                   : pre.approval_voting_params,
	...
	// Allowed to be empty.
	let pending_old_cfg = v11::PendingConfigs::<T>::get().unwrap_or_default();
	let mut pending_new_cfg = Vec::new();

	for (session, v11) in pending_old_cfg.into_iter() {
		let v12 = translate(v11);
		pending_new_cfg.push((session, v12));
	}
	v12::PendingConfigs::<T>::set(Some(pending_new_cfg.clone()));

	let num_configs = (pending_new_cfg.len() + 1) as u64;
...
mod tests {
	...
	fn v12_deserialized_from_actual_data() {
		...
		let raw_config =
	hex_literal::hex!["
    "
	];
		...
		assert_eq!(v12.coretime_cores, 0);
		assert_eq!(v12.on_demand_base_fee, 10_000_000);
		...
		assert_eq!(v12.on_demand_queue_max_size, 500);
	}
	...
	fn test_migrate_to_v12_no_pending() {
		let old_cfg = V11HostConfiguration::<primitives::BlockNumber>::default();

		new_test_ext(Default::default()).execute_with(|| {
			// Implant the v11 version in the state.
			v11::ActiveConfig::<Test>::set(Some(old_cfg));
