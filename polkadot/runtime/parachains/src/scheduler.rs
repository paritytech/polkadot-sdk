...
impl<T: Config> Pallet<T> {
	...
	pub(crate) fn next_up_on_time_out(core: CoreIndex) -> Option<ScheduledCore> {
		let max_availability_timeouts =
			<configuration::Pallet<T>>::config().coretime_max_availability_timeouts;
	...
	pub fn free_cores_and_fill_claimqueue(
		...
		let config = <configuration::Pallet<T>>::config();
		let max_availability_timeouts = config.coretime_max_availability_timeouts;
		let ttl = config.coretime_ttl;
