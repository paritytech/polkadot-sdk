#[cfg(test)]
	...
	fn basic_claim_moving_works() {
		new_test_ext().execute_with(|| {
			assert_eq!(Balances::free_balance(42), 0);
			assert_noop!(
				claims::mock::Claims::move_claim(RuntimeOrigin::signed(1), eth(&alice()), eth(&bob()), None),
				BadOrigin
			);
	...
	fn attesting_works() {
		new_test_ext().execute_with(|| {
			assert_eq!(Balances::free_balance(42), 0);
			assert_noop!(
				claims::mock::Claims::attest(RuntimeOrigin::signed(69), StatementKind::Saft.to_text().to_vec()),
	...
	fn add_claim_works() {
		new_test_ext().execute_with(|| {
			assert_noop!(
				claims::mock::Claims::mint_claim(RuntimeOrigin::signed(42), eth(&bob()), 200, None, None),
			...
			assert_ok!(claims::mock::Claims::mint_claim(RuntimeOrigin::root(), eth(&bob()), 200, None, None));
			assert_eq!(claims::Total::<Test>::get(), total_claims() + 200);
			assert_ok!(claims::mock::Claims::claim(
				RuntimeOrigin::none(),
				69,
	...
	fn add_claim_with_statement_works() {
		new_test_ext().execute_with(|| {
			...
			assert_noop!(
				claims::mock::Claims::claim_attest(RuntimeOrigin::none(), 69, signature.clone(), vec![],),
				Error::<Test>::SignerHasNoClaim
			);
			assert_ok!(claims::mock::Claims::claim_attest(
				RuntimeOrigin::none(),
