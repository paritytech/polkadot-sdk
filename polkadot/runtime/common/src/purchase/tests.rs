...
fn set_statement_works_and_handles_basic_errors() {
	new_test_ext().execute_with(|| {
		...
		// Invalid origin
		assert_noop!(
			Purchase::set_statement(RuntimeOrigin::signed_with_basic_filter(alice()), statement.clone()),
			BadOrigin,
		);
		// Too Long
		let long_statement = [0u8; 10_000].to_vec();
		assert_noop!(
			Purchase::set_statement(RuntimeOrigin::signed_with_basic_filter(configuration_origin()), long_statement),
			Error::<Test>::InvalidStatement,
		);
		// Just right...
		assert_ok!(Purchase::set_statement(
			RuntimeOrigin::signed_with_basic_filter(configuration_origin()),
			statement.clone()
		));
...
fn set_unlock_block_works_and_handles_basic_errors() {
	new_test_ext().execute_with(|| {
		...
		// Invalid origin
		assert_noop!(
			Purchase::set_unlock_block(RuntimeOrigin::signed_with_basic_filter(alice()), unlock_block),
			BadOrigin,
		);
...
fn set_payment_account_works_and_handles_basic_errors() {
	new_test_ext().execute_with(|| {
		...
		// Invalid Origin
		assert_noop!(
			Purchase::set_payment_account(RuntimeOrigin::signed_with_basic_filter(alice()), payment_account.clone()),
			BadOrigin,
		);
		// Just right...
...
fn account_creation_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Purchase::create_account(
			RuntimeOrigin::signed_with_basic_filter(validity_origin()),
			alice(),
...
fn payout_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Purchase::payout(RuntimeOrigin::signed_with_basic_filter(payment_account()), alice(),));
		assert_ok!(Purchase::payout(RuntimeOrigin::signed_with_basic_filter(payment_account()), bob(),));
		// Payment is made.
		...
		assert_ok!(vest_call.clone().dispatch(RuntimeOrigin::signed_with_basic_filter(alice())));
		assert_ok!(vest_call.clone().dispatch(RuntimeOrigin::signed_with_basic_filter(bob())));
		assert_eq!(<Test as Config>::VestingSchedule::vesting_balance(&alice()), Some(45));
		assert_eq!(<Test as Config>::VestingSchedule::vesting_balance(&bob()), Some(140));
...
fn payout_handles_basic_errors() {
	new_test_ext().execute_with(|| {
		// Wrong Origin
		assert_noop!(Purchase::payout(RuntimeOrigin::signed_with_basic_filter(alice()), alice(),), BadOrigin);
		...
		assert_noop!(
			Purchase::payout(RuntimeOrigin::signed_with_basic_filter(payment_account()), bob(),),
			Error::<Test>::VestingScheduleExists
		);
		// Invalid Account (never created)
		assert_noop!(
			Purchase::payout(RuntimeOrigin::signed_with_basic_filter(payment_account()), alice(),),
			Error::<Test>::InvalidAccount
		);
