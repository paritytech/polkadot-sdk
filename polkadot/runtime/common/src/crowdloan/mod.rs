...
mod tests {
	...
	fn create_works() {
		new_test_ext().execute_with(|| {
			...
			assert_ok!(Crowdloan::create(RuntimeOrigin::signed_with_basic_filter(1), para, 1000, 1, 4, 9, None));
	...
	fn create_handles_basic_errors() {
		new_test_ext().execute_with(|| {
			...
			assert_noop!(
				Crowdloan::create(RuntimeOrigin::signed_with_basic_filter(1), 1.into(), 1000, 1, 4, 9, None),
				e
			);
			// Cannot create a crowdloan with bad lease periods
			let e = Error::<Test>::LastPeriodBeforeFirstPeriod;
			assert_noop!(Crowdloan::create(RuntimeOrigin::signed_with_basic_filter(1), para, 1000, 4, 1, 9, None), e);
			let e = Error::<Test>::LastPeriodTooFarInFuture;
			assert_noop!(Crowdloan::create(RuntimeOrigin::signed_with_basic_filter(1), para, 1000, 1, 9, 9, None), e);
			...
			// in lease period 1.
			assert_noop!(
				Crowdloan::create(RuntimeOrigin::signed_with_basic_filter(1), para, 1000, 1, 4, 41, None),
	...
	fn contribute_works() {
		new_test_ext().execute_with(|| {
			...
			assert_ok!(Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(1), para, 49, None));
	...
	fn contribute_with_verifier_works() {
		new_test_ext().execute_with(|| {
			...
			// Missing signature
			assert_noop!(
				Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(1), para, 49, None),
			...
			// Invalid signature
			assert_noop!(
				Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(1), para, 49, Some(invalid_signature)),
				Error::<Test>::InvalidSignature
			);
			...
			assert_noop!(
				Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(1), para, 49, Some(valid_signature)),
				Error::<Test>::InvalidSignature
			);
	...
	fn contribute_handles_basic_errors() {
		new_test_ext().execute_with(|| {
			...
			assert_noop!(
				Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(1), para, 9, None),
				Error::<Test>::ContributionTooSmall
			);

			// Set up a crowdloan
			assert_ok!(Crowdloan::create(RuntimeOrigin::signed_with_basic_filter(1), para, 1000, 1, 4, 9, None));
			assert_ok!(Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(1), para, 101, None));

			// Cannot contribute past the limit
			assert_noop!(
				Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(2), para, 900, None),
			...
			let para_2 = new_para();
			let index = NextFundIndex::<Test>::get();
			assert_ok!(Crowdloan::create(RuntimeOrigin::signed_with_basic_filter(1), para_2, 1000, 1, 4, 40, None));
			...
			assert_noop!(
				Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(1), para_2, 49, None),
			...
			let para_3 = new_para();
			assert_ok!(Crowdloan::create(RuntimeOrigin::signed_with_basic_filter(1), para_3, 1000, 1, 4, 40, None));
			System::run_to_block::<AllPalletsWithSystem>(40);
			let now = System::block_number();
			assert_eq!(TestAuctioneer::lease_period_index(now).unwrap().0, 2);
			assert_noop!(
				Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(1), para_3, 49, None),
				Error::<Test>::ContributionPeriodOver
			);
		});
	}
	...
	fn cannot_contribute_during_vrf() {
		new_test_ext().execute_with(|| {
			...
			assert_ok!(Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(2), para, 250, None));

			System::run_to_block::<AllPalletsWithSystem>(10);
			// Can't contribute when auction is in the VRF delay period.
			assert!(TestAuctioneer::auction_status(System::block_number()).is_vrf());
			assert_noop!(
				Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(2), para, 250, None),
	...
	fn bidding_works() {
		new_test_ext().execute_with(|| {
			...
			assert_ok!(Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(2), para, 100, None));
			System::run_to_block::<AllPalletsWithSystem>(3);
			assert_ok!(Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(3), para, 150, None));
			System::run_to_block::<AllPalletsWithSystem>(5);
			assert_ok!(Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(4), para, 200, None));
			System::run_to_block::<AllPalletsWithSystem>(8);
			assert_ok!(Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(2), para, 250, None));
			System::run_to_block::<AllPalletsWithSystem>(10);
	...
	fn withdraw_from_failed_works() {
		new_test_ext().execute_with(|| {
			...
			// Set up a crowdloan
			assert_ok!(Crowdloan::create(RuntimeOrigin::signed_with_basic_filter(1), para, 1000, 1, 1, 9, None));
			assert_ok!(Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(2), para, 100, None));
			assert_ok!(Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(3), para, 50, None));
			...
			assert_ok!(Crowdloan::withdraw(RuntimeOrigin::signed_with_basic_filter(2), 2, para));
			assert_eq!(Balances::free_balance(&account_id), 50);
			assert_eq!(Balances::free_balance(2), 2000);

			assert_ok!(Crowdloan::withdraw(RuntimeOrigin::signed_with_basic_filter(2), 3, para));
	...
	fn withdraw_cannot_be_griefed() {
		new_test_ext().execute_with(|| {
			...
			assert_ok!(Balances::transfer_allow_death(RuntimeOrigin::signed_with_basic_filter(1), account_id, 10));
			...
			assert_ok!(Crowdloan::dissolve(RuntimeOrigin::signed_with_basic_filter(1), para));
			assert_eq!(Balances::free_balance(&account_id), 0);
	...
	fn refund_works() {
		new_test_ext().execute_with(|| {
			...
			assert_ok!(Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(1), para, 100, None));
			assert_ok!(Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(2), para, 200, None));
			assert_ok!(Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(3), para, 300, None));

			assert_eq!(Balances::free_balance(account_id), 600);

			// Can't refund before the crowdloan it has ended
			assert_noop!(
				Crowdloan::refund(RuntimeOrigin::signed_with_basic_filter(1337), para),
				Error::<Test>::FundNotEnded,
			);

			// Move to the end of the crowdloan
			System::run_to_block::<AllPalletsWithSystem>(10);
			assert_ok!(Crowdloan::refund(RuntimeOrigin::signed_with_basic_filter(1337), para));
	...
	fn multiple_refund_works() {
		new_test_ext().execute_with(|| {
			...
			// Set up a crowdloan ending on 9
			assert_ok!(Crowdloan::create(RuntimeOrigin::signed_with_basic_filter(1), para, 100000, 1, 1, 9, None));
			// Make more contributions than our limit
			for i in 1..=RemoveKeysLimit::get() * 2 {
				Balances::make_free_balance_be(&i.into(), (1000 * i).into());
				assert_ok!(Crowdloan::contribute(
					RuntimeOrigin::signed_with_basic_filter(i.into()),
					para,
					(i * 100).into(),
					None
				));
	...
	fn refund_and_dissolve_works() {
		new_test_ext().execute_with(|| {
			...
			assert_ok!(Crowdloan::refund(RuntimeOrigin::signed_with_basic_filter(2), para));
	...
	fn dissolve_works() {
		new_test_ext().execute_with(|| {
			...
			assert_noop!(
				Crowdloan::dissolve(RuntimeOrigin::signed_with_basic_filter(1), para),
	...
	fn withdraw_from_finished_works() {
		new_test_ext().execute_with(|| {
			...
			assert_noop!(
				Crowdloan::withdraw(RuntimeOrigin::signed_with_basic_filter(2), 2, para),
				Error::<Test>::BidOrLeaseActive
			);
	...
	fn on_swap_works() {
		new_test_ext().execute_with(|| {
			...
			// Set up crowdloans
			assert_ok!(Crowdloan::create(RuntimeOrigin::signed_with_basic_filter(1), para_1, 1000, 1, 1, 9, None));
			assert_ok!(Crowdloan::create(RuntimeOrigin::signed_with_basic_filter(1), para_2, 1000, 1, 1, 9, None));
			// Different contributions
			assert_ok!(Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(2), para_1, 100, None));
			assert_ok!(Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(3), para_2, 50, None));
			// Original state
	...
	fn cannot_create_fund_when_already_active() {
		new_test_ext().execute_with(|| {
			...
			assert_noop!(
				Crowdloan::create(RuntimeOrigin::signed_with_basic_filter(1), para_1, 1000, 1, 1, 9, None),
				Error::<Test>::FundNotEnded,
			);
		});
	}
	...
	fn add_memo_works() {
		new_test_ext().execute_with(|| {
			...
			assert_noop!(
				Crowdloan::add_memo(RuntimeOrigin::signed_with_basic_filter(1), para_1, b"hello, world".to_vec()),
				Error::<Test>::NoContributions,
			);
			// Make a contribution. Initially no memo.
			assert_ok!(Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(1), para_1, 100, None));
			assert_eq!(Crowdloan::contribution_get(0u32, &1), (100, vec![]));
			// Can't place a memo that is too large.
			assert_noop!(
				Crowdloan::add_memo(RuntimeOrigin::signed_with_basic_filter(1), para_1, vec![123; 123]),
	...
	fn poke_works() {
		new_test_ext().execute_with(|| {
			...
			assert_noop!(
				Crowdloan::poke(RuntimeOrigin::signed_with_basic_filter(1), para_1),
				Error::<Test>::NoContributions
			);
			assert_ok!(Crowdloan::contribute(RuntimeOrigin::signed_with_basic_filter(2), para_1, 100, None));
			System::run_to_block::<AllPalletsWithSystem>(6);
			assert_ok!(Crowdloan::poke(RuntimeOrigin::signed_with_basic_filter(1), para_1));
...
mod benchmarking {
	...
	mod benchmarks {
		...
		fn create() -> Result<(), BenchmarkError> {
			...
			assert_last_event::<T>(Event::<T>::Created { para_id }.into_with_basic_filter());
			Ok(())
		}
		...
		fn refund(k: Linear<0, { T::RemoveKeysLimit::get() }>) -> Result<(), BenchmarkError> {
			...
			assert_last_event::<T>(Event::<T>::AllRefunded { para_id: fund_index }.into_with_basic_filter());
			Ok(())
		}
		...
		fn dissolve() -> Result<(), BenchmarkError> {
			...
			assert_last_event::<T>(Event::<T>::Dissolved { para_id: fund_index }.into_with_basic_filter());
			Ok(())
		}
		...
		fn edit() -> Result<(), BenchmarkError> {
			...
			assert_last_event::<T>(Event::<T>::Edited { para_id }.into_with_basic_filter());
		...
		fn poke() -> Result<(), BenchmarkError> {
			...
			_(RawOrigin::Signed(caller), fund_index);
			assert!(!NewRaise::<T>::get().is_empty());
			assert_last_event::<T>(Event::<T>::AddedToNewRaise { para_id: fund_index }.into_with_basic_filter());
			Ok(())
		}
		...
		fn on_initialize(n: Linear<2, 100>) -> Result<(), BenchmarkError> {
			...
			for i in 0..n {
				...
				Crowdloan::<T>::contribute(
					RawOrigin::Signed(contributor).into_with_basic_filter(),
					fund_index,
					contribution,
					Some(sig),
				)?;
