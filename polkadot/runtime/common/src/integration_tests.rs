...
fn basic_swap_works() {
	...
	new_test_ext().execute_with(|| {
		...
		let para_deposit = <Test as paras_registrar::Config>::ParaDeposit::get();
		...
		assert_eq!(
			...
			crowdloan_depoist + para_deposit + code_deposit + 10
		);
		// Para 2001 has a genesis head size of 20.
		assert_eq!(Balances::reserved_balance(&account_id(2)), para_deposit + code_deposit + 20);
