...
sp_api::impl_runtime_apis! {
	...
	impl beefy_primitives::BeefyApi<Block, BeefyId, Hash> for Runtime {
		...
		fn submit_report_fork_equivocation_unsigned_extrinsic(
			_fork_equivocation_proof: beefy_primitives::ForkEquivocationProof<BlockNumber, BeefyId, BeefySignature, Header, Hash>,
	...
	impl mmr::MmrApi<Block, Hash, BlockNumber> for Runtime {
		...
		) -> Result<(Vec<mmr::EncodableOpaqueLeaf>, mmr::Proof<(u64, Hash)>), mmr::Error> {
			Err(mmr::Error::PalletNotIncluded)
		}

		fn verify_proof(_leaves: Vec<mmr::EncodableOpaqueLeaf>, _proof: mmr::Proof<(u64, Hash)>)
			-> Result<(), mmr::Error>
		{
			Err(mmr::Error::PalletNotIncluded)
		}
