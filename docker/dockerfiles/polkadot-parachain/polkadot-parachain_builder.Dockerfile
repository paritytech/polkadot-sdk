...
RUN cargo build --release --locked -p polkadot-parachain-omni-node
...
COPY --from=builder /cumulus/target/release/polkadot-parachain-omni-node /usr/local/bin
...
RUN useradd -m -u 1000 -U -s /bin/sh -d /cumulus polkadot-parachain && \
    ...
    ln -s /data /cumulus/.local/share/polkadot-parachain-omni-node && \
# unclutter and minimize the attack surface
    rm -rf /usr/bin /usr/sbin && \
# check if executable works in this container
    /usr/local/bin/polkadot-parachain-omni-node --version
