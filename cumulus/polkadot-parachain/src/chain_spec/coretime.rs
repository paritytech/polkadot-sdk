...
pub mod rococo {
	...
	use parachains_common::{rococo::currency, AccountId, AuraId};
	use sc_chain_spec::ChainType;
	use sp_core::sr25519;

	pub(crate) const CORETIME_ROCOCO: &str = "coretime-rococo";
	const CORETIME_ROCOCO_ED: CoretimeBalance = currency::EXISTENTIAL_DEPOSIT;
	...
	fn genesis(
		...
		id: ParaId,
	) -> coretime_rococo_runtime::RuntimeGenesisConfig {
		coretime_rococo_runtime::RuntimeGenesisConfig {
			system: coretime_rococo_runtime::SystemConfig {
				code: coretime_rococo_runtime::WASM_BINARY
			...
			balances: coretime_rococo_runtime::BalancesConfig {
				balances: endowed_accounts
					.iter()
					.cloned()
					.map(|k| (k, CORETIME_ROCOCO_ED * 4096))
					.collect(),
			},
...
pub mod westend {
	...
	use parachains_common::{westend::currency, AccountId, AuraId};
	use sc_chain_spec::ChainType;
	use sp_core::sr25519;

	pub(crate) const CORETIME_WESTEND: &str = "coretime-westend";
	const CORETIME_WESTEND_ED: CoretimeBalance = currency::EXISTENTIAL_DEPOSIT;
	...
	fn genesis(
		...
		endowed_accounts: Vec<AccountId>,
		id: ParaId,
	) -> coretime_westend_runtime::RuntimeGenesisConfig {
		coretime_westend_runtime::RuntimeGenesisConfig {
			system: coretime_westend_runtime::SystemConfig {
				code: coretime_westend_runtime::WASM_BINARY
					.expect("WASM binary was not built, please build it!")
					.to_vec(),
				..Default::default()
			},
			balances: coretime_westend_runtime::BalancesConfig {
				balances: endowed_accounts
					.iter()
					.cloned()
					.map(|k| (k, CORETIME_WESTEND_ED * 4096))
					.collect(),
			},
