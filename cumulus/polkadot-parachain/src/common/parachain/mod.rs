...
use crate::common::{
	command::NodeCommandRunner, BuildImportQueue as BuildImportQueueT, BuildRpcExtensions,
	BuildSelectChain, ConstructNodeRuntimeApi, NodeBackend, NodeSpec, NodeSpecProvider,
};
...
impl BuildSelectChain<Block> for BuildParachainSelectChain {
	...
	fn build_select_chain(_backend: Arc<NodeBackend<Block>>) -> Self::SelectChain {
		()
	}
}
...
where
	...
	BuildImportQueue: BuildImportQueueT<Block, ParachainClient<RuntimeApi>>,
{
	type Block = Block;
...
pub(crate) trait ParachainNodeSpec {
	...
	type BuildImportQueue: BuildImportQueueT<
		...
		BlockImport = ParachainBlockImport<Self::RuntimeApi>,
	>;
	...
	type StartConsensus: StartConsensus<Self::RuntimeApi>;
		...
		Net: NetworkBackend<Block, Hash>,
	{
		Box::pin(async move {
			...
			let params = ParachainNode::<Self::RuntimeApi, Self::BuildImportQueue>::new_partial(
				&parachain_config,
			)?;
...
impl<T> DynParachainNodeSpec for T
where
	T: NodeCommandRunner + ParachainNodeSpec,
{
	fn start_node(
		self: Box<Self>,
		parachain_config: Configuration,
		...
		match parachain_config.network.network_backend {
			sc_network::config::NetworkBackendType::Libp2p =>
				<Self as ParachainNodeSpec>::start_node::<sc_network::NetworkWorker<_, _>>(
					parachain_config,
					polkadot_config,
					collator_options,
					para_id,
			...
			sc_network::config::NetworkBackendType::Litep2p =>
				<Self as ParachainNodeSpec>::start_node::<sc_network::Litep2pNetworkBackend>(
					parachain_config,
					polkadot_config,
					collator_options,
					para_id,
