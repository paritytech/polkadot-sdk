...
#			pub UniversalLocationAHW: InteriorLocation = [GlobalConsensus(Wococo), Parachain(1000)].into();
#		}
#
#		// SS58=42
...
#						 &Location::new(2, [GlobalConsensus(Rococo)] }).unwrap()
...
#						 &Location::new(2, [GlobalConsensus(Rococo), Parachain(1000)] }).unwrap()
#				 ).to_ss58check_with_version(42_u16.into())
#		);
...
#						 &Location::new(1, [Parachain(1000)] }).unwrap()
#				 ).to_ss58check_with_version(42_u16.into())
#		);
#
#		// SS58=42
...
#						 &Location::new(2, [GlobalConsensus(Wococo)] }).unwrap()
#				 ).to_ss58check_with_version(42_u16.into())
#		);
...
#						 &Location::new(2, [GlobalConsensus(Wococo), Parachain(1000)] }).unwrap()
#				 ).to_ss58check_with_version(42_u16.into())
#		);
...
function force_create_foreign_asset() {
    ...
    echo "      runtime_para_id: ${runtime_para_id}"
    echo "      runtime_para_endpoint: ${runtime_para_endpoint}"
    echo "      asset_location: ${asset_location}"
    ...
    local tmp_output_file=$(mktemp)
    generate_hex_encoded_call_data "force-create-asset" "${runtime_para_endpoint}" "${tmp_output_file}" "$asset_location" "$asset_owner_account_id" $is_sufficient $min_balance
    local hex_encoded_data=$(cat $tmp_output_file)
