...
// This test ensures that the chain continues to make progress after authorities change.
#[tokio::test(flavor = "multi_thread")]
async fn authorities_change() -> Result<(), anyhow::Error> {
	...
	for (name, timeout_secs) in [("dave", 250u64), ("ferdie", 250u64)] {
		...
		let result = network
			.get_node(name)?
			.wait_log_line_count_with_timeout(
				"Imported new authorities from block header",
				false,
