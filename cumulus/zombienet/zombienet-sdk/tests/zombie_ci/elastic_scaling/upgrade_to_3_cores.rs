...
// and that the parachain produces 3 blocks per relay slot after the upgrade. It covers both
// async-backing and sync-backing starting states; both upgrade to the same `elastic_scaling`
// WASM whose `OnRuntimeUpgrade` migration writes `BlockProcessingVelocity = 3` into
// `pallet_parameters::Parameters`. The starting slot duration (6s for async-backing, 12s for
// sync-backing) is already correct from the chain-spec preset, so only velocity changes.

use crate::utils::initialize_network;
use anyhow::anyhow;
use cumulus_test_runtime::elastic_scaling::WASM_BINARY as WASM_ELASTIC_SCALING;
...
async fn elastic_scaling_upgrade_to_3_cores(
	...
	let wasm = WASM_ELASTIC_SCALING.expect("Wasm runtime not build");
