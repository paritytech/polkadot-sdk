...
pub mod pallet {
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		...
		fn integrity_test() {
			...
			// The relay chain <-> parachain slot conversion (e.g. in the V3 scheduling signature
			// verifier and the consensus hook) assumes the parachain slot duration is a positive
