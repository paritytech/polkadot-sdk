...
pub enum SchedulingValidationError {
	...
	///
	/// With an empty chain, `internal_scheduling_parent` is derived as
	/// `scheduling_parent`. Allowing `relay_parent != scheduling_parent` here would
	/// produce `is_resubmission = true` with no slot-anchor header available,
	/// which would later panic the verifier on `header_chain.last()`.
...
/// `PSC::SchedulingSignatureVerifier` using `header_chain.last()` as the slot
/// anchor and the returned `internal_scheduling_parent`.
...
mod tests {
	...
	/// plus the IP hash (= `relay_parent` for initial submission), i.e. the parent
	/// of the chain's last header.
	...
	fn initial_submission_allows_signed_scheduling_info() {
		...
		let proof =
			SchedulingProof { header_chain: headers, signed_scheduling_info: Some(signed_info) };
		let result = check_scheduling(&proof, relay_parent, scheduling_parent, 3);
	...
	fn reject_empty_chain_with_mismatched_relay_parent() {
		// With an empty chain there is no slot-anchor header. Allowing
		// `relay_parent != scheduling_parent` would produce `is_resubmission = true`
		// and later panic the verifier on `header_chain.last()`. Reject it here.
		let scheduling_parent = RelayHash::repeat_byte(0xAA);
