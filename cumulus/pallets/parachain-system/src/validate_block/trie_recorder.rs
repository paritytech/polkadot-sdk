...
mod tests {
	...
	fn proof_recorder_equivalence_no_cache() {
		...
		for _ in 1..10 {
			...
			assert_eq!(
				reference_recorder.to_storage_proof(),
				recorder_for_test.to_storage_proof(),
			);
			// Also consensus-critical: the estimated proof size must match the std recorder's, since
			// it is summed into `storage_proof_size` → weight-reclaim → `System::BlockWeight` → the
			// state root. Any divergence build↔validate would break validation.
			assert_eq!(
				reference_recorder.estimate_encoded_size(),
				recorder_for_test.estimate_encoded_size(),
			);
