...
mod tests {
	...
	fn test_verify_relay_parent_descendants_happy_case(
		...
		let (relay_read_root, relay_read_proof) = build_relay_chain_storage_proof(Some(authorities), None);
	...
	/// Run `verify_relay_parent_descendants` with a host-backed [`RelayChainStateProof`] whose reads
	/// are served (and verified against `root`) from the given relay-state `proof` — mirroring how
	/// the reads flow through the `read_relay_chain_state` host function in production.
	fn verify_with_relay_reads(
		...
		let provider = sp_additional_data::VerifyingAdditionalDataProvider::<
			sp_runtime::traits::BlakeTwo256,
		>::from_map_with_root(root, map)
		.expect("valid relay-proof map");
		let mut ext = sp_io::TestExternalities::default();
		ext.register_extension(sp_additional_data::AdditionalDataExt(Box::new(provider)));
		ext.execute_with(|| {
