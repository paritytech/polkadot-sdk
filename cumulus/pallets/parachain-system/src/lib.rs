...
pub mod pallet {
	...
	pub trait Config: frame_system::Config<OnSetCode = ParachainSetCode<Self>> {
		...
		/// 3. Swap the verifier for one whose `enabled()` returns `true` via a runtime upgrade
		///
		...
		fn on_initialize(_n: BlockNumberFor<T>) -> Weight {
			...
			match CumulusDigestItem::core_info_exists_at_max_once(
				...
				CoreInfoExistsAtMaxOnce::Once(core_info) => {
					...
					if !T::SchedulingSignatureVerifier::enabled() {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_validation_data(
			...
			let v3_enabled = T::SchedulingSignatureVerifier::enabled();
