...
fn create_funded_user<T: Config>(
	...
	let balance = <T as crate::Config>::Currency::minimum_balance() * balance_factor.into();
	let _ = <T as crate::Config>::Currency::make_free_balance_be(&user, balance);
	user
}
...
fn register_candidates<T: Config>(count: u32) {
	...
	for who in candidates {
		<T as crate::Config>::Currency::make_free_balance_be(&who, CandidacyBond::<T>::get() * 3u32.into());
...
mod benchmarks {
	...
	fn add_invulnerable(
		...
		CandidacyBond::<T>::put(<T as crate::Config>::Currency::minimum_balance());
		...
		for (who, _) in candidates.iter() {
			...
			<T as crate::Config>::Currency::make_free_balance_be(who, deposit * 1000_u32.into());
			CandidateList::<T>::try_mutate(|list| {
				list.try_push(CandidateInfo { who: who.clone(), deposit }).unwrap();
				Ok::<(), BenchmarkError>(())
			})
			.unwrap();
			<T as crate::Config>::Currency::reserve(who, deposit)?;
	...
	fn set_candidacy_bond(
		...
		let initial_bond_amount: BalanceOf<T> = <T as crate::Config>::Currency::minimum_balance() * 2u32.into();
		...
		let bond_amount = if k > 0 {
			CandidateList::<T>::mutate(|candidates| {
				for info in candidates.iter_mut().skip(kicked as usize) {
					info.deposit = <T as crate::Config>::Currency::minimum_balance() * 3u32.into();
				}
			});
			<T as crate::Config>::Currency::minimum_balance() * 3u32.into()
		} else {
			<T as crate::Config>::Currency::minimum_balance()
		};
	...
	fn update_bond(
		...
		let bond_amount: BalanceOf<T> =
			<T as crate::Config>::Currency::minimum_balance() + <T as crate::Config>::Currency::minimum_balance();
		...
		assert!(
			CandidateList::<T>::get().iter().last().unwrap().deposit ==
				<T as crate::Config>::Currency::minimum_balance() * 2u32.into()
		);
		Ok(())
	}
	...
	fn register_as_candidate(c: Linear<1, { T::MaxCandidates::get() - 1 }>) {
		...
		let bond: BalanceOf<T> = <T as crate::Config>::Currency::minimum_balance() * 2u32.into();
		<T as crate::Config>::Currency::make_free_balance_be(&caller, bond);
	...
	fn take_candidate_slot(c: Linear<{ min_candidates::<T>() + 1 }, { T::MaxCandidates::get() }>) {
		...
		let caller: T::AccountId = whitelisted_caller();
		let bond: BalanceOf<T> = <T as crate::Config>::Currency::minimum_balance() * 10u32.into();
		<T as crate::Config>::Currency::make_free_balance_be(&caller, bond);
	...
	fn note_author() {
		...
		<T as crate::Config>::Currency::make_free_balance_be(
			...
			<T as crate::Config>::Currency::minimum_balance() * 4u32.into(),
		);
		let author = account("author", 0, SEED);
		let new_block: BlockNumberFor<T> = 10u32.into();

		frame_system::Pallet::<T>::set_block_number(new_block);
		assert!(<T as crate::Config>::Currency::free_balance(&author) == 0u32.into());
