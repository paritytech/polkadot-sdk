...
type Migrations = (
	...
	pallet_core_fellowship::migration::MigrateV1ToV2<
		Runtime,
		CoreFellowshipBlockNumberConverter,
...
pub struct CoreFellowshipBlockNumberConverter;

impl pallet_core_fellowship::migration::v2::ConvertBlockNumber<BlockNumber, BlockNumber>
	for CoreFellowshipBlockNumberConverter
{
	fn equivalent_moment_in_time(local_moment: BlockNumber) -> BlockNumber {
		...
		if local_block_number >= local_moment {
			relay_block_number.saturating_sub(local_block_number.saturating_sub(local_moment))
		} else {
			relay_block_number.saturating_add(local_moment.saturating_sub(local_block_number))
		}
	}

	fn equivalent_block_duration(local_duration: BlockNumber) -> BlockNumber {
		local_duration
	}
}
