...
use xcm::{
	latest::prelude::*,
	prelude::{AlwaysLatest, DetermineVersion, XcmVersion},
};
	...
	NetworkDistanceAsParentCount: Get<u8>,
{
	...
	impl<N: Get<NetworkId>, C: Get<u8>> Get<(NetworkId, u8)> for NetworkWithParentCount<N, C> {
		fn get() -> (NetworkId, u8) {
			(N::get(), C::get())
		}
	}
...
pub mod test_data {
	...
	macro_rules! grab_haul_blob (
		($name:ident, $grabbed_payload:ident) => {
			...
			impl DetermineVersion for $name {
				fn determine_version_for(dest: &MultiLocation, handle_unknown: bool) -> Option<XcmVersion> {
					AlwaysLatest::determine_version_for(dest, handle_unknown)
				}
			}
		}
	);
	...
	pub(crate) fn simulate_message_exporter_on_bridged_chain<
		...
		DestinationNetwork: Get<(NetworkId, u8)>,
	>(
