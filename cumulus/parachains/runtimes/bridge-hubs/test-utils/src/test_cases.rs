...
/// Test-case makes sure that Runtime can dispatch XCM messages from GRANDPA chain (relay or
/// standalone) submitted by relayer, with proofs (finality, para heads, message) independently
/// submitted.
pub fn relayed_incoming_message_from_grandpa_chain_works<Runtime, AllPalletsWithoutSystem, XcmConfig, HrmpChannelOpener, GPI, MPI, MB>(
	collator_session_key: CollatorSessionKeys<Runtime>,
	runtime_para_id: u32,
	bridged_para_id: u32,
	sibling_parachain_id: u32,
	...
	<MB as MessageBridge>::BridgedChain: Send + Sync + 'static,
	<MB as MessageBridge>::ThisChain: Send + Sync + 'static,
	UnderlyingChainOf<MessageBridgedChain<MB>>: bp_runtime::Chain<Hash = ParaHash>,
	XcmConfig: xcm_executor::Config,
	HrmpChannelOpener: frame_support::inherent::ProvideInherent<
		Call = cumulus_pallet_parachain_system::Call<Runtime>,
	>,
...
pub mod test_data {
	...
	/// Prepare storage proofs of messages, stored at the source chain.
	pub fn make_complex_relayer_delivery_proofs_from_grandpa_chain<
		BridgedChain,
		MB,
		InnerXcmRuntimeCall,
	>(
	...
	/// Make bridged parachain header with given state root and relay header that is finalizing it.
	pub fn make_complex_bridged_heads_proof_from_<BridgedRelayChain, MB>(
		para_state_root: ParaHash,
		para_header_number: u32,
		relay_header_number: u32,
		bridged_para_id: u32,
