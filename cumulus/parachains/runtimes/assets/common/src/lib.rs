...
impl<Runtime: pallet_asset_conversion::Config<PoolId = (L, L), AssetKind = L>, L: TryFrom<Location> + TryInto<Location> + Clone + Decode + EncodeLike + PartialEq> PoolAdapter<Runtime> {
	...
	/// Provides a current prices. Wrapper over `pallet_asset_conversion::Pallet::<T>::quote_price_tokens_for_exact_tokens`.
	///
	/// An error of type `()` is returned if the version conversion fails for XCM locations.
	/// This error should be mapped by the caller to a more descriptive one.
	pub fn quote_price_tokens_for_exact_tokens(asset_1: Location, asset_2: Location, amount: Runtime::Balance, include_fees: bool) -> Result<Option<Runtime::Balance>, ()> {
		// convert latest to the `L` version.
