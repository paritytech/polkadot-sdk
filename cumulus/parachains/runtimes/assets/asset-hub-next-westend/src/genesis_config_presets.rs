...
pub fn get_preset(id: &PresetId) -> Option<Vec<u8>> {
	...
	let patch = match id.try_into() {
		Ok(PRESET_GENESIS) => asset_hub_next_westend_genesis(
			// initial collators.
			vec![
				(
		...
		Ok(sp_genesis_builder::LOCAL_TESTNET_RUNTIME_PRESET) => asset_hub_next_westend_genesis(
			// initial collators.
			vec![
		...
		Ok(sp_genesis_builder::DEV_RUNTIME_PRESET) => asset_hub_next_westend_genesis(
			// initial collators.
