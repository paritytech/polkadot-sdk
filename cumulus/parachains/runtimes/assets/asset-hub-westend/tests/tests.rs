...
mod spec_messaging {
	...
	fn new_test_ext() -> sp_io::TestExternalities {
		let mut ext: sp_io::TestExternalities = frame_system::GenesisConfig::<Runtime>::default()
			...
			.into();
		ext.execute_with(|| {
			// Both transports `wrap_version` against the destination: set
			// the `SafeXcmVersion` fallback, exactly the operational
			// requirement of opening a spec-msg pair (normally genesis-set).
	...
	/// as `Ready`, as the validation-data inherent would from relay state.
	...
	fn hrmp_closed_sibling_routes_via_spec_msg_only_over_an_open_channel() {
		new_test_ext().execute_with(|| {
			// No HRMP channel (fresh state: everything reports `Closed`) and
			// no spec-msg channel: the router falls through — nothing lands
			// on a stream and the send surfaces `XcmpQueue`'s delivery
			// failure instead of being silently diverted.
			assert!(send_xcm::<XcmRouter>(sibling(), test_xcm()).is_err());
			assert!(OutboundMessages::<Runtime>::iter().next().is_none());

			// Arming the spec-msg channel diverts the send onto the
			// designated channel stream, as a `Data` leaf holding exactly
			// the SCALE-encoded `VersionedXcm`.
			open_spec_msg_channel();
			assert_ok!(send_xcm::<XcmRouter>(sibling(), test_xcm()));
			...
			// The end-of-block fold commits the touched stream and feeds
			// the `Provides` UMP signal
			// (`parachain_system::Config::UmpSignalSource`) and the header
			// digest.
			let root =
	...
	fn hrmp_closing_flag_diverts_new_traffic_while_the_channel_drains() {
		new_test_ext().execute_with(|| {
			...
			// spec-msg, before the closure is observable in the relay
			// state.
			...
			// Rollback: clearing the flag restores HRMP-wins while the
			// channel exists — nothing new lands on the stream.
	...
	fn consumed_payloads_fit_the_message_queue() {
		// `EnqueueToXcmQueue` requires the queue's `MaxMessageLen` (derived
		// from its `HeapSize`) to cover everything the receiver part can
		// consume.
	...
	fn worst_case_messaging_inherent_fits_the_pov_budget() {
		let touched: u32 =
			<Runtime as cumulus_pallet_spec_messaging::Config>::MaxTouchedStreams::get();
		let gaps: u32 = <Runtime as cumulus_pallet_spec_messaging::Config>::MaxContextGaps::get();

		// A worst-case consumption block: the touched-stream cap exhausted,
		// the register-read slots all opening context gaps.
		...
		let call =
			RuntimeCall::SpecMessaging(cumulus_pallet_spec_messaging::Call::enact_messages {
				data,
			});
		let info = call.get_dispatch_info();

		// The `proof_size` reservation charged up front for the POV lifts
		// the submitter attaches outside block execution...
		...
		assert!(
			info.call_weight.proof_size() <= RuntimeBlockWeights::get().max_block.proof_size() / 4
		);
	}
}
