...
fn relay_dest_assertions_fail(_t: SystemParaToRelayTest) {
	Rococo::assert_ump_queue_processed(false, Some(CoretimeRococo::para_id()), None);
}

fn para_origin_assertions(t: SystemParaToRelayTest) {
	type RuntimeEvent = <CoretimeRococo as Chain>::RuntimeEvent;

	CoretimeRococo::assert_xcm_pallet_attempted_complete(None);
...
fn limited_teleport_native_assets_from_system_para_to_relay_fails() {
	...
	// Fund a sender
	CoretimeRococo::fund_accounts(vec![(CoretimeRococoSender::get(), ROCOCO_ED * 2_000u128)]);
