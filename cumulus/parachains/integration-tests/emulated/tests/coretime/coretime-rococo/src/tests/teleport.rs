...
fn para_origin_assertions(t: SystemParaToRelayTest) {
	...
	CoretimeRococo::assert_xcm_pallet_attempted_complete(Some(Weight::from_parts(
		720_053_000,
		7_203,
	)));

	CoretimeRococo::assert_parachain_system_ump_sent();
...
fn para_dest_assertions(t: RelayToSystemParaTest) {
	...
	CoretimeRococo::assert_dmp_queue_complete(Some(Weight::from_parts(157_718_000, 3593)));
...
fn system_para_limited_teleport_assets(t: SystemParaToRelayTest) -> DispatchResult {
	<CoretimeRococo as CoretimeRococoPallet>::PolkadotXcm::limited_teleport_assets(
		t.signed_origin,
		bx!(t.args.dest.into()),
		bx!(t.args.beneficiary.into()),
		bx!(t.args.assets.into()),
...
fn system_para_teleport_assets(t: SystemParaToRelayTest) -> DispatchResult {
	<CoretimeRococo as CoretimeRococoPallet>::PolkadotXcm::teleport_assets(
		t.signed_origin,
		bx!(t.args.dest.into()),
		bx!(t.args.beneficiary.into()),
		bx!(t.args.assets.into()),
...
fn limited_teleport_native_assets_from_relay_to_system_para_works() {
	...
	let test_args = TestContext {
		...
		receiver: CoretimeRococoReceiver::get(),
	...
	test.set_assertion::<CoretimeRococo>(para_dest_assertions);
...
fn limited_teleport_native_assets_back_from_system_para_to_relay_works() {
	...
	let test_args = TestContext {
		sender: CoretimeRococoSender::get(),
	...
	test.set_assertion::<CoretimeRococo>(para_origin_assertions);
	test.set_assertion::<Rococo>(relay_dest_assertions);
	test.set_dispatchable::<CoretimeRococo>(system_para_limited_teleport_assets);
	test.assert();

	let sender_balance_after = test.sender.balance;
	let receiver_balance_after = test.receiver.balance;

	let delivery_fees = CoretimeRococo::execute_with(|| {
		xcm_helpers::transfer_assets_delivery_fees::<
			<CoretimeRococoXcmConfig as xcm_executor::Config>::XcmSender,
		>(test.args.assets.clone(), 0, test.args.weight_limit, test.args.beneficiary, test.args.dest)
	});
...
fn teleport_native_assets_back_from_system_para_to_relay_works() {
	...
	test.set_assertion::<CoretimeRococo>(para_origin_assertions);
	test.set_assertion::<Rococo>(relay_dest_assertions);
	test.set_dispatchable::<CoretimeRococo>(system_para_teleport_assets);
	test.assert();
