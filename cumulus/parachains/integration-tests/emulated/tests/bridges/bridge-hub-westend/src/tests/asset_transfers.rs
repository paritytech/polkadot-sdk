...
fn dry_run_transfer_to_rococo_sends_xcm_to_bridge_hub() {
	...
	<AssetHubWestend as TestExt>::execute_with(|| {
		...
		type OriginCaller = <AssetHubWestend as Chain>::OriginCaller;
		...
		assert_eq!(result.forwarded_xcms[0].0, VersionedLocation::from(Location::new(1, [Parachain(BridgeHubWestend::para_id().into())])));
	});
}
