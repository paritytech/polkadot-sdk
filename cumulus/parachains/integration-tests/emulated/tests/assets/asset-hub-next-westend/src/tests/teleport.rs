...
fn relay_dest_assertions_fail(_t: SystemParaToRelayTest) {
	Westend::assert_ump_queue_processed(
		false,
		Some(AssetHubNextWestend::para_id()),
...
fn para_origin_assertions(t: SystemParaToRelayTest) {
	...
	AssetHubNextWestend::assert_parachain_system_ump_sent();
...
fn system_para_limited_teleport_assets(t: SystemParaToRelayTest) -> DispatchResult {
	<AssetHubNextWestend as AssetHubNextWestendPallet>::PolkadotXcm::limited_teleport_assets(
		t.signed_origin,
		bx!(t.args.dest.into()),
...
fn system_para_to_para_transfer_assets(t: SystemParaToParaTest) -> DispatchResult {
	<AssetHubNextWestend as AssetHubNextWestendPallet>::PolkadotXcm::transfer_assets(
		t.signed_origin,
		bx!(t.args.dest.into()),
		bx!(t.args.beneficiary.into()),
		bx!(t.args.assets.into()),
...
fn teleport_to_other_system_parachains_works() {
	let amount = ASSET_HUB_NEXT_WESTEND_ED * 100;
	let native_asset: Assets = (Parent, amount).into();

	test_parachain_is_trusted_teleporter!(
		AssetHubNextWestend,          // Origin
		AssetHubNextWestendXcmConfig, // XCM Configuration
...
fn teleport_from_and_to_relay() {
	...
	test_relay_is_trusted_teleporter!(
		Westend,
		WestendXcmConfig,
		vec![AssetHubNextWestend],
		(native_asset, amount)
	);

	test_parachain_is_trusted_teleporter_for_relay!(
		AssetHubNextWestend,
		AssetHubNextWestendXcmConfig,
		Westend,
		amount
	);
}
...
fn limited_teleport_native_assets_from_system_para_to_relay_fails() {
	...
	let destination = AssetHubNextWestend::parent_location().into();
	let beneficiary_id = WestendReceiver::get().into();
	let assets = (Parent, amount_to_send).into();

	let test_args = TestContext {
		sender: AssetHubNextWestendSender::get(),
	...
	test.set_assertion::<AssetHubNextWestend>(para_origin_assertions);
	test.set_assertion::<Westend>(relay_dest_assertions_fail);
	test.set_dispatchable::<AssetHubNextWestend>(system_para_limited_teleport_assets);
	test.assert();

	let sender_balance_after = test.sender.balance;
	let receiver_balance_after = test.receiver.balance;

	let delivery_fees = AssetHubNextWestend::execute_with(|| {
		xcm_helpers::teleport_assets_delivery_fees::<
			<AssetHubNextWestendXcmConfig as xcm_executor::Config>::XcmSender,
...
pub fn do_bidirectional_teleport_foreign_assets_between_para_and_asset_hub_using_xt(
	...
	let fee_amount_to_send: Balance = ASSET_HUB_NEXT_WESTEND_ED * 100;
	...
	let ah_as_seen_by_penpal = PenpalA::sibling_location_of(AssetHubNextWestend::para_id());
	...
	let sov_penpal_on_ah = AssetHubNextWestend::sovereign_account_id_of(penpal_as_seen_by_ah);
	AssetHubNextWestend::fund_accounts(vec![(
		sov_penpal_on_ah.clone().into(),
		ASSET_HUB_NEXT_WESTEND_ED * 100_000_000_000,
	)]);
	...
	let foreign_asset_at_asset_hub_next_westend =
		Location::new(1, [Junction::Parachain(PenpalA::para_id().into())])
			...
			.unwrap();
	let penpal_to_ah_beneficiary_id = AssetHubNextWestendReceiver::get();

	// Penpal to AH test args
	let penpal_to_ah_test_args = TestContext {
		sender: PenpalASender::get(),
		receiver: AssetHubNextWestendReceiver::get(),
	...
	let ah_receiver_assets_before = AssetHubNextWestend::execute_with(|| {
		type Assets = <AssetHubNextWestend as AssetHubNextWestendPallet>::ForeignAssets;
		<Assets as Inspect<_>>::balance(
			foreign_asset_at_asset_hub_next_westend.clone().try_into().unwrap(),
			&AssetHubNextWestendReceiver::get(),
		)
	});

	penpal_to_ah.set_assertion::<PenpalA>(penpal_to_ah_foreign_assets_sender_assertions);
	penpal_to_ah
		.set_assertion::<AssetHubNextWestend>(penpal_to_ah_foreign_assets_receiver_assertions);
	...
	let ah_receiver_assets_after = AssetHubNextWestend::execute_with(|| {
		type Assets = <AssetHubNextWestend as AssetHubNextWestendPallet>::ForeignAssets;
		<Assets as Inspect<_>>::balance(
			foreign_asset_at_asset_hub_next_westend.clone().try_into().unwrap(),
			&AssetHubNextWestendReceiver::get(),
	...
	let ah_assets: Assets = vec![
		(Parent, fee_amount_to_send).into(),
		(foreign_asset_at_asset_hub_next_westend.clone(), asset_amount_to_send).into(),
	]
	.into();
	...
	let ah_sender_assets_before = AssetHubNextWestend::execute_with(|| {
		type ForeignAssets = <AssetHubNextWestend as AssetHubNextWestendPallet>::ForeignAssets;
		<ForeignAssets as Inspect<_>>::balance(
			foreign_asset_at_asset_hub_next_westend.clone().try_into().unwrap(),
	...
	ah_to_penpal
		.set_assertion::<AssetHubNextWestend>(ah_to_penpal_foreign_assets_sender_assertions);
	ah_to_penpal.set_assertion::<PenpalA>(ah_to_penpal_foreign_assets_receiver_assertions);
	ah_to_penpal.set_dispatchable::<AssetHubNextWestend>(ah_to_para_dispatchable);
	ah_to_penpal.assert();
	...
	let ah_sender_assets_after = AssetHubNextWestend::execute_with(|| {
		type ForeignAssets = <AssetHubNextWestend as AssetHubNextWestendPallet>::ForeignAssets;
		<ForeignAssets as Inspect<_>>::balance(
			foreign_asset_at_asset_hub_next_westend.clone().try_into().unwrap(),
			&AssetHubNextWestendSender::get(),
