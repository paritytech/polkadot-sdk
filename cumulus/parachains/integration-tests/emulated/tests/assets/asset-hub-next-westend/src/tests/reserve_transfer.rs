...
pub fn system_para_to_para_sender_assertions(t: SystemParaToParaTest) {
	...
	AssetHubNextWestend::assert_xcm_pallet_attempted_complete(None);

	let sov_acc_of_dest = AssetHubNextWestend::sovereign_account_id_of(t.args.dest.clone());
	...
	AssetHubNextWestend::assert_xcm_pallet_sent();
}
...
pub fn para_to_system_para_receiver_assertions(t: ParaToSystemParaTest) {
	...
	let sov_acc_of_penpal = AssetHubNextWestend::sovereign_account_id_of(t.args.dest.clone());
...
fn system_para_to_para_assets_sender_assertions(t: SystemParaToParaTest) {
	type RuntimeEvent = <AssetHubNextWestend as Chain>::RuntimeEvent;
	AssetHubNextWestend::assert_xcm_pallet_attempted_complete(Some(Weight::from_parts(
		864_610_000,
		8799,
	)));
	assert_expected_events!(
		...
		vec![
			...
			RuntimeEvent::Assets(
				...
				from: *from == t.sender.account_id,
				to: *to == AssetHubNextWestend::sovereign_account_id_of(
					t.args.dest.clone()
				),
				amount: *amount == t.args.amount,
			},
			...
			RuntimeEvent::Balances(pallet_balances::Event::Minted { who, .. }) => {
				who: *who == AssetHubNextWestend::sovereign_account_id_of(
					t.args.dest.clone()
				),
			},
			// Delivery fees are paid
...
fn para_to_system_para_assets_receiver_assertions(t: ParaToSystemParaTest) {
	type RuntimeEvent = <AssetHubNextWestend as Chain>::RuntimeEvent;
	let sov_penpal_on_ahr = AssetHubNextWestend::sovereign_account_id_of(
...
fn para_to_para_asset_hub_hop_assertions(t: ParaToParaThroughAHTest) {
	type RuntimeEvent = <AssetHubNextWestend as Chain>::RuntimeEvent;
	let sov_penpal_a_on_ah = AssetHubNextWestend::sovereign_account_id_of(
		AssetHubNextWestend::sibling_location_of(PenpalA::para_id()),
	);
	let sov_penpal_b_on_ah = AssetHubNextWestend::sovereign_account_id_of(
...
fn system_para_to_para_reserve_transfer_assets(t: SystemParaToParaTest) -> DispatchResult {
	<AssetHubNextWestend as AssetHubNextWestendPallet>::PolkadotXcm::limited_reserve_transfer_assets(
		t.signed_origin,
		bx!(t.args.dest.into()),
		bx!(t.args.beneficiary.into()),
		bx!(t.args.assets.into()),
...
fn reserve_transfer_native_asset_from_relay_to_asset_hub_fails() {
	...
	let destination = Westend::child_location_of(AssetHubNextWestend::para_id());
	let beneficiary: Location =
		AccountId32Junction { network: None, id: AssetHubNextWestendReceiver::get().into() }.into();
...
fn reserve_transfer_native_asset_from_asset_hub_to_relay_fails() {
	...
	let signed_origin = <AssetHubNextWestend as Chain>::RuntimeOrigin::signed(
		AssetHubNextWestendSender::get().into(),
	);
	let destination = AssetHubNextWestend::parent_location();
	let beneficiary_id = WestendReceiver::get();
	let beneficiary: Location =
		AccountId32Junction { network: None, id: beneficiary_id.into() }.into();
	let amount_to_send: Balance = ASSET_HUB_NEXT_WESTEND_ED * 1000;
...
fn reserve_transfer_native_asset_from_asset_hub_to_para() {
	// Init values for Asset Hub
	let destination = AssetHubNextWestend::sibling_location_of(PenpalA::para_id());
	let sender = AssetHubNextWestendSender::get();
	let amount_to_send: Balance = ASSET_HUB_NEXT_WESTEND_ED * 2000;
	let assets: Assets = (Parent, amount_to_send).into();
	...
	// Set assertions and dispatchables
	test.set_assertion::<AssetHubNextWestend>(system_para_to_para_sender_assertions);
	test.set_assertion::<PenpalA>(system_para_to_para_receiver_assertions);
	test.set_dispatchable::<AssetHubNextWestend>(system_para_to_para_reserve_transfer_assets);
	test.assert();
...
fn reserve_transfer_native_asset_from_para_to_asset_hub() {
	// Init values for Parachain
	let destination = PenpalA::sibling_location_of(AssetHubNextWestend::para_id());
	let sender = PenpalASender::get();
	let amount_to_send: Balance = ASSET_HUB_NEXT_WESTEND_ED * 1000;
	let assets: Assets = (Parent, amount_to_send).into();
	let system_para_native_asset_location = RelayLocation::get();
	...
	// Init values for Asset Hub
	let receiver = AssetHubNextWestendReceiver::get();
	let penpal_location_as_seen_by_ahr =
		AssetHubNextWestend::sibling_location_of(PenpalA::para_id());
	let sov_penpal_on_ahr =
		AssetHubNextWestend::sovereign_account_id_of(penpal_location_as_seen_by_ahr);

	// fund Parachain's SA on Asset Hub with the native tokens held in reserve
	AssetHubNextWestend::fund_accounts(vec![(sov_penpal_on_ahr.into(), amount_to_send * 2)]);
	...
	test.set_assertion::<AssetHubNextWestend>(para_to_system_para_receiver_assertions);
...
fn reserve_transfer_multiple_assets_from_asset_hub_to_para() {
	// Init values for Asset Hub
	let destination = AssetHubNextWestend::sibling_location_of(PenpalA::para_id());
	let sov_penpal_on_ahr = AssetHubNextWestend::sovereign_account_id_of(destination.clone());
	let sender = AssetHubNextWestendSender::get();
	let fee_amount_to_send = ASSET_HUB_NEXT_WESTEND_ED * 100;
	let asset_amount_to_send = ASSET_HUB_NEXT_WESTEND_ED * 100;
	let asset_owner = AssetHubNextWestendAssetOwner::get();
	let asset_owner_signer =
		<AssetHubNextWestend as Chain>::RuntimeOrigin::signed(asset_owner.clone());
	...
	let fee_asset_index = assets
		...
		.unwrap() as u32;
	AssetHubNextWestend::mint_asset(
		asset_owner_signer,
		RESERVABLE_ASSET_ID,
		asset_owner,
	...
	AssetHubNextWestend::fund_accounts(vec![(sov_penpal_on_ahr.into(), ASSET_HUB_NEXT_WESTEND_ED)]);
	...
	// Query initial balances
	let sender_balance_before = test.sender.balance;
	let sender_assets_before = AssetHubNextWestend::execute_with(|| {
		type Assets = <AssetHubNextWestend as AssetHubNextWestendPallet>::Assets;
	...
	// Set assertions and dispatchables
	test.set_assertion::<AssetHubNextWestend>(system_para_to_para_assets_sender_assertions);
	...
	// Query final balances
	let sender_balance_after = test.sender.balance;
	let sender_assets_after = AssetHubNextWestend::execute_with(|| {
		type Assets = <AssetHubNextWestend as AssetHubNextWestendPallet>::Assets;
		<Assets as Inspect<_>>::balance(RESERVABLE_ASSET_ID, &sender)
	});
...
fn reserve_transfer_multiple_assets_from_para_to_asset_hub() {
	...
	let sov_penpal_on_ahr =
		AssetHubNextWestend::sovereign_account_id_of(penpal_location_as_seen_by_ahr);
	let ah_asset_owner = AssetHubNextWestendAssetOwner::get();
	let ah_asset_owner_signer =
		<AssetHubNextWestend as Chain>::RuntimeOrigin::signed(ah_asset_owner);

	// Fund SA-of-Penpal-on-AHR to be able to pay for the fees.
	AssetHubNextWestend::fund_accounts(vec![(
		sov_penpal_on_ahr.clone().into(),
		ASSET_HUB_NEXT_WESTEND_ED * 1000,
	)]);
	...
	let receiver_balance_before = test.receiver.balance;
	let receiver_assets_before = AssetHubNextWestend::execute_with(|| {
		type Assets = <AssetHubNextWestend as AssetHubNextWestendPallet>::Assets;
	...
	test.set_assertion::<AssetHubNextWestend>(para_to_system_para_assets_receiver_assertions);
	test.set_dispatchable::<PenpalA>(para_to_system_para_reserve_transfer_assets);
	test.assert();
	...
	let receiver_balance_after = test.receiver.balance;
	let receiver_assets_after = AssetHubNextWestend::execute_with(|| {
		type Assets = <AssetHubNextWestend as AssetHubNextWestendPallet>::Assets;
		<Assets as Inspect<_>>::balance(RESERVABLE_ASSET_ID, &receiver)
	});
...
fn reserve_transfer_usdt_from_asset_hub_to_para() {
	let usdt_id = 1984u32;
	let penpal_location = AssetHubNextWestend::sibling_location_of(PenpalA::para_id());
	let penpal_sov_account = AssetHubNextWestend::sovereign_account_id_of(penpal_location.clone());
	...
	// So we can swap the custom asset that comes from AssetHubNextWestend for native asset to pay
	// for fees.
	...
	let sender_initial_balance = AssetHubNextWestend::execute_with(|| {
		type Assets = <AssetHubNextWestend as AssetHubNextWestendPallet>::Assets;
		<Assets as Inspect<_>>::balance(usdt_id, &sender)
	});
	let sender_initial_native_balance = AssetHubNextWestend::execute_with(|| {
		type Balances = <AssetHubNextWestend as AssetHubNextWestendPallet>::Balances;
	...
	let sender_after_balance = AssetHubNextWestend::execute_with(|| {
		type Assets = <AssetHubNextWestend as AssetHubNextWestendPallet>::Assets;
		<Assets as Inspect<_>>::balance(usdt_id, &sender)
	});
	let sender_after_native_balance = AssetHubNextWestend::execute_with(|| {
		type Balances = <AssetHubNextWestend as AssetHubNextWestendPallet>::Balances;
		Balances::free_balance(&sender)
	});
...
fn reserve_transfer_usdt_from_para_to_para_through_asset_hub() {
	...
	// This ED isn't reflected in any derivative in a PenpalA account.
	AssetHubNextWestend::fund_accounts(vec![
		(sov_of_sender_on_asset_hub.clone().into(), ASSET_HUB_NEXT_WESTEND_ED),
		(sov_of_receiver_on_asset_hub.clone().into(), ASSET_HUB_NEXT_WESTEND_ED),
	]);
	...
	create_pool_with_wnd_on!(AssetHubNextWestend, usdt, false, AssetHubNextWestendSender::get());
	...
	test.set_assertion::<PenpalA>(para_to_para_through_hop_sender_assertions);
	test.set_assertion::<AssetHubNextWestend>(para_to_para_asset_hub_hop_assertions);
	test.set_assertion::<PenpalB>(para_to_para_through_hop_receiver_assertions);
...
fn reserve_withdraw_from_untrusted_reserve_fails() {
	...
	// this should fail
	AssetHubNextWestend::execute_with(|| {
		let result = <AssetHubNextWestend as AssetHubNextWestendPallet>::PolkadotXcm::transfer_assets_using_type_and_then(
			signed_origin.clone(),
	...
	AssetHubNextWestend::execute_with(|| {
		let xcm: Xcm<asset_hub_next_westend_runtime::RuntimeCall> = Xcm(vec![
		...
		let result = <AssetHubNextWestend as AssetHubNextWestendPallet>::PolkadotXcm::execute(
			signed_origin,
