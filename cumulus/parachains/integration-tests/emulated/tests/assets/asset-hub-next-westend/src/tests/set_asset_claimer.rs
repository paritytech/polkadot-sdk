...
use westend_system_emulated_network::{
	asset_hub_next_westend_emulated_chain::asset_hub_next_westend_runtime::RuntimeOrigin as AssetHubRuntimeOrigin,
...
fn test_set_asset_claimer_within_a_chain() {
	...
	AssetHubNextWestend::fund_accounts(vec![(alice_account.clone(), trap_amount * 2)]);
...
fn account_and_location(account: &str) -> (AccountId32, Location) {
	let account_id = AssetHubNextWestend::account_id_of(account);
...
fn test_set_asset_claimer_between_the_chains() {
	let alice = AssetHubNextWestend::account_id_of(ALICE);
	let alice_bh_sibling = Location::new(
		1,
		[
			Parachain(AssetHubNextWestend::para_id().into()),
	...
	let xcm_on_bh = Xcm::<()>::builder_unsafe()
		...
		.build();
	let bh_on_ah = AssetHubNextWestend::sibling_location_of(BridgeHubWestend::para_id()).into();
	AssetHubNextWestend::execute_with(|| {
		assert_ok!(<AssetHubNextWestend as AssetHubNextWestendPallet>::PolkadotXcm::send(
