...
fn system_para_sets_relay_xcm_supported_version() {
	// Init test variables
	let parent_location = AssetHubNextWestend::parent_location();
	let force_xcm_version_call =
		<AssetHubNextWestend as Chain>::RuntimeCall::PolkadotXcm(pallet_xcm::Call::<
			<AssetHubNextWestend as Chain>::Runtime,
		>::force_xcm_version {
