...
fn assets_can_be_claimed() {
	let amount = AssetHubNextWestendExistentialDeposit::get();
...
fn chain_can_claim_assets_for_its_users() {
	// Many Penpal users have assets trapped in AssetHubNextWestend.
	...
	AssetHubNextWestend::execute_with(|| {
		for (location, assets) in &beneficiaries {
			<AssetHubNextWestend as AssetHubNextWestendPallet>::PolkadotXcm::drop_assets(
				location,
				assets.clone().into(),
	...
	let penpal_to_asset_hub = PenpalA::sibling_location_of(AssetHubNextWestend::para_id());
	...
	// Fund PenpalA's sovereign account on AssetHubNextWestend so it can pay for fees.
	AssetHubNextWestend::execute_with(|| {
		let penpal_as_seen_by_asset_hub =
			AssetHubNextWestend::sibling_location_of(PenpalA::para_id());
		let penpal_sov_account_on_asset_hub =
			AssetHubNextWestend::sovereign_account_id_of(penpal_as_seen_by_asset_hub);
	...
	AssetHubNextWestend::execute_with(|| {
		for (location, expected_assets) in &beneficiaries {
			let sov_account = AssetHubNextWestend::sovereign_account_id_of(location.clone());
			let actual_assets =
				<AssetHubNextWestend as Chain>::Runtime::query_account_balances(sov_account)
					.unwrap();
