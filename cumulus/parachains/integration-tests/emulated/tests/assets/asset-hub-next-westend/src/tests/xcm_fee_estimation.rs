...
fn transfer_assets_para_to_para_through_ah_call(
	...
	let asset_hub_location: Location = PenpalA::sibling_location_of(AssetHubNextWestend::para_id());
...
fn multi_hop_works() {
	...
	let sender_as_seen_by_ah = AssetHubNextWestend::sibling_location_of(PenpalA::para_id());
	let sov_of_sender_on_ah =
		AssetHubNextWestend::sovereign_account_id_of(sender_as_seen_by_ah.clone());
	...
	AssetHubNextWestend::fund_accounts(vec![(sov_of_sender_on_ah.clone(), amount_to_send * 2)]);
	...
	<PenpalA as TestExt>::execute_with(|| {
		...
		let (destination_to_query, messages_to_query) = &result
			...
			.iter()
			.find(|(destination, _)| {
				*destination == VersionedLocation::from(Location::new(1, [Parachain(1100)]))
			})
			.unwrap();
	...
	<AssetHubNextWestend as TestExt>::execute_with(|| {
		type Runtime = <AssetHubNextWestend as Chain>::Runtime;
		type RuntimeCall = <AssetHubNextWestend as Chain>::RuntimeCall;
	...
	AssetHubNextWestend::fund_accounts(vec![(sov_of_sender_on_ah, amount_to_send * 2)]);
	...
	test.set_assertion::<AssetHubNextWestend>(hop_assertions);
