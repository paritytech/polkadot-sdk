...
fn ah_to_para_transfer_assets(t: SystemParaToParaTest) -> DispatchResult {
	...
	<AssetHubNextWestend as AssetHubNextWestendPallet>::PolkadotXcm::transfer_assets_using_type_and_then(
		t.signed_origin,
		bx!(t.args.dest.into()),
...
fn transfer_foreign_assets_from_asset_hub_to_para() {
	...
	let native_amount_to_send: Balance = ASSET_HUB_NEXT_WESTEND_ED * 1000;
	let native_asset_location = RelayLocation::get();
	let receiver = PenpalAReceiver::get();
	let assets_owner = PenpalAssetOwner::get();
	// Foreign asset used: bridged ROC
	let foreign_amount_to_send = ASSET_HUB_NEXT_WESTEND_ED * 10_000_000;
	...
	AssetHubNextWestend::force_create_foreign_asset(
	...
	AssetHubNextWestend::mint_foreign_asset(
		<AssetHubNextWestend as Chain>::RuntimeOrigin::signed(assets_owner),
		roc_at_westend_parachains.clone().try_into().unwrap(),
		sender.clone(),
	...
	let sender_rocs_before = AssetHubNextWestend::execute_with(|| {
		type ForeignAssets = <AssetHubNextWestend as AssetHubNextWestendPallet>::ForeignAssets;
	...
	test.set_dispatchable::<AssetHubNextWestend>(ah_to_para_transfer_assets);
	test.assert();

	// Query final balances
	let sender_balance_after = test.sender.balance;
	let sender_rocs_after = AssetHubNextWestend::execute_with(|| {
		type ForeignAssets = <AssetHubNextWestend as AssetHubNextWestendPallet>::ForeignAssets;
		<ForeignAssets as Inspect<_>>::balance(
			roc_at_westend_parachains.clone().try_into().unwrap(),
			&sender,
...
fn transfer_foreign_assets_from_para_to_asset_hub() {
	...
	let native_amount_to_send: Balance = ASSET_HUB_NEXT_WESTEND_ED * 10000;
	let native_asset_location = RelayLocation::get();
	let assets_owner = PenpalAssetOwner::get();
	...
	let receiver_rocs_before = AssetHubNextWestend::execute_with(|| {
		type ForeignAssets = <AssetHubNextWestend as AssetHubNextWestendPallet>::ForeignAssets;
		<ForeignAssets as Inspect<_>>::balance(
			roc_at_westend_parachains.clone().try_into().unwrap(),
			&receiver,
	...
	let receiver_rocs_after = AssetHubNextWestend::execute_with(|| {
		type ForeignAssets = <AssetHubNextWestend as AssetHubNextWestendPallet>::ForeignAssets;
		<ForeignAssets as Inspect<_>>::balance(
...
fn transfer_foreign_assets_from_para_to_para_through_asset_hub() {
	...
	let sov_of_sender_on_ah = AssetHubNextWestend::sovereign_account_id_of(sender_as_seen_by_ah);
	let receiver_as_seen_by_ah = AssetHubNextWestend::sibling_location_of(PenpalB::para_id());
	let sov_of_receiver_on_ah =
		AssetHubNextWestend::sovereign_account_id_of(receiver_as_seen_by_ah);
	let roc_to_send = ASSET_HUB_NEXT_WESTEND_ED * 10_000_000;
	...
	AssetHubNextWestend::fund_accounts(vec![(sov_of_sender_on_ah.clone().into(), wnd_to_send * 2)]);
	AssetHubNextWestend::mint_foreign_asset(
		<AssetHubNextWestend as Chain>::RuntimeOrigin::signed(assets_owner),
		roc_at_westend_parachains.clone().try_into().unwrap(),
	...
	let wnds_in_sender_reserve_on_ah_before =
		<AssetHubNextWestend as Chain>::account_data_of(sov_of_sender_on_ah.clone()).free;
	let rocs_in_sender_reserve_on_ah_before = AssetHubNextWestend::execute_with(|| {
		type Assets = <AssetHubNextWestend as AssetHubNextWestendPallet>::ForeignAssets;
	...
	let wnds_in_receiver_reserve_on_ah_before =
		<AssetHubNextWestend as Chain>::account_data_of(sov_of_receiver_on_ah.clone()).free;
	let rocs_in_receiver_reserve_on_ah_before = AssetHubNextWestend::execute_with(|| {
		type Assets = <AssetHubNextWestend as AssetHubNextWestendPallet>::ForeignAssets;
		<Assets as Inspect<_>>::balance(
			roc_at_westend_parachains.clone().try_into().unwrap(),
	...
	test.set_assertion::<AssetHubNextWestend>(para_to_para_assethub_hop_assertions);
	...
	let rocs_in_sender_reserve_on_ah_after = AssetHubNextWestend::execute_with(|| {
		type Assets = <AssetHubNextWestend as AssetHubNextWestendPallet>::ForeignAssets;
		<Assets as Inspect<_>>::balance(
			roc_at_westend_parachains.clone().try_into().unwrap(),
			&sov_of_sender_on_ah,
	...
	let wnds_in_sender_reserve_on_ah_after =
		<AssetHubNextWestend as Chain>::account_data_of(sov_of_sender_on_ah).free;
	let rocs_in_receiver_reserve_on_ah_after = AssetHubNextWestend::execute_with(|| {
		type Assets = <AssetHubNextWestend as AssetHubNextWestendPallet>::ForeignAssets;
		<Assets as Inspect<_>>::balance(
			roc_at_westend_parachains.clone().try_into().unwrap(),
			&sov_of_receiver_on_ah,
	...
	let wnds_in_receiver_reserve_on_ah_after =
		<AssetHubNextWestend as Chain>::account_data_of(sov_of_receiver_on_ah).free;
...
fn transfer_native_asset_from_relay_to_penpal_through_asset_hub() {
	...
	let sov_penpal_on_ah = AssetHubNextWestend::sovereign_account_id_of(
		AssetHubNextWestend::sibling_location_of(PenpalA::para_id()),
	);
	// Query initial balances
	let sender_balance_before = test.sender.balance;
	let sov_penpal_on_ah_before = AssetHubNextWestend::execute_with(|| {
	...
	fn transfer_assets_dispatchable(t: RelayToParaThroughAHTest) -> DispatchResult {
		let fee_idx = t.args.fee_asset_item as usize;
		let fee: Asset = t.args.assets.inner().get(fee_idx).cloned().unwrap();
		let asset_hub_location = Westend::child_location_of(AssetHubNextWestend::para_id());
		...
		Dmp::make_parachain_reachable(AssetHubNextWestend::para_id());
	...
	test.set_assertion::<AssetHubNextWestend>(asset_hub_assertions);
	...
	// Query final balances
	let sender_balance_after = test.sender.balance;
	let sov_penpal_on_ah_after = AssetHubNextWestend::execute_with(|| {
		<AssetHubNextWestend as AssetHubNextWestendPallet>::Balances::free_balance(sov_penpal_on_ah)
	});
	let receiver_assets_after = PenpalA::execute_with(|| {
		type ForeignAssets = <PenpalA as PenpalAPallet>::ForeignAssets;
...
fn transfer_native_asset_from_penpal_to_relay_through_asset_hub() {
	...
	AssetHubNextWestend::fund_accounts(vec![(sov_penpal_on_ah.clone().into(), amount_to_send * 2)]);
...
fn bidirectional_transfer_multiple_assets_between_penpal_and_asset_hub() {
	...
	fn execute_xcm_asset_hub_to_penpal(t: SystemParaToParaTest) -> DispatchResult {
		...
		<AssetHubNextWestend as AssetHubNextWestendPallet>::PolkadotXcm::execute(
			t.signed_origin,
			bx!(xcm::VersionedXcm::from(xcm.into())),
			Weight::MAX,
		)
