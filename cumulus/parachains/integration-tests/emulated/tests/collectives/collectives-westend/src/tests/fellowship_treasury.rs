...
fn fellowship_treasury_spend() {
	...
	Westend::execute_with(|| {
		...
		let treasury_location_on_asset_hub: Location = (Parent, PalletInstance(37)).into();
	...
	CollectivesWestend::execute_with(|| {
		...
		let alice_location: Location =
			[Junction::AccountId32 { network: None, id: Westend::account_id_of(ALICE).into() }]
				.into();
	...
	AssetHubWestend::execute_with(|| {
		...
		let alice_account = Westend::account_id_of(ALICE);
		assert_eq!(
