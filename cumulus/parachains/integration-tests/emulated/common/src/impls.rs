...
/// Implementation of `BridgeMessageHandler` for `pallet_bridge_messages`,
/// configured with `pallet_xcm_bridge_hub`.
...
where
	...
	<T as BridgeMessagesConfig<TI>>::MessageDispatch:
		MessageDispatch<DispatchLevelResult = pallet_xcm_bridge_hub::XcmBlobMessageDispatchResult>,
{
...
/// Implementation of `BridgeMessageHandler` for `pallet_bridge_messages`,
/// configured with `pallet_xcm_bridge`.
pub struct BridgeMessagesHandler<S, SI, T, TI> {
	_marker: std::marker::PhantomData<(S, SI, T, TI)>,
}
impl<S, SI, T, TI> BridgeMessageHandler for BridgeMessagesHandler<S, SI, T, TI>
where
	S: BridgeMessagesConfig<SI>,
	SI: 'static,
	T: BridgeMessagesConfig<TI>,
