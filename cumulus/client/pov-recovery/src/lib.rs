...
pub struct PoVRecovery<Block: BlockT, PC, RC> {
	...
	waiting_for_parent:
		HashMap<Block::Hash, Vec<(Block, Option<sp_additional_data::AdditionalData>)>>,
	...
	RCInterface: RelayChainInterface + Clone,
{
	...
	fn import_blocks(
		&mut self,
		blocks: impl Iterator<Item = (Block, Option<sp_additional_data::AdditionalData>)>,
	) {
		...
		while let Some((block, additional_data)) = blocks.pop_front() {
			...
			incoming_blocks.push(IncomingBlock {
				...
				state: None,
				indexed_body: None,
				// Recovered blocks re-execute on import; carry the relay-read proof recovered as
				// part of the PoV so `read_relay_chain_state` can be served.
				additional_data,
			});
