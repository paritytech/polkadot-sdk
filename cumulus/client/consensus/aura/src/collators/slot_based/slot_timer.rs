...
impl SlotTimer {
	...
	pub fn new_with_offset(time_offset: Duration, relay_slot_duration: Duration) -> Self {
		Self { time_offset, relay_slot_duration, last_reported_slot: None, v3_enabled: false }
	}

	/// Setter for `v3_enabled` field.
	///
	/// Can be updated during block building depending on what runtime returns.
	pub fn set_v3_enabled(&mut self, v3_enabled: bool) {
	...
	pub async fn wait_until_next_slot(&mut self) -> Result<SlotTime, ()> {
		let time_offset = if self.v3_enabled { self.time_offset } else { Duration::ZERO };
		let (time_until_next_attempt, timestamp) =
			time_until_next_slot(duration_now(), self.relay_slot_duration, time_offset);
		...
		Ok(SlotTime::new(self.relay_slot_duration, slot_start_timestamp, time_offset))
	}
}
