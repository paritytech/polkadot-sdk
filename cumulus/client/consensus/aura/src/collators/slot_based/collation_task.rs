		...
		RClient: RelayChainInterface + Clone + 'static,
	{
		match self {
			...
			CollatorMessage::Segment(CollatorSegmentMessage {
				...
				// Hydrating `resubmittable_headers` into entries ahead of the bundle lands with
				// the resubmission path; for now the bundle is the segment's only entry.
...
/// `parse_ump_signals` reads the same `SelectCore`/`ApprovedPeer` the PVF would compute. The
/// collation carrying the unrewritten tail must be dropped: its commitments would not match
/// the PVF's output.
#[derive(Debug, thiserror::Error)]
enum UmpOverrideError {
	#[error("failed to decode an `UMPSignal` in the scheduling tail: {0}")]
	UndecodableSignal(#[from] codec::Error),

	#[error("upward messages are full; cannot re-emit the scheduling tail")]
	Overflow,
}
...
fn override_ump_scheduling_tail(
	...
	if let Some(pos) = upward_messages.iter().position(|m| m == &UMP_SEPARATOR) {
		for bytes in upward_messages.iter().skip(pos + 1) {
			...
			// compile here, because the truncate below would silently drop it. A decode error
			// means the block emitted a signal this node cannot understand (e.g. a newer
			// runtime), which the caller handles by dropping the collation.
	...
	upward_messages
		...
		.map_err(|_| UmpOverrideError::Overflow)?;
	upward_messages
		.try_push(
...
async fn build_collation<Block: BlockT, RClient: RelayChainInterface + Clone + 'static>(
	...
	if let Some(signed_info) = scheduling_signals_override.as_ref() {
		if let Err(err) = override_ump_scheduling_tail(&mut collation.upward_messages, signed_info)
		{
			tracing::error!(
				target: LOG_TARGET,
				?err,
				?relay_parent,
				"Failed to override the UMP scheduling tail; dropping the collation.",
			);
			return None;
		}
	}
