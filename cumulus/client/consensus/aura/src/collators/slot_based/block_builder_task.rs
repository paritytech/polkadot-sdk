	...
	async move {
		...
		loop {
			...
			let max_claim_queue_offset =
				para_client.runtime_api().max_claim_queue_offset(best_hash).unwrap_or(2);
		...
		+ SchedulingV3EnabledApi<Block>,
{
	...
	let v3_enabled = para_client.runtime_api().scheduling_v3_enabled(parent_hash).unwrap_or(false);
