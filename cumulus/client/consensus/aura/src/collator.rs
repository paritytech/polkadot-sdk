...
use sp_additional_data::{AdditionalData, AdditionalDataGetter};
	...
	P::Signature: TryFrom<Vec<u8>> + Member + Codec,
{
	...
	pub async fn collate(
		...
		// additional-data channel. It requires `ParachainBlockData::V3`, which is only produced
		// when the relay chain's `CandidateReceiptV3` node feature (V3 scheduling) is enabled —
		// and only the slot-based collator gates on `v3_enabled`. So here no `AdditionalDataExt`
		// is registered and no additional data is carried (`build_block` yields
		// `BuiltBlock.additional_data = None`, so the produced `ParachainBlockData` is V1/V2).
		// See `build_block` / `BuildBlockAndImportParams::additional_data_handle` for the
		// slot-based path.
