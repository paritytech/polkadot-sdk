...
pub async fn find_parent_for_building<B: BlockT>(
	...
	filter_parent: impl Fn(&B::Header) -> bool,
	...
	let best_parent_header =
		find_deepest_valid_parent(start_hash, start_header, backend, &rp_ancestry, filter_parent);
...
fn find_deepest_valid_parent<Block: BlockT>(
	...
	while let Some(hash) = frontier.pop() {
		...
		if header.number() > best.number() && filter_parent(&header) {
			best = header.clone();
		}
