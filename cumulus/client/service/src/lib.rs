	...
	Network: NetworkBackend<Block, <Block as BlockT>::Hash>,
{
	let warp_sync_config = match parachain_config.network.sync_mode {
		SyncMode::Warp => {
			...
			let target_block =
				if let Ok(zombie_target_header_path) = std::env::var("ZOMBIE_TARGET_HEADER_PATH") {
					let header_content = tokio::fs::read_to_string(&zombie_target_header_path)
						.await
						.expect("env var ZOMBIE_TARGET_HEADER_PATH should be valid. qed");
					let header: <Block as BlockT>::Header =
						serde_json::from_str(&header_content).unwrap();
					log::info!(
