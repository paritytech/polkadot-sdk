...
impl SubstrateCli for Cli {
	...
	fn load_spec(&self, path: &str) -> std::result::Result<Box<dyn ChainSpec>, String> {
		// can only work with a chain spec for now.
		...
		None => {
			...
			runner.run_node_until_exit(|config| async move {
				...
				match config.chain_spec.runtime()? {
					...
					AssetHubKusama => crate::service::start_asset_hub_node::<RuntimeApi, AuraId>(
						config,
						polkadot_config,
						collator_options,
						id,
					...
					AssetHubRococo | AssetHubWestend =>
						crate::service::start_asset_hub_lookahead_node::<RuntimeApi, AuraId>(
							config,
							polkadot_config,
							collator_options,
							id,
					...
					CollectivesPolkadot => crate::service::start_generic_aura_node(
						config,
						polkadot_config,
						collator_options,
						id,
					...
					CollectivesWestend => crate::service::start_generic_aura_lookahead_node(
						config,
						polkadot_config,
						collator_options,
						id,
					...
					Seedling | Shell => crate::service::start_shell_node(
						config,
						polkadot_config,
						collator_options,
						id,
					...
					Penpal(_) | Default => crate::service::start_rococo_parachain_node(
						config,
						polkadot_config,
						collator_options,
						id,
					...
					Glutton | GluttonWestend => crate::service::start_basic_lookahead_node(
						config,
						polkadot_config,
						collator_options,
						id,
