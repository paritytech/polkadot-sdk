...
// possible to have a very small `Cargo.toml` file that only has one dependency, the umbrella crate.
// This helps with selecting the right combination of crate versions, since otherwise 3rd party
// tools are needed to select a compatible set of versions.
//
// ## Features
//
...
// crates, while the `node` feature enables all `std` crates. It should be used like any other crate
// in the repo, with `default-features = false`.
//
...
// - Example and fuzzing crates are not exported. This is currently detected by checking the name of
//   the crate for these magic words. In the future, it will utilize custom metadata, as it is done
//   in the `rococo-runtime` crate.
