...
//! The initial state conveyed in  the `GenesisConfig` struct is transformed into state storage items by
//! means of the [`BuildGenesisConfig`] trait which shall be implemented for the pallet's `GenesisConfig`
...
//! sneak peeked here: [`RuntimeGenesisConfig`].
//!
...
//! just provides a `Vec` of names of supported presets, while the latter one delegates the call to a
//! function that maps the name to a preset: [`chain_spec_guide_runtime::presets::get_builtin_preset`]
