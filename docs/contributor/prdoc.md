...
When creating a PR, the author needs to decide with the `R0-no-code-change` label whether the PR has to
...
picked if no other applies. The `None` option is equivalent to the `R0-no-code-change` label, but on a crate
