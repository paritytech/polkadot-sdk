...
/// Caps unmetered work on submitted execution-header RLP (Gloas). `submit` carries a fixed
/// weight but keccaks and walks these bytes, so without a ceiling a caller can send megabytes.
/// The worst case for the 23-field shape is 952 bytes: every integer field at its full 256-bit
/// width and `extra_data` at its 32-byte cap. 2048 leaves room for ~30 more fields. Exceeding
/// the bound fails at decode.
