...
mod gloas_end_to_end {
	...
	#[test]
	fn rejects_a_malformed_execution_header() {
		new_tester().execute_with(|| {
			store_header();
			let mut proof = execution_proof();
			proof.execution_header = VersionedExecutionPayloadHeader::Gloas(
				vec![0xc0; 8].try_into().expect("fits; qed"),
			);
			assert_err!(
				EthereumBeaconClient::verify_execution_proof(&proof),
				Error::<Test>::MalformedExecutionHeader
			);
		});
	}
