...
use bp_runtime::{
	...
	StorageProofChecker, StorageProofError, TrustedVecDb, UnderlyingChainProvider, UntrustedVecDb,
	VecDbError,
};
...
pub enum HeaderChainError {
	...
	/// Error generated by the `vec_db` module.
	VecDb(VecDbError),
}
...
pub trait HeaderChain<C: Chain> {
	...
	/// Get a `TrustedVecDb` starting from an `UntrustedVecDb`.
	fn verify_vec_db_storage(
		header_hash: HashOf<C>,
		db: UntrustedVecDb,
	) -> Result<TrustedVecDb, HeaderChainError> {
		...
		db.verify::<HasherOf<C>>(C::STATE_VERSION, &state_root)
			.map_err(HeaderChainError::VecDb)
	}
