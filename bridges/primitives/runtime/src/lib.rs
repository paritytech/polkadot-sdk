...
/// The version of offchain relayer, that is compatible with the pallet and runtime
/// configurations. Running relayer that is not compatible with that version may lead
/// to relayer slashing, losing tokens, submitting invalid transactions, ...
/// Every bridge pallet provides this version as a constant. It changes on some runtime
/// upgrades, which change the bridge code/logic, allowing relayer to keep running even
/// if unrelated runtime upgrade happens.
#[derive(
	Encode,
	Decode,
	Clone,
...
pub struct RelayerVersion {
	...
	/// new version.
	///
	/// **IMPORTANT**: it **CAN** be changed every time [`Self::auto`] value is changed.
	/// However, if tests have detected some change and [`Self::auto`] is changed, BUT
	/// you are 100% sure that it won't break existing relayer, you may keep the previous
	/// [`Self::manual`] value. It also **MUST** be changed if [`Self::auto`] stays the
	/// same BUT there's some breaking change that is not detected by tests.
	///
	/// Every change to this field shall result in building and deploying a new relayer
	/// version. This is the only field that is checked by the relayer.
	pub manual: u32,
	/// Version that is generated by runtime tests. Change it everytime when related
