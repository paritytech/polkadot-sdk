...
async function main() {
  ...
  // PAPI 2.0's getSmProvider expects a FACTORY function, not a chain object
  ...
  console.log("Calling getFinalizedBlock()...");
  const timeout = setTimeout(() => {
    console.log("TIMEOUT - getFinalizedBlock hung");
    process.exit(1);
  }, 30000);
  
  const block = await client.getFinalizedBlock();
  clearTimeout(timeout);
  console.log("Synced to block:", block.number);
  
  // Now try a transaction
  const api = client.getUnsafeApi();
  const tx = api.tx.System.remark({ remark: Binary.fromText("smoldot-papi2-works") });
