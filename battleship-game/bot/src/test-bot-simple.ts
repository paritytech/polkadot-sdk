...
async function simpleTest() {
  ...
  if (!joinSuccess) throw new Error('Failed to join game');
  ...
  if (!commitSuccess) throw new Error('Failed to commit grid');
  ...
  if (!gameStarted) throw new Error('Game did not start in time');
