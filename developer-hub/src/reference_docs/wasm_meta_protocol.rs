...
//!
//! The rest of the system that is NOT the state transition function is called the **client**, and
...
//! Runtime can be updates on the fly, within the execution of a block, and the client is (for the
...
//!
//! The client and the runtime need to communicate. This is done through two concepts:
//!
//! 1. **Host functions**: a way for the (WASM) runtime to talk to the client. All host functions
...
//! set of host functions that the runtime at any point in time requires. Given the fact that a
//! runtime can evolve in time, and a blockchain client (typically) wishes to be capable of
//! re-executing all the previous blocks, this means that a client must always maintain support for
//! the old host functions. This also implies that adding a new host function is a big commitment
//! and should be done with care. This is why, for example, adding a new host function to Polkadot
//! always requires an RFC.
//!
//! ## Client vs. Runtime
//!
//! A common question is: which components of the system end up being part of the client, and which
//! ones of the runtime?
//!
...
//! facilitate/enable it is part of the client. For example, the database, networking, and even
//! consensus algorithm are all client-side components.
//!
...
//! ## State
//!
//! From the previous sections, we know that the a database component is part of the client, not the
//! runtime. We also hinted that a set of host functions ([`sp_io::storage`]) are how the runtime
//! issues commands to the client to read/write to the state. Let's dive deeper into this.
//!
//! The state of the blockchain, what we seek to come to consensus about, is indeed *kept* in the
//! client side. Nonetheless, the runtime is the only component that:
//!
//! 1. Can update the state.
//! 2. Can fully interpret the state.
//!
//! In fact, [`sp_core::storage::well_known_keys`] are the only state keys that the client side is
...
//!
//! In the above diagram, all of the state keys and values are opaque bytes to the client. The
//! client does not know what they mean, and it does not now what is the type of the corresponding
//! value (e.g. if it is a number of a vector). Contrary, the runtime knows both the meaning of their
...
//! because the client side code is kept oblivious to all of the details of the state transition
//! function. Therefore, the state transition function can freely upgrade without the client needing
//! to know.
//!
